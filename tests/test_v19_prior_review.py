...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import prior_review as V, source_prior as S, reachable_retrospective as Q
...
def test_undefined_error_and_permutation():
    ...
    for key in ('recency_group_tv', 'early_group_tv'):
...
def test_complete_integration_and_corruptions(tmp_path, corrupt):
    ...
    if corrupt in ('hash', 'pair', 'count'):
        ...
        path = root/'inputs/original/raw/source_prior_summary_points.json.gz'
        rows = json.loads(gzip.decompress(path.read_bytes()))
        if corrupt == 'summary': rows[0]['recency_expected_source_entropy'] += .1
