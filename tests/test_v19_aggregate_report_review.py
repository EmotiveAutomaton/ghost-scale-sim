...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import aggregate_report_review as V, aggregate_report_state as S, reachable_retrospective as Q
...
def test_undefined_error_and_permutation():
    ...
    for key in ('future_squared_error', 'max_future_probability_error', 'updated_group_total_variation'):
        broken = {k:v.copy() for k, v in saved.items()}
        pos = np.flatnonzero(np.isnan(broken[key]))
        assert len(pos)
        broken[key].flat[pos[0]] = 0.
        with pytest.raises(ValueError, match='undefined'): V.reconstruct(w, st, p, ids, broken)
...
def complete(tmp_path):
    ...
    producer['design']['report_state'] = 'fixed-uniform-source-joint-group-endpoint'
...
def test_complete_integration_and_corruptions(tmp_path, corrupt):
    ...
    if corrupt in ('hash', 'pair', 'count'):
        ...
        path = root/'inputs/original/raw/aggregate_report_summary_points.json.gz'
        rows = json.loads(gzip.decompress(path.read_bytes()))
        if corrupt == 'summary': rows[0]['aggregate_state_bytes'] += .1
