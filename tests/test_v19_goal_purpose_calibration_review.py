"""Independent purpose grouping, empty denominators and corruption controls."""
import copy
import gzip
import json
import shutil
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import goal_purpose_calibration_review as V
from ghostscale.validation.soundingline.v19 import goal_purpose_calibration as O
from ghostscale.validation.soundingline.v18_3.io import read,write,canonical,file_digest
from test_v19_goal_purpose_calibration import fixture as producer_fixture
...
def fixture(root):
    ...
    for n in ('PLAN.json','SUMMARY.json','goal_purpose_calibration_points.json.gz','ARRAY_SCHEMA.json'):
...
def test_known_cancellation_permutations_mass_and_empty_means():
    ...
    rng=np.random.default_rng(191018);p=rng.dirichlet([1,2,3],(17,3));c=rng.dirichlet([2,1,3],(2,17,3));w=rng.dirichlet(np.ones(17),2);ops=rng.integers(2,size=(17,3))
    r=V.sufficient(p,c,w,ops);assert V.compare_arrays(r,O.sufficient(p,c,w,ops))<1e-14
    for rr in (V.sufficient(p[::-1],c[:,::-1],w[:,::-1],ops[::-1]),V.sufficient(p,c,w,1-ops),V.sufficient(p[:,:,::-1],c[:,:,:,::-1],w,ops)):
    ...
    assert np.allclose(r['purpose_mass'].sum(3),1)
...
def test_invalid_inputs(problem):
    ...
    elif problem=='operation':ops[0,0]=2
...
def test_corruptions(tmp_path,problem):
    ...
    if problem in ('probability','marginal','operation','decision','sum','empty','group-mass','signed','metric','native'):
        ...
        field={'probability':'probabilities','marginal':'marginals','operation':'operations','decision':'coordinate','sum':'bin_forecast_sum','empty':'bin_forecast_mean','group-mass':'purpose_mass','signed':'conditional_signed_error','metric':'cancellation_gap','native':'bin_correct_sum'}[problem]
        ...
        p=base/('parent/parent/goal_decision_points.json.gz' if problem=='parent-score' else 'original/goal_purpose_calibration_points.json.gz');v=json.loads(gzip.decompress(p.read_bytes()))
        ...
        elif problem=='operation-design':v['design']['purposes'].reverse()
        else:v['axes'].reverse()
...
def test_independent_request_membership_and_target_rejection():
    packets=[]
    for request in (0,1):
        packets.append(dict(schema='v19.local.public.1',tier='E2-full',inputs=dict(
            artifact=[0,0,0],initial=[0,0,0],requested_purpose=request,
            observations=[dict(step=i,operation='inspect',before=[0,0,0],
                after=[0,0,0],tool_proposal=None) for i in range(3)])))
    expected=np.array([[0,0,0],[1,1,1]])
    assert np.array_equal(V.purpose_groups(packets),expected)
    alternate=copy.deepcopy(packets)
    for p in alternate:
        p['inputs']['artifact']=[1,1,1]
        for e in p['inputs']['observations']:
            e['operation']='accept-tool';e['tool_proposal']=e['after']
    assert np.array_equal(V.purpose_groups(alternate),expected)
    for bad in (dict(goal=0),dict(hidden_adopted_purpose=1)):
        changed=copy.deepcopy(packets);changed[0]['inputs'].update(bad)
        with pytest.raises(ValueError):V.purpose_groups(changed)
    for invalid in (2,-1,False,0.0,'0',None):
        changed=copy.deepcopy(packets);changed[0]['inputs']['requested_purpose']=invalid
        with pytest.raises(ValueError):V.purpose_groups(changed)


def test_purpose_marginalization_matches_ungrouped_scalar_bins():
    import math
    rng=np.random.default_rng(191018)
    p=rng.dirichlet([1,2,3],(31,3));c=rng.dirichlet([2,1,3],(2,31,3))
    w=rng.dirichlet(np.ones(31),2);groups=rng.integers(2,size=(31,3))
    out=V.sufficient(p,c,w,groups)
    for li in range(2):
        for t in range(3):
            for g in range(3):
                for b in range(10):
                    ids=[i for i in range(31) if V.bin_id(p[i,t,g])==b]
                    for key,values in (('bin_weight',w[li]),('bin_forecast_sum',w[li]*p[:,t,g]),('bin_correct_sum',w[li]*c[li,:,t,g])):
                        assert math.fsum(out[key][li,t,g,:,b])==pytest.approx(math.fsum(values[i] for i in ids),abs=1e-14)


def test_single_request_identity_with_nonzero_error():
    p=np.tile([.6,.3,.1],(2,3,1));c=np.tile([.4,.5,.1],(1,2,3,1))
    w=np.array([[.2,.8]]);groups=np.zeros((2,3),int)
    result=V.sufficient(p,c,w,groups)
    assert np.allclose(result['grouped_bin_error'],result['pooled_bin_error'],atol=1e-14)
    assert result['grouped_bin_error'][0,0,0]==pytest.approx(.2)
    assert np.max(abs(result['cancellation_gap']))<1e-14
    assert np.isnan(result['conditional_signed_error'][...,1]).all()
    for invalid in (groups.astype(float),groups.astype(bool)):
        with pytest.raises(ValueError,match='purpose'):V.sufficient(p,c,w,invalid)
