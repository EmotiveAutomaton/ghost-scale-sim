...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import forgetting_review as V, retrospective_forgetting as S, reachable_retrospective as Q
...
def test_complete_integration_and_corruptions(tmp_path, corrupt):
    ...
    if corrupt in ('hash', 'pair', 'count'):
        ...
        path = root/'inputs/original/raw/retrospective_forgetting_summary_points.json.gz'
        rows = json.loads(gzip.decompress(path.read_bytes()))
        if corrupt == 'summary': rows[0]['half_expected_squared_regret'] += .1
