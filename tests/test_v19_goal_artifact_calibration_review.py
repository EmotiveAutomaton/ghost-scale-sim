"""Independent artifact grouping, empty denominators and corruption controls."""
import copy
import gzip
import json
import shutil
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import goal_artifact_calibration_review as V
from ghostscale.validation.soundingline.v19 import goal_artifact_calibration as O
from ghostscale.validation.soundingline.v18_3.io import read,write,canonical,file_digest
from test_v19_goal_artifact_calibration import fixture as producer_fixture
...
def fixture(root):
    ...
    for n in ('PLAN.json','SUMMARY.json','goal_artifact_calibration_points.json.gz','ARRAY_SCHEMA.json'):
...
def test_known_cancellation_permutations_mass_and_empty_means():
    ...
    rng=np.random.default_rng(191017);p=rng.dirichlet([1,2,3],(17,3));c=rng.dirichlet([2,1,3],(2,17,3));w=rng.dirichlet(np.ones(17),2);ops=rng.integers(8,size=(17,3))
    r=V.sufficient(p,c,w,ops);assert V.compare_arrays(r,O.sufficient(p,c,w,ops))<1e-14
    for rr in (V.sufficient(p[::-1],c[:,::-1],w[:,::-1],ops[::-1]),V.sufficient(p,c,w,7-ops),V.sufficient(p[:,:,::-1],c[:,:,:,::-1],w,ops)):
    ...
    assert np.allclose(r['artifact_mass'].sum(3),1)
...
def test_invalid_inputs(problem):
    ...
    elif problem=='operation':ops[0,0]=8
...
def test_corruptions(tmp_path,problem):
    ...
    if problem in ('probability','marginal','operation','decision','sum','empty','group-mass','signed','metric','native'):
        ...
        field={'probability':'probabilities','marginal':'marginals','operation':'operations','decision':'coordinate','sum':'bin_forecast_sum','empty':'bin_forecast_mean','group-mass':'artifact_mass','signed':'conditional_signed_error','metric':'cancellation_gap','native':'bin_correct_sum'}[problem]
        ...
        p=base/('parent/parent/goal_decision_points.json.gz' if problem=='parent-score' else 'original/goal_artifact_calibration_points.json.gz');v=json.loads(gzip.decompress(p.read_bytes()))
        ...
        elif problem=='operation-design':v['design']['artifacts'].reverse()
        else:v['axes'].reverse()
...
def test_independent_endpoint_membership_all_eight_and_target_rejection():
    from itertools import product
    packets=[]
    for endpoint in product((0,1),repeat=3):
        packets.append(dict(schema='v19.local.public.1',tier='E2-full',inputs=dict(
            artifact=list(endpoint),initial=list(endpoint),requested_purpose=0,
            observations=[dict(step=i,operation='inspect',before=list(endpoint),
                after=list(endpoint),tool_proposal=None) for i in range(3)])))
    expected=np.repeat(np.arange(8)[:,None],3,axis=1)
    assert np.array_equal(V.artifact_groups(packets),expected)
    alternate=copy.deepcopy(packets)
    for p in alternate:
        p['inputs']['requested_purpose']=1
        for e in p['inputs']['observations']:
            e['operation']='accept-tool';e['tool_proposal']=e['after']
    assert np.array_equal(V.artifact_groups(alternate),expected)
    for bad in (dict(goal=0),dict(hidden_adopted_purpose=1)):
        changed=copy.deepcopy(packets);changed[0]['inputs'].update(bad)
        with pytest.raises(ValueError):V.artifact_groups(changed)
    for invalid in ([2,0,0],[0,0],[False,0,0]):
        changed=copy.deepcopy(packets);changed[0]['inputs']['artifact']=invalid
        with pytest.raises(ValueError):V.artifact_groups(changed)


def test_artifact_marginalization_matches_ungrouped_scalar_bins():
    import math
    rng=np.random.default_rng(191017)
    p=rng.dirichlet([1,2,3],(31,3));c=rng.dirichlet([2,1,3],(2,31,3))
    w=rng.dirichlet(np.ones(31),2);groups=rng.integers(8,size=(31,3))
