...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import context_review as V, report_context as S, reachable_retrospective as Q
...
def test_undefined_error_and_permutation():
    ...
    for key in ('source_entropy', 'future_squared_gain', 'future_optimal_squared_loss', 'context_to_endpoint_squared_regret'):
...
def test_complete_integration_and_corruptions(tmp_path, corrupt):
    ...
    if corrupt in ('hash', 'pair', 'count'):
        ...
        path = root/'inputs/original/raw/report_context_summary_points.json.gz'
        rows = json.loads(gzip.decompress(path.read_bytes()))
        if corrupt == 'summary': rows[0]['endpoint_expected_source_entropy'] += .1
...
def test_context_permutation_and_constant_context():
    w, st, p, ids, saved = example()
    _, a = V.calculate(w, st, p, ids)
    permutation = np.array([2, 0, 3, 1])
    _, b = V.calculate(w, st, p[:, np.argsort(permutation)], [[t, permutation[c], e] for t, c, e in ids])
    for key in a: np.testing.assert_allclose(a[key], b[key], atol=1e-12, rtol=0)
    _, same = V.calculate(w, st, p, [[t, 0, e] for t, c, e in ids])
    np.testing.assert_allclose(same['context_added_future_gain'], 0., atol=1e-12, rtol=0)
