...
def test_final_cursor_transition_does_not_hide_frozen_plan_or_failure_changes(tmp_path):
    root = tmp_path/"results/v16"
    for name in ["pyproject.toml", "uv.lock", "docs/METHODS.md", "docs/versions/v16-acquired-craft/CODING_PACKAGE.md"]:
        path = tmp_path/name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("known fixture", encoding="utf-8")
    for name in ["operations/ACTIVE_JOB.json", "operations/QUEUE_PLAN.json", "operations/failures/0001.json", "setup/operations/ACTIVE_JOB.json"]:
        write(root/name, {"job": "confirmation"})
    before = snapshot(tmp_path, root)
    assert "results/v16/operations/ACTIVE_JOB.json" not in before
    assert "results/v16/setup/operations/ACTIVE_JOB.json" in before
    write(root/"operations/ACTIVE_JOB.json", {"job": "closeout"}, immutable=False)
    assert snapshot(tmp_path, root) == before
    write(root/"operations/failures/0001.json", {"changed_failure": True}, immutable=False)
    assert snapshot(tmp_path, root) != before
    assert "results/v16/operations/QUEUE_PLAN.json" in before
