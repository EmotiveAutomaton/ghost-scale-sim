...
import uuid
import pytest
from ghostscale.validation.soundingline.v16.transfer_stable import (
...
def test_amended_standalone_transfer_known_answer_and_private_read_denial(tmp_path):
...
def test_amended_standalone_transfer_learning_changes_executed_competence_and_noise_does_not(tmp_path):
...
def test_amended_stage9_transfer_has_complete_opaque_options_and_exact_evidence_shape():
    public = wrapped("reading", reading_observation(), strategy="maker")
...
def test_amended_standalone_retains_recoding_and_zero_value_repairs(tmp_path):
    from ghostscale.validation.soundingline.v16.inquiry_stable_reference import chosen, PRIOR, MAPS
    from ghostscale.validation.soundingline.v16.recoding import inquiry_observation, inquiry_result_equivalent
    public = {"schema_version": "v16.inquiry.1", "task_id": "fixture", "beliefs": [
        [1/6 if mapping[0] == 2 else 0.0 for mapping in MAPS]+[0.0], list(PRIOR)],
    ...
    install_consumer(tmp_path)
    extension = "ghostscale.validation.soundingline.v16.inquiry_stable:choose"
    with Consumer(tmp_path) as reader:
        before = reader.request(wrapped(extension, public, policy="value-learning"))["result"]
        expected = chosen(public, "value-learning")
        assert before["domain"] == expected["domain"]
        assert before["scores"] == pytest.approx(expected["scores"], abs=1e-10)
        renamed = inquiry_observation(public, (2, 0, 3, 1))
        after = reader.request(wrapped(extension, renamed, policy="value-learning"))["result"]
        assert inquiry_result_equivalent("inquiry", before, after, public, renamed, (2, 0, 3, 1))
        mixed = [0.99]+[0.0]*23+[0.01]
        public.update(beliefs=[mixed, mixed], opportunity_cost=0.0, feedback_batches=[1, 1])
        assert reader.request(wrapped(extension, public, policy="value-learning"))["result"]["domain"] is None


def test_amended_transfer_evaluator_requires_committed_predictions_and_rejects_changed_results(tmp_path):
    public = wrapped("reading", reading_observation(), strategy="maker")
