...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import coarsening_review as V, report_coarsening as S, reachable_retrospective as Q
...
def test_undefined_error_and_permutation():
    ...
    for key in ('source_entropy', 'future_squared_gain', 'future_optimal_squared_loss', 'fine_to_coarse_squared_regret'):
...
def test_complete_integration_and_corruptions(tmp_path, corrupt):
    ...
    if corrupt in ('hash', 'pair', 'count'):
        ...
        path = root/'inputs/original/raw/report_coarsening_summary_points.json.gz'
        rows = json.loads(gzip.decompress(path.read_bytes()))
        if corrupt == 'summary': rows[0]['fine_expected_source_entropy'] += .1
