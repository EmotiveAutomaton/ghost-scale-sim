...
def fixture(root):
    ...
    p['design'].update(resolutions=[5,10,20],bin_edges={str(n):O.EDGES[n].tolist() for n in O.RESOLUTIONS},bootstrap_seed=191021)
    binroot=root/'bin-fixture';binroot.mkdir()
    shutil.copytree(root/'inputs',binroot/'inputs')
    O.B.run(binroot,p,lambda **kw:None)
    (root/'inputs/parent_bin').mkdir()
    shutil.copyfile(binroot/'goal_bin_resolution_points.json.gz',root/'inputs/parent_bin/goal_bin_resolution_points.json.gz')
    p['design']['input_files']={f.relative_to(root/'inputs').as_posix():file_digest(f) for f in (root/'inputs').rglob('*') if f.is_file()}
    return p
...
def test_known_opposite_and_identical_errors_and_native_self():
    ...
    assert x['absolute_error'][0,0,0]==pytest.approx(.05)
    assert x['gap_5'][0,0,0]==pytest.approx(.05)
    assert x['gap_20'][0,0,0]==pytest.approx(0,abs=1e-14)
...
def test_empty_means_probability_one_and_faulty_assignment(monkeypatch):
    ...
    def wrong(p,n):
        ...
        return v
    monkeypatch.setattr(O.B,'bins',wrong)
...
def test_complete_handler_roster_and_fixed_unbinned_score(tmp_path):
    ...
    rows=json.loads(gzip.decompress((root/'goal_absolute_envelope_points.json.gz').read_bytes()))
    ...
    assert len(s['envelope_estimates'])==len(s['means'])*3
    ...
    for f in (root/'residuals').glob('*.npz'):
        with np.load(f,allow_pickle=False) as z:
            assert z['signed'].shape==(len(cfg['development_lineages']),s['packets'],3,3)
            assert z['absolute_contribution'].shape==(len(cfg['development_lineages']),2,s['packets'],3,3)
            assert np.isfinite(z['signed']).all() and np.min(z['absolute_contribution'])>=0
            with np.load(root/'groups'/f.name,allow_pickle=False) as other:
                assert np.allclose(z['absolute_contribution'].sum(2),other['absolute_error'],rtol=0,atol=1e-14)
    for bad in (rows[:-1],rows+[rows[0]]):
        with pytest.raises(ValueError,match='roster'):O.aggregate(bad,cfg)


@pytest.mark.parametrize('problem',['hash','missing','duplicate','parent-score','reader','reference','mass','population','bins','resolutions','probability','marginal','operation','decision','bin-parent-score','bin-parent-missing'])
def test_corruptions(tmp_path,problem):
    ...
    elif problem in ('bin-parent-score','bin-parent-missing'):
        p=base/'parent_bin/goal_bin_resolution_points.json.gz';v=json.loads(gzip.decompress(p.read_bytes()))
        if problem.endswith('missing'):v.pop()
...
def test_scalar_absolute_residuals_contributions_and_nesting():
    rng=np.random.default_rng(191021)
    p=rng.dirichlet([1,2,3],(19,3));target=rng.dirichlet([3,2,1],(2,19,3));w=rng.dirichlet(np.ones(19),2)
    result=O.sufficient(p,target,w)
    for law,t,g in product(range(2),range(3),range(3)):
        signed=[float(p[i,t,g])-float(target[law,i,t,g]) for i in range(19)]
        absolute=math.fsum(float(w[law,i])*abs(signed[i]) for i in range(19))
        assert result['absolute_error'][law,t,g]==pytest.approx(absolute,abs=1e-14)
        assert np.array_equal(result['frame_residual'][law,:,t,g],signed)
        for n in (5,10,20):
            # Sum residuals directly, rather than subtracting bin forecast and target sums.
            binned=math.fsum(abs(math.fsum(float(w[law,i])*signed[i] for i in range(19)
                if sum(float(p[i,t,g])>=j/n for j in range(1,n))==b)) for b in range(n))
            assert result[f'gap_{n}'][law,t,g]==pytest.approx(absolute-binned,abs=1e-14)
        for i in range(19):assert result['frame_absolute_contribution'][law,i,t,g]==float(w[law,i])*abs(signed[i])
    assert np.min(result['gap_20'])>=-1e-14
    assert np.min(result['gap_5']-result['gap_10'])>=-1e-14
    assert np.min(result['gap_10']-result['gap_20'])>=-1e-14


def test_zero_weight_erases_error_and_unequal_weight_cancellation():
    p=np.tile([.15,.85,0.],(2,3,1));c=p[None].copy()
    c[0,0,:,:2]+=[.05,-.05];c[0,1,:,:2]+=[-.05,.05]
    x=O.sufficient(p,c,np.array([[.25,.75]]))
    assert x['absolute_error'][0,0,0]==pytest.approx(.05)
    assert x['gap_20'][0,0,0]==pytest.approx(.025)
    c[0,0]=p[0]
    zero=O.sufficient(p,c,np.array([[1.,0.]]))
    assert all(np.max(abs(zero[k]))==0 for k in O.METRICS)


def test_faulty_parent_binned_error_breaks_bound(monkeypatch):
    old=O.B.sufficient
    def corrupt(*args):
        result=old(*args);result['bin_error_20']+=.1;return result
    monkeypatch.setattr(O.B,'sufficient',corrupt)
    p=np.tile([.15,.85,0.],(2,3,1))
    with pytest.raises(ValueError,match='triangle'):O.sufficient(p,p[None],np.array([[.5,.5]]))
