...
def fixture(root):
    ...
    p['design'].update(bin_edges=O.C.EDGES.tolist(),bootstrap_seed=191018,purposes=list(O.PURPOSES))
    return p


def scalar(p,c,w,ops):
    shape=(len(c),3,3,2,10);out={k:np.zeros(shape) for k in ('bin_weight','bin_forecast_sum','bin_correct_sum')}
    for law,t,g,op,b in product(range(len(c)),range(3),range(3),range(2),range(10)):
...
def test_all_sums_against_scalar_and_group_frame_class_permutations():
    rng=np.random.default_rng(191016);p=rng.dirichlet([1,2,3],(17,3));c=rng.dirichlet([2,1,3],(2,17,3));w=rng.dirichlet(np.ones(17),2);ops=rng.integers(2,size=(17,3))
    r=O.sufficient(p,c,w,ops)
    for k,v in scalar(p,c,w,ops).items():assert np.allclose(v,r[k],atol=1e-14,rtol=0)
    for permuted in (O.sufficient(p[::-1],c[:,::-1],w[:,::-1],ops[::-1]),O.sufficient(p,c,w,1-ops),O.sufficient(p[:,:,::-1],c[:,:,:,::-1],w,ops)):
...
def test_complete_handler_roster_and_all_empty_groups_retained(tmp_path):
    ...
    rows=json.loads(gzip.decompress((root/'goal_purpose_calibration_points.json.gz').read_bytes()))
    ...
    for f in (root/'groups').glob('*.npz'):
        with np.load(f,allow_pickle=False) as z:
            assert z['bin_weight'].shape==(len(cfg['development_lineages']),2,3,3,2,10)
...
def test_corruptions(tmp_path,problem):
    ...
    elif problem=='operation-design':plan['design']['purposes'].reverse()
    else:
...
def test_partition_uses_request_not_adopted_purpose_artifact_or_operations():
    packets=[]
    for purpose,artifact in product(O.PURPOSES,product((0,1),repeat=3)):
        packets.append(dict(schema='v19.local.public.1',tier='E2-full',inputs=dict(
            artifact=list(artifact),initial=list(artifact),requested_purpose=purpose,
            observations=[dict(step=i,operation='inspect',before=list(artifact),
                after=list(artifact),tool_proposal=None) for i in range(3)])))
    assert np.array_equal(O.purpose_groups(packets),np.repeat(np.repeat([0,1],8)[:,None],3,axis=1))
    changed=copy.deepcopy(packets)
    for p in changed:
        p['inputs']['artifact']=[1-x for x in p['inputs']['artifact']]
        for e in p['inputs']['observations']:
            e['operation']='accept-tool';e['tool_proposal']=e['after']
    assert np.array_equal(O.purpose_groups(packets),O.purpose_groups(changed))
    changed[0]['inputs']['goal']=0
    with pytest.raises(ValueError):O.purpose_groups(changed)
    for bad in (2,-1,True,0.,None,'0'):
        changed=copy.deepcopy(packets);changed[0]['inputs']['requested_purpose']=bad
        with pytest.raises(ValueError):O.purpose_groups(changed)


def test_one_request_identity_and_retained_empty_group():
    p=np.tile([.6,.3,.1],(2,3,1));c=p[None].copy()
    c[0,0,:,0]+=.1;c[0,0,:,1]-=.1
    z=O.sufficient(p,c,np.array([[.5,.5]]),np.zeros((2,3),int))
    assert np.allclose(z['pooled_bin_error'],z['grouped_bin_error'],atol=1e-14,rtol=0)
    assert np.max(abs(z['cancellation_gap']))<1e-14
    assert np.isnan(z['conditional_signed_error'][...,1]).all()
    assert np.all(z['purpose_mass'][...,1]==0)
