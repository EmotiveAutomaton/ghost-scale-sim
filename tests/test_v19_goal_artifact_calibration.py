...
def fixture(root):
    ...
    p['design'].update(bin_edges=O.C.EDGES.tolist(),bootstrap_seed=191017,artifacts=[list(a) for a in O.ARTIFACTS])
    return p


def scalar(p,c,w,ops):
    shape=(len(c),3,3,8,10);out={k:np.zeros(shape) for k in ('bin_weight','bin_forecast_sum','bin_correct_sum')}
    for law,t,g,op,b in product(range(len(c)),range(3),range(3),range(8),range(10)):
...
def test_all_sums_against_scalar_and_group_frame_class_permutations():
    rng=np.random.default_rng(191016);p=rng.dirichlet([1,2,3],(17,3));c=rng.dirichlet([2,1,3],(2,17,3));w=rng.dirichlet(np.ones(17),2);ops=rng.integers(8,size=(17,3))
    r=O.sufficient(p,c,w,ops)
    for k,v in scalar(p,c,w,ops).items():assert np.allclose(v,r[k],atol=1e-14,rtol=0)
    for permuted in (O.sufficient(p[::-1],c[:,::-1],w[:,::-1],ops[::-1]),O.sufficient(p,c,w,7-ops),O.sufficient(p[:,:,::-1],c[:,:,:,::-1],w,ops)):
...
def test_complete_handler_roster_and_all_empty_groups_retained(tmp_path):
    ...
    rows=json.loads(gzip.decompress((root/'goal_artifact_calibration_points.json.gz').read_bytes()))
    ...
    for f in (root/'groups').glob('*.npz'):
        with np.load(f,allow_pickle=False) as z:
            assert z['bin_weight'].shape==(len(cfg['development_lineages']),2,3,3,8,10)
...
def test_corruptions(tmp_path,problem):
    ...
    elif problem=='operation-design':plan['design']['artifacts'].reverse()
    else:
...
def test_partition_uses_all_public_endpoints_and_no_operation_or_teacher():
    packets=[]
    for artifact in O.ARTIFACTS:
        packets.append(dict(schema='v19.local.public.1',tier='E2-full',inputs=dict(
            artifact=list(artifact),initial=list(artifact),requested_purpose=0,
            observations=[dict(step=i,operation='inspect',before=list(artifact),
                after=list(artifact),tool_proposal=None) for i in range(3)])))
    assert np.array_equal(O.artifact_groups(packets),np.repeat(np.arange(8)[:,None],3,axis=1))
    ...
    assert np.array_equal(O.artifact_groups(packets),O.artifact_groups(changed))
    changed[0]['inputs']['goal']=0
    with pytest.raises(ValueError):O.artifact_groups(changed)
    changed=copy.deepcopy(packets);changed[0]['inputs']['artifact'][0]=2
    with pytest.raises(ValueError):O.artifact_groups(changed)
