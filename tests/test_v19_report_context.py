...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import report_context as S, reachable_retrospective as Q
...
def test_scalar_bayes_all_reports_and_one_hot_loss(mode, sparse):
    ...
    for ai, a in enumerate(S.ALPHAS):
        values = {}
        for ri in range(40):
            endpoint = ri if ri < 8 else (ri-8) % 8
            ctx = None if ri < 8 else (ri-8)//8
            joint = np.array([[w[h]*(a*(old == endpoint)+(1-a)*p[st['past'][t-1, h], c, endpoint])*(ctx is None or c == ctx)/len(ids) for h in range(len(w))] for t, c, old in ids])
            ...
            if den == 0:
                ...
                if ri >= 8: assert np.isnan(raw['context_to_endpoint_squared_regret'][ai, ri-8])
                continue
        ...
        for label, indices in [('endpoint', range(8)), ('context', range(8, 40))]:
        ...
        for ri in range(8, 40):
            if ri in values:
                d = values[ri][4]-values[(ri-8)%8][4]
                assert raw['context_to_endpoint_squared_regret'][ai, ri-8] == pytest.approx((d*d).sum(-1).mean(), abs=4e-15)


def test_source_and_context_permutation_and_identity_controls():
    ...
    permutation = np.array([2, 0, 3, 1]); moved = p[:, np.argsort(permutation)]
    reordered = S.summarize(S.evaluate(w, st, moved, [[t, permutation[c], e] for t, c, e in ids]))
    for k in a:
        np.testing.assert_allclose(a[k], b[k], atol=4e-15, rtol=0)
        np.testing.assert_allclose(a[k], reordered[k], atol=4e-15, rtol=0)
    one = S.summarize(S.evaluate(w, st, p, ids[:1]))
    for k, v in one.items():
        if 'entropy' in k or k in ('context_added_future_gain', 'context_to_endpoint_regret'): assert abs(v).max() < 1e-14
    same = S.summarize(S.evaluate(w, st, p, [[1, 0, 0], [2, 0, 1]]))
    assert abs(same['context_added_future_gain']).max() < 1e-14
...
def test_complete_native_parent_integration(tmp_path, corrupt):
    ...
    if corrupt:
        ...
        assert result['posterior_rows'] == 512 and result['sources'] == 768 and result['report_queries'] == 102400
        assert all(result['controls'].values()) and not result['numerical_acceptance']
        rows = json.loads(gzip.decompress((root/'raw/report_context_summary_points.json.gz').read_bytes()))
        assert len(rows) == 2560 and len(list((root/'raw').glob('*.npz'))) == 16
        assert all(r['context_added_future_gain'] >= -1e-12 for r in rows)
