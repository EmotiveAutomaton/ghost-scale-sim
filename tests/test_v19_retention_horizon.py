...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import retention_horizon as S, reachable_retrospective as Q
    ...
    for wi,label in enumerate(S.WINDOWS):
        ...
        for ai,a in enumerate(S.ALPHAS):
            for e in range(8):
                ...
                for si in range(3):
                    explicit=sum(truth[si,c,y]*(sum((future[si,c]-np.eye(8)[y])**2)-sum((truth[si,c]-np.eye(8)[y])**2)) for c in range(4) for y in range(8))/4
                    assert raw['span_squared_regret'][si,wi,ai,e]==pytest.approx(explicit,abs=4e-15)
                    assert raw['span_max_future_probability_error'][si,wi,ai,e]==pytest.approx(abs(delta[si]).max(),abs=4e-15)
...
def test_complete_native_parent_integration(tmp_path,corrupt):
    ...
    plan['design']['forecast_spans']=list(S.SPANS)
    if corrupt:
        ...
        rows=json.loads(gzip.decompress((root/'raw/retention_horizon_summary_points.json.gz').read_bytes()))
        assert len(rows)==2560 and len(list((root/'raw').glob('*.npz')))==16
        assert all(abs(r['full_expected_squared_regret'])<1e-12 for r in rows)


@pytest.mark.parametrize('times',list(range(1,130)))
def test_disjoint_complete_time_spans_and_weighted_recombination(times):
    spans=S.span_slices(times);indices=[list(range(times)[s]) for s in spans]
    assert sum(indices,[])==list(range(times))
    assert indices[0]==[0] and len(indices[1])==(times-1+1)//2
    assert len(indices[1])-len(indices[2]) in (0,1)


@pytest.mark.parametrize('future_steps',[0,1,2])
def test_empty_forecast_spans_are_undefined_and_full_horizon_reproduces(future_steps):
    ...
    w=np.arange(1,17,dtype=float);w/=w.sum();st=Q.prepare(spec,w);p=law('random');ids=[[1,0,0],[2,1,1]]
    ...
    for si,span in enumerate(S.SPANS):
        empty=raw['forecast_span_lengths'][si]==0
        assert np.isnan(raw['span_squared_regret'][si]).all()==empty
        assert np.isnan(summary[span+'_recent_half_expected_squared_regret']).all()==empty
    lengths=raw['forecast_span_lengths'];total=np.sum(np.nan_to_num(raw['span_squared_regret'])*lengths[:,None,None,None],axis=0)/lengths.sum()
    np.testing.assert_array_equal(np.isnan(raw['future_squared_regret']),~raw['comparable'])
    np.testing.assert_allclose(np.where(raw['comparable'],total,np.nan),raw['future_squared_regret'],atol=4e-15,rtol=0)
