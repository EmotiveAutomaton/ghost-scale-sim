...
def complete(root, name, card, packet_hash="known-packet"):
    base = root/name/card
    write(base/"AGGREGATE.json", {"card_id": card, "known": 1})
    write(base/"RAW_MANIFEST.json", {"files": {"known_points.json": "fixture"}})
    write(base/"COMPLETION.json", {"execution_state": "completed", "instrument_state": "valid", "packet_hash": packet_hash,
...
def test_incremental_analysis_resumes_completed_cards_without_recalculating_or_claiming_partial_completion(tmp_path, monkeypatch):
    ...
    packet = freeze(root, name, [Path(module.__file__)], {"cards": entries})
    ...
    complete(root, name, "K01", packet["packet_hash"])
    ...
    complete(root, name, "K03", packet["packet_hash"])
