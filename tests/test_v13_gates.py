"""Every committed V13 verdict must carry its plain-language record, provenance, gates,
...
V13 = REPO / "results" / "validation" / "soundingline" / "v13"
...
def verdicts():
    if not V13.exists():
        return []
    out = []
    for p in sorted(V13.rglob("*.json")):
        try:
            out.append((p.relative_to(V13).as_posix(), p, json.loads(p.read_text(encoding="utf-8"))))
        except json.JSONDecodeError:
            out.append((p.relative_to(V13).as_posix(), p, None))
    return out


ALL = verdicts()
pytestmark = pytest.mark.skipif(not ALL and not MANIFEST.exists(), reason="no V13 artefacts on disk yet")
