...
import numpy as np
import pytest
from ghostscale.validation.soundingline.v19 import retention_horizon_review as V, retention_horizon as S, reachable_retrospective as Q
...
def test_undefined_error_and_permutation():
    ...
    for key in ('future_squared_regret', 'max_future_probability_error', 'updated_group_total_variation', 'span_squared_regret', 'span_max_future_probability_error'):
...
def complete(tmp_path, empty=False):
    original, producer = complete_source_fixture(tmp_path/'producer')
    if empty:
        path = original/'inputs/SCHEDULES.json'
        specs = read(path); spec = specs['4-2']; spec['length'] = 2
        paths = [[spec['signatures'][g][0]] for g in spec['membership']]
        spec['signatures'] = sorted(set(tuple(p) for p in paths))
        spec['membership'] = [spec['signatures'].index(tuple(p)) for p in paths]
        write(path, specs, immutable=False)
        producer['design']['input_files']['SCHEDULES.json'] = file_digest(path)
    producer['design']['selection_rule'] = 'midpoint-sorted-time-ranks'
    producer['design']['forecast_spans'] = list(S.SPANS)
...
def test_complete_integration_and_corruptions(tmp_path, corrupt):
    ...
    if corrupt in ('hash', 'pair', 'count'):
        ...
        path = root/'inputs/original/raw/retention_horizon_summary_points.json.gz'
...
@pytest.mark.parametrize('times', range(1, 130))
def test_independent_disjoint_time_partition(times):
    groups = V.time_groups(times)
    assert sum(groups, []) == list(range(times))
    assert groups[0] == [0]
    assert len(groups[1]) == (times-1+1)//2
    assert len(groups[1])-len(groups[2]) in (0, 1)
    assert groups == [list(range(times)[s]) for s in S.span_slices(times)]


@pytest.mark.parametrize('future_steps', [0, 1, 2, 3, 6])
def test_short_horizons_null_masks_and_weighted_regroup(future_steps):
    cp=2
    ...
    w=np.arange(1,17,dtype=float);w/=w.sum();p=law('random');ids=[[1,0,0],[2,1,1]]
    saved=S.evaluate(w,Q.prepare(spec,w),p,ids)
    actual=V.reconstruct(w,V.structure(spec),p,ids,saved)
    expected=S.summarize(saved)
    for key in actual:np.testing.assert_allclose(actual[key],expected[key],atol=1e-12,rtol=0)
    for si, span in enumerate(V.SPANS):
        empty=saved['forecast_span_lengths'][si]==0
        assert np.isnan(actual[span+'_recent_half_expected_squared_regret']).all()==empty
        if empty:
            broken={k:v.copy() for k,v in saved.items()};broken['span_squared_regret'][si]=0
            with pytest.raises(ValueError,match='undefined mask'):V.reconstruct(w,V.structure(spec),p,ids,broken)
    assert V.nullable_mean([None]*128) is None
    with pytest.raises(ValueError,match='mixed unavailable'):V.nullable_mean([None,0.])
    assert V.nullable_mean([1.,3.])==2.


def test_summary_unavailable_cannot_become_zero(tmp_path):
    root,plan=complete(tmp_path)
    path=root/'inputs/original/raw/retention_horizon_summary_points.json.gz'
    rows=json.loads(gzip.decompress(path.read_bytes()))
    rows[0]['far_full_expected_squared_regret']=None
    path.write_bytes(gzip.compress(canonical(rows),mtime=0))
    plan['design']['input_files'][path.relative_to(root/'inputs').as_posix()]=file_digest(path)
    with pytest.raises(ValueError,match='summary unavailable'):V.run(root,plan,lambda **kw:None)


def test_empty_spans_remain_null_through_complete_pipeline(tmp_path):
    root, plan = complete(tmp_path, empty=True)
    result = V.run(root, plan, lambda **kw:None)
    assert result['passed'] and result['rows'] == 2560
    strata = read(root/'PAIRED_STRATA.json')
    assert len(strata) == 20
    for row in strata:
        assert row['current_steps'] == 1 and row['near_steps'] == row['far_steps'] == 0
        for span in ('near', 'far'):
            for label in V.WINDOWS:
                assert row[span+'_'+label+'_expected_squared_regret'] is None
                assert row[span+'_'+label+'_expected_max_probability_error'] is None
