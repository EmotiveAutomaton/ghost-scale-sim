...
def build_data(namespace,split,count,limited,heartbeat,aligned=False,probe_mode='standard',reader_facts=False):
    xs=[];ys=[];ids=[]
    for index in range(count):
        ...
        for j in range(4):
            if reader_facts:
                case['probes'][j]['context']['reader_fact']=random.Random(m.seed(namespace,split,index,j,'reader-fact')).choice([None,0,1])
...
def geometry(x,train_x,train_y,dev_x,dev_y,test_truth,seed):
    ...
    for nonlinear in (False,True):
        train_rep=np.tanh(a) if nonlinear else a
        test_rep=np.tanh(b) if nonlinear else b
        # Each basis must be fit in the representation that its decoder sees.
        # Reusing pre-nonlinearity PCA would weaken the named variance rival.
        for name in ('learned-alignment','variance-pca','random-axes'):
            projection=projection_basis(train_rep,y,name,random_axes)
            train=train_rep@projection
            test=test_rep@projection
    ...
    return result,dict(full_inverse_max_error=invariant,rank=8,preprocessing_dimension=d,
                       ...
                       inverse_shared=True,transform_seed=seed,projection_fit='matching linear or nonlinear training representation')


def projection_basis(train,y,method,random_axes,rank=8):
    centered=train-train.mean(0)
    if method=='learned-alignment':return np.linalg.svd(centered.T@(y-y.mean(0)),full_matrices=False)[0][:,:rank]
    if method=='variance-pca':return np.linalg.svd(centered,full_matrices=False)[2][:rank].T
    if method=='random-axes':return random_axes[:,:rank]
...
def run_comparison(root,design,limited,heartbeat):
    ...
    x,y,train_ids=build_data(design['namespace']+'-train','train',1250,limited,heartbeat,aligned,design.get('probe_mode','standard'),design.get('reader_facts',False))
    dx,dy,dev_ids=build_data(design['namespace']+'-dev','dev',64,limited,heartbeat,aligned,design.get('probe_mode','standard'),design.get('reader_facts',False))
    if (root/'FIT.json').exists():
        ...
        write(root/'FIT.json',dict(**receipt,training_makers=1250,training_history_episodes=10000,
              ...
              reader_facts_varied_in_training=design.get('reader_facts',False),
    ...
    if design.get('reader_facts'):
        for index,case in enumerate(cases):
            for j in range(4):case['probes'][j]['context']['reader_fact']=random.Random(m.seed(design['namespace'],'test',index,j,'reader-fact')).choice([None,0,1])
