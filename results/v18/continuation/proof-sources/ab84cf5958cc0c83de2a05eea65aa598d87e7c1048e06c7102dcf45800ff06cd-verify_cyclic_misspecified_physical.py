"""Verify completed QUEUE-15 without restarting science or modifying its source."""
import gzip
import hashlib
import json
import sys
...
SOURCE = LOCAL / "source-cyclic-misspecified-physical-1"
ROOT = LOCAL / "g2-cyclic-misspecified-physical-1"
REPORT = LOCAL / "g2-cyclic-misspecified-physical-report-1"
EXPECTED_EVENT_ID = "23fe8d84951f6ab224856a3296a90e2751117699086dbb9334a8e1c4de4617fc"
...
queue_status = read(LOCAL / "QUEUE-15-STATUS.json")
event = read(LOCAL / "QUEUE-15-EVENT.json")
event_id = hashlib.sha256(json.dumps(["QUEUE-15", event], sort_keys=True).encode()).hexdigest()
...
for name in complete["blocks"]:
    ...
    for unit in block["units"]:
        ...
        assert unit["case"]["truth_excluded"]
        assert unit["case"]["private"]["true_world"] not in unit["case"]["public"]["models"]
        seen.add(case_id)
...
write(
    REPORT / "LINEAGE.json",
    {
        ...
        "scope": (
            "exact event receipt; every raw case matches the frozen reused input; evaluator truth is absent "
            "from every supplied family; all frozen source bytes match; complete independent candidate-relative "
            "setup, actual setup-outcome, task physics and aggregation checks pass"
        ),
...
archive = LOCAL / "source-cyclic-misspecified-physical-1.zip"
