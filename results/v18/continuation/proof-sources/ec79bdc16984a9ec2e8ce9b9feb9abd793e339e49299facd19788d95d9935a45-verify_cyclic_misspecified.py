"""Verify completed QUEUE-14 without restarting science or modifying its source."""
import gzip
import hashlib
import json
import sys
...
SOURCE = LOCAL / "source-cyclic-misspecified-1"
ROOT = LOCAL / "g2-cyclic-misspecified-1"
REPORT = LOCAL / "g2-cyclic-misspecified-report-1"
EXPECTED_EVENT_ID = "e4eff449ef402611fde44dbaeca5633ca2970bf0e796564d3844816cef2618c4"
...
queue_status = read(LOCAL / "QUEUE-14-STATUS.json")
event = read(LOCAL / "QUEUE-14-EVENT.json")
event_id = hashlib.sha256(json.dumps(["QUEUE-14", event], sort_keys=True).encode()).hexdigest()
...
write(
    REPORT / "LINEAGE.json",
    {
        ...
        "scope": (
            "exact event receipt; every raw case matches the frozen input; evaluator truth "
            "is absent from every supplied candidate family; all frozen source bytes match; "
            "full independent physics and aggregation pass"
        ),
...
archive = LOCAL / "source-cyclic-misspecified-1.zip"
