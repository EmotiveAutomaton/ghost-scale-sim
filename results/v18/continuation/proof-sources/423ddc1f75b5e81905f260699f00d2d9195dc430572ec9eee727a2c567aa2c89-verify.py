...
def branch_report(root,output):
    ...
    dimensions={
      ...
      'g4-history':('family','condition','tier','method')}[branch]
    for name in completed['blocks']:
        ...
        for unit in block['units']:
            case=unit['case'];uid=case['structural_unit'];contexts.add(uid)
            ...
            for row in unit['rows']:
                ...
                if branch in ('g1-native','g2-native','g2-transfer','g3-representation'):
                    ...
                    if branch=='g3-representation':assert row['acquisition']['storage_tokens']<=row['storage_cap']
            ...
            for diagnostic in unit.get('diagnostics') or []:
