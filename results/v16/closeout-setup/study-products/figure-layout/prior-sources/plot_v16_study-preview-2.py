    ...
    for ax, cid, xkey, ykey, arms, title in [
        ...
        for arm_index, (color, arm) in enumerate(zip(COLORS, arms)):
            ...
            for x, y, row in zip(xs, ys, rows):
                positions.setdefault((x,y), []).append(str(row["condition_factors"]["search_primitive_budget"]))
            for position, budgets in positions.items():
                ax.annotate("/".join(budgets), position, xytext=(4,4+10*arm_index), textcoords="offset points", fontsize=8, color=color)
