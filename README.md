...
> ### ◐ Provenance, under the Ghost Scale
>
> This repository applies its own framework to itself. GitHub cannot render opacity, so the
> declaration is written out instead — the part that actually does the work is a reader knowing
> which mode to read in.
>
> | | section | tier | what that means |
> |---|---|---|---|
> | **◐** | everything from here down to and including *[Where the literature landed afterwards](#where-the-literature-landed-afterwards)* | **Curator — 60%** | a human and an AI were both directly involved. The questions, the orderings, the judgements about what matters and what is being over-claimed are the author's; the drafting is synthesis. |
> | **○** | *[What this is](#what-this-is)* onward | **Ghost — 5%** | generated with human direction and curation, not word-by-word authored. **Read it as reference material rather than as evidence of a person choosing every phrase.** Skim it, or hand it to a model and ask it what you want to know. |
>
> Nothing here is 100%. Claiming otherwise would be the exact failure the scale exists to catch.
>
> *Prompt, disclosed per the scale's own rule: the ghost sections below were produced across an
> extended working session in which the author directed the research programme, made every design
> decision, rejected several proposed experiments, and corrected the model's account of its own
> theory more than once. The results they describe are generated by the committed code and are
...
> ### ○ Ghost text begins here — 5% opacity
>
> Everything below this line is generated with human direction and curation. It is accurate and it
> is checkable — every number traces to a committed results file — but **it was not composed word
> by word by a person, and it does not repay being read as though it was.**
>
> If you want the human-curated layer, it is above this line, plus
> [FINDINGS.md](FINDINGS.md) and [WALKTHROUGH.md](WALKTHROUGH.md).
