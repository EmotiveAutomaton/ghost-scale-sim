"""Admit and execute the bounded consumer-specific X03 controls."""
import argparse
...
from ghostscale.validation.soundingline.v16.attack_collisions import DESIGN, TRANSFER_FAMILIES, execute
from ghostscale.validation.soundingline.v16.collision_cases import CASES
from ghostscale.validation.soundingline.v16.active_consumers import attack_consumers

PACKET = "collision-attack-fixture-1"
REQUIRED = ["test_collision_controls_reject_forced_certainty_and_ignored_separating_evidence"] + [
    f"test_actual_collision_and_separating_evidence_controls[{family}]" for family in sorted(CASES)] + [
    f"test_actual_standalone_collision_controls[{family}]" for family in TRANSFER_FAMILIES]
...
def sources(root):
    ...
    files = {REPO/"runners/attack_v16_collisions.py", PACKAGE/"attack_collisions.py",
             PACKAGE/"collision_cases.py", REPO/"tests/test_v16_collisions.py"}
...
def main():
    ...
    if args.resume and not (args.root/"packets"/f"{PACKET}.json").exists():
        raise ValueError("resume never prepares collision controls")
    files = sources(args.root)
    admission_path = args.root/"collision-setup/ADMISSION.json"
    if args.admit_from_junit:
        ...
        if not tests or any(value != "passed" for value in tests.values()) or any(name not in tests for name in REQUIRED):
            raise ValueError("collision admission tests are missing or failed")
    ...
    if any(admission["tests"].get(name) != "passed" for name in REQUIRED):
        raise ValueError("collision required executed admission missing")
    for path in files:
        if admission["source_hashes"].get(str(path.relative_to(REPO)).replace("\\", "/")) != file_digest(path):
            raise ValueError("collision source differs from executed test receipt")
    with supervisor(args.root, "collision-attack") as heartbeat:
        design = {**DESIGN, "consumer_cards": attack_consumers(args.root)["X03"]}
