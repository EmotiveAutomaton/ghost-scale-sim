...
def inventory(root, name):
    known_profile = name == "expansion-fixture-profile" and campaign(root)["campaign_id"].startswith("fixture-")
    original_expansion = name == "constructor-expansion-1" or known_profile
    if name not in {"constructor-expansion-1", "boundary-expansion-1", "confirmation-1"} and not known_profile:
    ...
    if not original_expansion and completion["packet_hash"] != packet["packet_hash"]:
