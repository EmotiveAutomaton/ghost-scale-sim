...
from ghostscale.validation.soundingline.v16.attack_access_stable import DESIGN, execute

REQUIRED = ["test_amended_access_frames_retain_actual_reader_identity_and_hide_evaluator",
            "test_amended_access_aliases_cache_and_private_read_are_actual",
            "test_amended_standalone_transfer_known_answer_and_private_read_denial"]
PACKET = "access-attack-fixture-2"
...
def sources(root):
    ...
    paths = {REPO/"runners/attack_v16_access_stable.py", PACKAGE/"consumer_frames.py", PACKAGE/"attack_access.py",
             PACKAGE/"reader_process.py", PACKAGE/"transfer.py", REPO/"runners/v16_reader_worker.py",
             REPO/"tests/test_v16_consumer_frames.py"}
    paths.update(PACKAGE/name for name in ["attack_access_stable.py", "active_consumers.py", "transfer_stable.py"])
    paths.add(REPO/"tests/test_v16_access_stable.py")
...
def main():
    ...
    admission_path = args.root/"access-stable-setup/ADMISSION.json"
