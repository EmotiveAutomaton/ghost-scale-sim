"""Fresh-clone validation for V14 (spec §20.2): clone HEAD, run the validators there, compare the
scientific fields, and write a receipt.

    python runners/fresh_clone_v14.py [--install]
...
WORK = V.V14_RESULTS / "fresh_clone_work"
RECEIPT = V.V14_RESULTS / "FRESH_CLONE_RECEIPT.json"
...
def main() -> int:
    ...
    if r.returncode != 0 or not (dest / "runners" / "validate_v14_program.py").exists():
    ...
    checks = {}
    for name, cmd in (("determinism", [py, "-m", "ghostscale.validation.soundingline.v14.determinism", "--order", "forward"]),
                      ("validator", [py, str(dest / "runners" / "validate_v14_program.py"), "--interim"]),
                      ("gates_test", [py, "-m", "pytest", "-q", str(dest / "tests" / "test_v14_gates.py")]),):
    ...
    local = sh([sys.executable, "-m", "ghostscale.validation.soundingline.v14.determinism", "--order", "forward"], cwd=REPO)
    clone_det = checks["determinism"]
    fields_match = None
    if clone_det["returncode"] == 0 and local.returncode == 0:
        rr = sh([py, "-m", "ghostscale.validation.soundingline.v14.determinism", "--order", "forward"], cwd=dest, env=env)
        try:
