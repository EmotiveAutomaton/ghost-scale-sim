"""Admit source-bound X04 controls and run the finite native consumer profile."""
import argparse
...
from ghostscale.validation.soundingline.v16.attack_fairness import DESIGN, execute
from ghostscale.validation.soundingline.v16.active_consumers import attack_consumers

PACKET = "fairness-attack-fixture-1"
REQUIRED = ["test_fairness_known_macro_budget_and_initial_learning_costs",
    "test_fairness_shared_observation_check_rejects_hidden_extra_context",
    "test_fairness_real_process_profile_and_meter_preserve_known_answer"]+[
    f"test_fairness_intentionally_broken_cost_and_access_controls[{kind}]" for kind in
    ["macro", "training", "definition", "extra-information"]]+[
    f"test_fairness_actual_reading_invocations_and_unchanged_results[{strategy}-{multiplier}]" for strategy, multiplier in
    [("maker", 4), ("direct-table", 8), ("generic", 4), ("primitive", 4)]]
...
def sources(root):
    ...
    files = {REPO/"runners/attack_v16_fairness.py", REPO/"tests/test_v16_fairness.py"}
    files.update(PACKAGE/name for name in ["attack_fairness.py", "fairness.py", "reading_cost_meter.py"])
...
def main():
    ...
    if args.resume and not (args.root/"packets"/f"{PACKET}.json").exists():
        raise ValueError("resume never prepares fairness controls")
    files = sources(args.root)
    admission_path = args.root/"fairness-setup/ADMISSION.json"
    if args.admit_from_junit:
        ...
        if not tests or any(value != "passed" for value in tests.values()) or any(name not in tests for name in REQUIRED):
            raise ValueError("fairness admission tests are missing or failed")
    ...
    if any(admission["tests"].get(name) != "passed" for name in REQUIRED):
        raise ValueError("fairness required executed admission missing")
    for path in files:
        if admission["source_hashes"].get(str(path.relative_to(REPO)).replace("\\", "/")) != file_digest(path):
            raise ValueError("fairness source differs from executed test receipt")
    with supervisor(args.root, "fairness-attack") as heartbeat:
        packet = freeze(args.root, PACKET, files, {**DESIGN, "consumer_cards": attack_consumers(args.root)["X04"]})
