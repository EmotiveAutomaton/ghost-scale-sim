"""Watchdog for the V13 run: keeps the multi-day queue healthy without touching it.
...
watchdog stops and leaves ``results/v13/WATCHDOG_HALTED.json`` as a loud marker. A stale
...
``idle``). It writes its own journal to ``results/v13/logs/watchdog.log``. This file is not
...
STATUS = REPO / "results" / "v13" / "RUNNER_STATUS.json"
COMPLETION = REPO / "results" / "v13" / "COMPLETION.json"
LOG = REPO / "results" / "v13" / "logs" / "watchdog.log"
HALT = REPO / "results" / "v13" / "WATCHDOG_HALTED.json"
...
def relaunch() -> int:
    out = (REPO / "results" / "v13" / "logs" / f"run_all_resume_{time.strftime('%m%d_%H%M%S')}.log").open("w", encoding="utf-8")
    p = subprocess.Popen([str(PY), str(REPO / "runners" / "run_v13.py"), "--stage", "all"],
