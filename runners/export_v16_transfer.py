...
from ghostscale.validation.soundingline.v16.transfer import DESIGN, MODULES, export, consume, evaluate

PACKET_ID = "transfer-fixture-1"
REQUIRED_TESTS = [
    "test_standalone_transfer_known_answer_and_private_read_denial",
    "test_standalone_transfer_learning_changes_executed_competence_and_noise_does_not",
    "test_stage9_transfer_has_complete_opaque_options_and_exact_evidence_shape",
    "test_transfer_evaluator_requires_committed_predictions_and_rejects_changed_results"]


def sources():
    return [PACKAGE/f"{name}.py" for name in [*MODULES, "transfer", "transfer_consumer"]] + [
        REPO/"runners/export_v16_transfer.py", REPO/"runners/replay_v16_readers.py",
        REPO/"tests/test_v16_transfer.py"]
...
def admit(root, junit):
    ...
    write(root/"transfer-setup/ADMISSION.json", record)
    return {"recorded_passed_tests": len(tests)}


def execute(root, heartbeat):
    admission = read(root/"transfer-setup/ADMISSION.json")
