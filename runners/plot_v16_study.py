...
def run(source, output):
    ...
    fig.subplots_adjust(bottom=.32, wspace=.28)
    for ax, cid, xkey, ykey, arms, title in [
        ...
        for color, arm in zip(COLORS, arms):
            ...
            ax.plot(xs, ys, "-", label=arm.replace("-", " "), color=color, linewidth=1.7)
            for x, y, marker, size in zip(xs, ys, ["s", "o", "^"], [5, 8, 5]):
        ...
        ax.set(title=title+f"\n{cid}: {card['n_per_condition']} makers per budget",
               xlabel="Mean search primitive evaluations", ylabel="Mean successful task fraction", ylim=(-.04,1.08))
        ax.grid(alpha=.18)
        ax.legend(fontsize=8)
    fig.legend(handles=[Line2D([], [], color="#374151", marker=marker, linestyle="none", label=label,
        markerfacecolor="none" if marker == "o" else "#374151", markersize=size)
        for marker, size, label in [("s", 5, "Budget 8"), ("o", 8, "Budget 32"), ("^", 5, "Budget 128")]],
        loc="lower center", bbox_to_anchor=(.5,.14), ncol=3, frameon=False, fontsize=9)
    save(fig, "construction-cost", "Each marker is one original budget regime; lines connect regimes for the same arm and are not pooled estimates. Coincident values are not displaced. Training has equal attempt counts; personal and pooled training contain different histories. Search primitives exclude training, definition costs and OS overhead, which remain separately recorded. These are descriptive arm means; paired uncertainty is in the complete comparison table.")
