"""A fresh process independently checks the final primary confirmation ledger."""
import argparse
...
from ghostscale.validation.soundingline.v16.confirmation_audit import run
...
def main():
    ...
    write(args.output/"RECEIPT.json", {"execution_state": "completed", **result, "recorded_at": now(),
        ...
        "sources": {path.relative_to(REPO).as_posix(): file_digest(path) for path in
            [Path(__file__).resolve(), REPO/"ghostscale/validation/soundingline/v16/confirmation_audit.py"]},
        "scope": "Confirmation primary statistics and multiplicity only; native physics, all other aggregates and closeout remain separate"})
    print({"execution_state": "completed", "primary_claims_independently_checked": result["primary_count"]})
