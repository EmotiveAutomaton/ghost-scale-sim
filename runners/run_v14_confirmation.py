"""V14 confirmation pass (spec wave 5): promoted cards re-run on the untouched confirmation
...
CONSTRUCTED_MECHANISM. Confirmation verdicts go to results/validation/soundingline/v14/confirmation/
and the ledger to results/v14/CONFIRMATION.json.
...
and the curator kept that plan (2026-08-28). The rule is therefore *recorded, never silent*:
  ...
  * an amendment preserves the original packet beside the replacement, with the reason, the
    time, and the cards added, in the ledger and in results/v14/AMENDMENTS.json;
...
``--no-amend`` refuses to widen instead, leaving the packet untouched.
    ...
    python runners/run_v14_confirmation.py --auto
    python runners/run_v14_confirmation.py --only C04 O02
    python runners/run_v14_confirmation.py --auto --no-amend
"""
...
from ghostscale.validation.soundingline.v14.runtime import init_worker      # noqa: E402
from ghostscale.validation.soundingline.v14.schemas import RESOLVED, card_from_dict  # noqa: E402

LEDGER = V.V14_RESULTS / "CONFIRMATION.json"
...
def promoted(doc: dict) -> list:
    """Every card eligible for confirmation: landed in discovery, every criterion held, claim
    ceiling METHOD or CONSTRUCTED_MECHANISM, not an infrastructure, attack or closure card."""
    out = []
    for d in doc["cards"]:
        if d["trunk"] in ("I", "B", "X") or d["status"] != "LANDED":
            continue
...
def select_candidates(doc: dict) -> dict:
    """The frozen set under spec section 8: at most CONFIRMATION_CAP cards, at most one per
    flight family. Within a flight the primary card is preferred when eligible, else the first
    eligible card in the flight's declared order; flights are taken in their declared order
    until the cap. The rule is recorded beside the choice."""
    from ghostscale.prereg_v14 import CONFIRMATION_CAP, FLIGHTS, FLIGHT_CARDS, FLIGHT_PRIMARY_CARD
    elig = promoted(doc)
    chosen, by_flight = [], {}
    for flight in FLIGHTS:
        if len(chosen) >= CONFIRMATION_CAP:
            break
        cards = FLIGHT_CARDS.get(flight, [])
        primary = FLIGHT_PRIMARY_CARD.get(flight)
        pick = primary if primary in elig else next((c for c in cards if c in elig), None)
        if pick is not None:
            chosen.append(pick)
            by_flight[flight] = pick
    return {"promoted": chosen, "eligible": elig, "by_flight": by_flight, "cap": CONFIRMATION_CAP,
            "rule": "one per flight, primary card preferred, flights in declared order, capped"}
...
def _lock_identity() -> dict:
    ...
    """
    from ghostscale.prereg_v14 import PREREG_PATH, STRUCTURAL_PATH
...
# The pass.
# --------------------------------------------------------------------------- #
def run(doc: dict, pool, only=None, amend: bool = False, elapsed: float = 0.0, window_closed=None,
        amend_reason: str = "recorded amendment", wait_for_freeze: bool = True) -> None:
    """The confirmation pass under the one-window contract: the packet freezes at hour
    FREEZE_HOUR (an early arrival waits for it, with a heartbeat), never widens afterwards
    unless explicitly amended, and no confirmation unit is submitted after the deadline."""
    from runners.run_v14 import record_runtime, run_card, tier_for, status, elapsed_hours
    from ghostscale.prereg_v14 import FREEZE_HOUR
    ...
    packet = ledger.get("frozen")
    if packet is None:
        while wait_for_freeze and elapsed_hours() < FREEZE_HOUR and not (window_closed and window_closed()):
            remaining = FREEZE_HOUR - elapsed_hours()
            status(stage="confirmation", waiting_for_freeze_hours=round(remaining, 3))
            print(f"confirmation: the candidate set freezes at hour {FREEZE_HOUR:.0f} ({remaining:.2f} h away); waiting", flush=True)
            time.sleep(min(600.0, max(30.0, remaining * 3600.0)))
        sel = select_candidates(doc)
        first = sel["promoted"]
        if only is not None:
            first = [c for c in first if c in only]
        if not first:
            raise FreezeViolation(f"nothing to freeze at hour {elapsed_hours():.2f}: eligible {sel['eligible']}, none selected")
        packet = make_packet(doc, first)
        packet.update({"selection": sel, "frozen_at_hour": round(elapsed_hours(), 3)})
        ledger["frozen"] = packet
        write_json_atomic(LEDGER, ledger)                    # frozen before the first confirmation world is touched
        try:
            write_json_atomic(M.CONFIRMATION_REGISTRY, {"frozen": packet, "when": packet["when"]})
        except Exception:                                    # noqa: BLE001
            pass
        print(f"confirmation packet frozen at hour {packet['frozen_at_hour']}: {len(first)} cards {first} ({sel['rule']})", flush=True)
    else:
    ...
    for cid in ids:
        ...
        if window_closed and window_closed():
            ledger["cards"][cid] = {"state": "WINDOW_CLOSED", "note": "the deadline passed before this card's confirmation opened", "packet_identity": packet_id}
            print(f"  [{cid}] not opened: the window is closed", flush=True)
            write_json_atomic(LEDGER, ledger)
            continue
        try:
            out = run_card(doc, card, "confirmation", tname, tier, pool)
...
def main() -> None:
    ...
    ap.add_argument("--amend", action="store_true",
                    help="permit a recorded widening of the frozen packet (never the default under the one-window contract)")
    ...
    args = ap.parse_args()
    if os.environ.get("GS_V14_SMOKE"):
        sys.exit("refusing to run the confirmation pass under GS_V14_SMOKE")
    ...
    with ProcessPoolExecutor(max_workers=workers, initializer=init_worker) as pool:
        try:
            run(doc, pool, only=args.only, amend=args.amend, amend_reason=args.amend_reason)
        except FreezeViolation as exc:
