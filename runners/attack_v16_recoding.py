"""Freeze and execute X02 against the actual V16 consumers."""
import argparse
...
from ghostscale.validation.soundingline.v16.attack_recoding import DESIGN, execute, RecodingFailure

REQUIRED = ["test_inquiry_recoding_retains_old_failure_and_validates_corrected_reader",
            "test_macro_interface_expands_boundaries_and_rejects_unknown_procedure",
            "test_recoded_actual_consumer_preserves_public_predictions",
            "test_physical_recoding_uses_actual_predictions_and_detects_tampering",
            "test_amended_standalone_retains_recoding_and_zero_value_repairs"]
PACKET = "recoding-attack-fixture-1"
...
def sources(root):
    ...
    paths = {REPO/"runners/attack_v16_recoding.py", PACKAGE/"consumer_frames.py", PACKAGE/"attack_access.py",
             PACKAGE/"reader_process.py", PACKAGE/"transfer.py", REPO/"runners/v16_reader_worker.py",
             REPO/"tests/test_v16_consumer_frames.py"}
    paths.update(PACKAGE/name for name in ["attack_recoding.py", "recoded_interface.py", "active_consumers.py", "recoding.py", "transfer_stable.py"])
    paths.update(REPO/path for path in ["tests/test_v16_recoded_interface.py", "tests/test_v16_recoding.py", "tests/test_v16_transfer_stable.py"])
...
def main():
    ...
    admission_path = args.root/"recoding-attack-setup/ADMISSION.json"
    ...
    with supervisor(args.root, "consumer-recoding-attack") as heartbeat:
        ...
        if (output/"COMPLETION.json").exists():
            ...
            except RecodingFailure as error:
                write(output/"private/PHYSICAL_FAILURE_points.json", {"message": str(error), "evidence": error.evidence})
                raise
    print(report)
