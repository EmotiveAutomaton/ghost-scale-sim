...
This is the canonical operating contract for Ghost Scale Simulation. Codex/GPT is
the current working agent. CLAUDE.md remains a compatibility entry point. Historical
authorship and contribution records retain their original attribution.

## Start here, including after a restart or compaction
...
- A green runtime receipt alone does not establish continuous occupancy. Consult
  `docs/versions/v15-boundary-map/HEALING_PLAN.md` for the confirmation and accounting debt.
- The existing fresh-clone checker compares structural hashes and a coverage prefix;
  it does not yet establish full scientific aggregate regeneration.
- Do work inline. At most one search/research subagent, only when the inherited rule
  and the session's higher-priority instructions permit it. No delegated repository audits.
- Announce changes to AGENTS.md or CLAUDE.md in the reply that makes them.
- Force pushes require explicit user approval. Permission for an ordinary push
  does not authorize a force push.

The research, collaboration, reporting, and provenance instructions below were
carried over from the pre-migration CLAUDE.md. Version-specific historical notes are
retained for their failure history, subject to the current qualifications above.

## Inherited working notes
