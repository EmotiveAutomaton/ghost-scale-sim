"""Atomic JSON publication for V15 working state.
...
provenance only. In V15 ``common.py``'s ``write_verdict``, ``record_completion`` and ``save_ckpt`` all route
through this helper from the start: V14 shipped them non-atomic and could not fix it inside the
lock.
