"""Card implementations for V13, one module per trunk (spec §5).
...
The runner maps ``unit_`` over the lane's worlds and repeats through a process pool with
hash-aware checkpoints, then calls ``reduce_``. ``ctx`` carries the lane, world id, repeat, tier
configuration, the card, the program configuration (fresh families for the transfer lane, cost
ecologies, expansions), the smoke flag, whether an upstream instrument failed (oracle input),
and, for attacks, the attack specification.

Rows a card emits carry the card's declared factor keys (manifest ``factors``) plus ``wid`` and
``rep``; the expected-cell receipt counts the realized crossings per independent unit. A card
that omits a factor fails its receipt, which is the anti-shrink assertion of spec §7.5.
"""
...
from ..world import DISCOVERY_STRUCTURES, FRESH_STRUCTURES, World, WorldParams, make_world, random_params

SMOKE = bool(os.environ.get("GS_V13_SMOKE"))
_WORLD_CACHE = {}


def smoke_mode() -> bool:
    return bool(os.environ.get("GS_V13_SMOKE"))
...
def world_params_for(lane: str, wid: int, cfg: dict | None = None) -> WorldParams:
    cfg = cfg or {}
    rng = np.random.default_rng(C.world_seed(lane, wid))
    p = random_params(rng)
    if lane in ("transfer", "attack"):
        # fresh common-substrate families lead the transfer lineage; fresh domains as well
        p.structures = tuple(FRESH_STRUCTURES) + tuple(DISCOVERY_STRUCTURES[2:])
    else:
        p.structures = tuple(DISCOVERY_STRUCTURES)
    p.n_families = 4 + int(cfg.get("extra_families", 0))
    if p.n_families > 4:
        p.structures = tuple(DISCOVERY_STRUCTURES) + tuple(FRESH_STRUCTURES)
    p.rare_makers = bool(cfg.get("rare_makers", False))
    return p
...
def world_for(ctx: dict) -> World:
    ...
    if key not in _WORLD_CACHE:
        ...
        _WORLD_CACHE[key] = make_world(wid, lane, params=p, rng=rng, extra_domains=int(cfg.get("extra_domains", 0)))
...
def sizes(ctx: dict) -> dict:
    ...
    makers = int(t["makers"])
    out = {"makers": makers, "readers": max(12, makers // 4), "teams": int(t["teams"]), "events": int(t["events"]),
           "repeats": int(t["repeats"]), "sources": max(8, makers // 4), "histories": max(6, makers // 8)}
    if smoke_mode() or ctx.get("smoke"):
        out.update({"makers": 24, "readers": 12, "teams": 8, "events": 24, "sources": 8, "histories": 4})
    return out


def cost_ecologies(ctx: dict) -> list:
    base = ["craft", "bureaucratic", "hazardous", "collegial"]
    if ctx["lane"] in ("transfer", "attack"):
        return ["frontier", "scarce", "hazardous", "collegial"]
    extra = int((ctx.get("cfg") or {}).get("extra_cost_ecologies", 0))
    return base + ["frontier", "scarce"][:extra]
...
def ci_abs(rows: list, value: str, where=None, seed_tag: str = "") -> float:
    ...
    a quiet null is quiet even when one smoke unit is noisy."""
...
def ci_pos(rows: list, value: str, where=None, seed_tag: str = "") -> float:
    """Demonstrated POSITIVE mean for a one-sided null gate: the lower bootstrap bound, clipped at
    zero. A focus that hurts where there is nothing to learn is not a gain."""
...
def sim_bin(distance: float, edges: tuple, anti: bool = False) -> str:
    if anti:
        return "anti"
    if distance < edges[0]:
        return "near"
...
def edges_for(distances) -> tuple:
    d = np.asarray(distances, float)
    if d.size < 3:
        return (0.05, 0.15)
    return (float(np.quantile(d, 1 / 3)), float(np.quantile(d, 2 / 3)))


def held_out_classifier(X, labels, rng, metric="js") -> float:
    """Held-out-half centroid classifier accuracy (never leave-one-out: LOO centroids sit below
    ...
    if len(classes) == 2 and sorted(map(tuple, X[L == classes[0]])) == sorted(map(tuple, X[L == classes[1]])):
        return 0.5                                          # identical evidence for both classes: chance is the honest answer, not a tie-break artifact
...
# --------------------------------------------------------------------------- #
# The seven-gate battery (spec §5.2) from numbers a card supplies.
# --------------------------------------------------------------------------- #
def battery(gr: G.GateReport, live=None, placebo=None, positive=None, surface=None, oracle=None,
    ...
    if surface is not None:
        gr.positive("surface:" + surface.get("name", "cheap_cues_at_chance"), observed=float(surface["accuracy"]),
                    ...
                    detail=surface.get("detail", "cheap source, length, count, label or entropy cues cannot solve the card"))
    else:
    ...
    if prediction is not None:
        gr.live("prediction:" + prediction.get("name", "inferred_object_constrains_held_out"), observed_change=float(prediction["gain"]),
                min_change=float(prediction["min"]), detail=prediction.get("detail", "the inferred object constrains a held-out choice, continuation, probe or response"))
    else:
...
def finish(card, v: dict, gr: G.GateReport, module_file: str, state: str, ctx: dict, closure_reason: str = "",
    ...
    if v.get("expected_cell_receipt") and not v["expected_cell_receipt"].get("ok", True) and state == "LANDED" and not ctx.get("smoke"):
