"""Shared machinery for V14 cards: seeds and lineages, proper scores, calibration, bootstrap,
...
from . import REPO, SEED_OFFSET_V14, v14_dir, verdict_dir
...
def seed(name: str) -> int:
    return SEED_OFFSET_V14 + (zlib.crc32(("v14|" + name).encode("utf-8")) % 1_000_000)
...
def ckpt_path(lane: str, card: str, wid: int, rep: int) -> Path:
    d = v14_dir("checkpoints") / lane / card
...
# --------------------------------------------------------------------------- #
COMPLETION = v14_dir() / "COMPLETION.json"
...
def record_completion(card_id: str, lane: str, out: Path, verdict: dict) -> None:
    doc = json.loads(COMPLETION.read_text(encoding="utf-8")) if COMPLETION.exists() else {"program": "v14", "entries": {}}
    try:
