"""V14 — Common Ground. Nested maker–reader priors, attention allocation, opportunity and cost,
...
Spec: V14_SPEC.md at the repository root (immutable handoff; filed beside this version's pages
when the version closes) and docs/versions/v14-common-ground/SPEC.md (implementation
...
Verdict layout (results/validation/soundingline/v14/):
...
results/v14/pilot_quarantine/ (ignored by git) and smoke passes go to a scratch directory.
"""
...
PILOT_QUARANTINE = V14_RESULTS / "pilot_quarantine" / "verdicts"
SMOKE_DIR = V14_RESULTS / "smoke" / "verdicts"


def v14_dir(sub: str | None = None) -> Path:
    d = V14_RESULTS if sub is None else V14_RESULTS / sub
...
def verdict_dir(lane: str = "discovery") -> Path:
    if lane == "pilot":
        ...
        d = V14_VERDICTS / sub if sub else V14_VERDICTS
    d.mkdir(parents=True, exist_ok=True)
    return d
