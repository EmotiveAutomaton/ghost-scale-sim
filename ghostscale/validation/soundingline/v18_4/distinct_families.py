"""Distinct predictive laws and equivalence-class prior sensitivity.
...
def combine(predictions,scores,priors,select=False):
    weighted=np.asarray(scores)+np.log(priors)
    return predictions[int(np.argmax(weighted))] if select else mixture(weighted)@predictions


def catalog(w):
    return [dict(w),dict(w,rule='satisficing' if w['rule']=='softmax' else 'softmax'),
...
def make_case(index,cell,kind,order,copy_span,length=32):
    ...
    w=W.make_world(cell,18048000+index);truth=dict(w)
    if kind=='initial-alternative':truth=catalog(w)[1]
    elif kind=='new-rule':truth=catalog(w)[2]
    elif kind=='outside-menu':truth=dict(w,rule='softmax',temperature=w['temperature']*.25)
    # State/draw positions shared across orders and copy counts, independent of kind.
    state=int(W.rng('v18.4-distinct-family-state',index,cell).integers(len(W.STATES)))
    base=[];half=length//2
    for j in range(length):
        c=W.QUERIES[j%5] if j<half else W.QUERIES[5+(j-half)%3]
        obs=W.observe(truth,state,c,W.rng('v18.4-distinct-family',index,cell,j),f'root-{j}')
        base.append(obs)
...
def read_stream(public,method,allow_expansion=True,force_at=None):
    ...
    prior_mode=public['prior_mode']
    if prior_mode not in ('equal-class','duplicate-supplied'):raise ValueError('undeclared class prior')
    priors=np.array([2.,1.,1.]) if prior_mode=='duplicate-supplied' else np.ones(3)
    ...
    for obs in public['history']:
        ...
        p=combine(predictions,scores,priors[:count],select)
        trace.append(dict(step=t,source=obs['source'],query=q,prediction=p.tolist(),
                          expanded=count==3,model_weights=mixture(scores+np.log(priors[:count])).tolist()))
        ...
        if paid:
            ...
            surprises.append(-math.log(float(mixture(scores[:2]+np.log(priors[:2]))@evidence)))
    ...
    return dict(method=method,trace=trace,final=final,expanded=float(count==3),
                ...
                class_priors=priors[:count].tolist(),model_entropy=float(np.mean([W.entropy(x['model_weights']) for x in trace])))
...
def unit(index,cell=0,kind='new-rule',order='old-first',copy_span=1,length=32,prior_mode='equal-class',split='test'):
    public,truth=make_case(index,cell,kind,order,copy_span,length)
    public['prior_mode']=prior_mode
    rows=[score(read_stream(public,m),truth) for m in METHODS]
    return dict(family='R2',index=index,cell=cell,kind=kind,order=order,copy_span=copy_span,length=length,prior_mode=prior_mode,
                public=public,evaluator=truth,rows=rows,
                scope='distinct initial decision laws; explicit predictive-class prior sensitivity; catalog access, not law invention')
...
def verify(unit):
    ...
    for row in unit['rows']:
        ...
        entropies=[-math.fsum(v*math.log(v) for v in x['model_weights'] if v>0) for x in row['trace']]
        if abs(math.fsum(entropies)/len(entropies)-row['model_entropy'])>1e-10:raise ValueError('model entropy differs')
        ...
        prior=np.array([2.,1.,1.]) if unit['prior_mode']=='duplicate-supplied' else np.ones(3)
        for t,item in enumerate(row['trace']+row['final']):
            observed=min(t,unit['length'])
            count=1 if row['method']=='fixed' else 2+int(item.get('expanded',row['expanded']))
            joint=[]
            for k in range(count):
                joint.append([math.log(prior[k]/n)+math.fsum(math.log(float(v)) for v in likelihood_bank[k][:observed,s]) for s in range(n)])
            top=max(v for a in joint for v in a)
            mass=np.exp(np.asarray(joint)-top)
            if row['method']=='fixed' or 'select' in row['method']:
                chosen=int(np.argmax(mass.sum(axis=1)))
                mass=np.array([a if k==chosen else np.zeros(n) for k,a in enumerate(mass)])
            mass/=mass.sum()
            prediction=np.sum([mass[k]@W.artifact_matrix(catalog(public['world'])[k],item['query']) for k in range(count)],axis=0)
            if np.max(abs(prediction-item['prediction']))>1e-10:raise ValueError('scalar prefix forecast differs')
