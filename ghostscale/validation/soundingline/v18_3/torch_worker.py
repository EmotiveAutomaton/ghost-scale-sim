...
from .io import replace,write as durable_write
...
def fit(root,identity,train,dev,config,pulse,limited):
    ...
    n=len(train['sample'])
    def save():
        name=f"step-{progress['steps']:07d}.pt";path=root/name
        if path.exists():name=f"step-{progress['steps']:07d}-{time.time_ns()}.pt";path=root/name
