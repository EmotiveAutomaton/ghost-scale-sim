...
import uuid
import numpy as np
from .io import read,write,file_digest,canonical,now
from ..v16.runtime import local_owner
from .native import ProcessClock
...
def score_E(root,pulse=lambda **kw:None):
    ...
    for condition,entry in evaluator['truth'].items():
        ...
        queries=D.NEW_QUERIES if condition in ('new-queries','both-new') else D.TRAIN_QUERIES
    ...
    result=dict(family='E',cells=summary,paired=paired,raw_sha256=file_digest(path),
