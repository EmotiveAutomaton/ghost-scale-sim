        ...
        try:
            ...
            for start in range(0,population,plan['block_size']):
                ...
                for case in selected_cases:
                    if original:
                    ...
                    elif plan['branch']=='g2-cyclic-action':
                        from . import cyclic_union
                        rows=cyclic_union.evaluate_target_action(
                            case,plan['design']['budget'],plan['design']['query_counts']);diagnostics=None
                    elif plan['branch']=='g2-cyclic-physical':
                        from . import cyclic_union
                        ...
                        rows=cyclic_union.evaluate_misspecified(
                            case,plan['design']['budget'],plan['design']['query_counts']);diagnostics=None
                    elif plan['branch']=='g2-cyclic-misspecified-physical':
                        from . import cyclic_union
                        rows=cyclic_union.evaluate_physical_misspecified(
                            case,plan['design']['budget'],plan['design']['query_counts']);diagnostics=None
                    else:
