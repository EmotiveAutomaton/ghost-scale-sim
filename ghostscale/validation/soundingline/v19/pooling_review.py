"""Independent input-pooling audit; no pooling or feedback producer imported.
...
def revised(counts, observed, arm):
    ...
    for key in inputs('forward'):
        ...
        for target in range(8):
            result[key][target]=1+math.fsum(float(counts[src][target]-1) for src in members)+sum(unique.get(src)==target for src in members)
...
def regroup(cells, lineages, draws, cfg):
    ...
    for key in keys:
        ...
        if key[5]!='unpooled': comparisons.append(('pool-minus-unpooled',key[:5]+('unpooled',)+key[6:]))
...
def review(original,output,cfg,pulse=lambda **kw:None):
    ...
    for draw,mode in product(design['training_draws'],('original','composition-holdout')):
        pulse(phase='independent-pooling',draw=draw,mode=mode)
        ...
        for rule,order,feedback,budget in product(T.RULES,design['orders'],design['feedback'],design['budgets']):
            ...
            for arm in ARMS:
                ...
                if budget==0 and arm=='unpooled': error=max(error,near(predicted,baseline))
    ...
    result=dict(passed=True,cells=len(cells),forecast_vectors=vectors,score_rows=summary['score_rows'],native_paths=native_paths,baseline_vectors=baseline_vectors,max_error=error,reader_packets=len(reader_ids),
        scope='independent supplied pooling maps,scalar empirical/prior counts,feedback,updated tables,path-sum forecasts,proper scores,population/support strata and paired conditional intervals; parent training/path probabilities inherit bound independent validation')
