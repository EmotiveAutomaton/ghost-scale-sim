"""Time-localized forecast errors for fixed recent and spaced source retention.
...
def span_slices(times):
    if times < 1: raise ValueError('future times')
    split = 1 + times//2
    return (slice(0,1), slice(1,split), slice(split,times))
...
def evaluate(weight, st, law, source_rows, operator=None):
    ...
    slices=span_slices(delta.shape[-3])
    span_lengths=np.array([len(range(delta.shape[-3])[part]) for part in slices],dtype=np.int32)
    span_square=np.full((3,*squared.shape),np.nan);span_max=np.full_like(span_square,np.nan)
    for si,part in enumerate(slices):
        if span_lengths[si]:
            values=delta[...,part,:,:]
            span_square[si]=np.where(both,(values*values).sum(-1).mean(axis=(-2,-1)),np.nan)
            span_max[si]=np.where(both,abs(values).max(axis=(-3,-2,-1)),np.nan)
    valid=np.where(both,squared,0.)
    recombined=np.sum(np.nan_to_num(span_square)*span_lengths[:,None,None,None],axis=0)/span_lengths.sum()
    if np.max(abs(recombined-valid))>TOL:raise ValueError('span recombination')
    ...
    return dict(forecast_span_lengths=span_lengths,span_squared_regret=span_square,span_max_future_probability_error=span_max,source_rows=ids.astype(np.int32), retained=keep, retained_sources=keep.sum(-1),
...
def summarize(raw):
    ...
    for si,span in enumerate(SPANS):
        out[span+'_steps']=np.full(5,raw['forecast_span_lengths'][si])
        for wi,label in enumerate(WINDOWS):
            for suffix,field in [('expected_squared_regret','span_squared_regret'),('expected_max_probability_error','span_max_future_probability_error')]:
                out[span+'_'+label+'_'+suffix]=((truth*np.nan_to_num(raw[field][si,wi])).sum(-1) if raw['forecast_span_lengths'][si] else np.full(5,np.nan))
    return out
...
def controls():
    ...
    return {'live:older_informative_time':bool(live['spaced_half_expected_squared_regret'][0]<live['recent_half_expected_squared_regret'][0]-.001),
            ...
            'placebo:certain_copy_identity':bool(all(abs(live[label+'_expected_squared_regret'][-1])<TOL for label in WINDOWS)),
...
def run(root,plan,pulse):
    ...
    if not all(checks.values()) or cfg['alphas']!=list(ALPHAS) or cfg['selection_rule']!='midpoint-sorted-time-ranks' or cfg['forecast_spans']!=list(SPANS):raise ValueError('controls/design')
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='retention-horizon',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
                    ...
                    for ai,alpha in enumerate(ALPHAS):rows.append(dict(lineage=lineage,evidence=evidence,length=length,checkpoint=cp,alpha=alpha,**{k:r[k] for k in ('draw','initial_maker','kind','switched','duplicates','stream','report_sources')},**{k:(None if np.isnan(v[ai]) else float(v[ai])) for k,v in metrics.items()}))
    ...
    (root/'raw/retention_horizon_summary_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    write(root/'EVIDENCE_ROLES.json',dict(reader='no new reader input',evaluator='supplied equal source prior,laws,complete posteriors and source identities;matched-count recency and evenly spaced sorted-time retention; explicit prior remainder;all future coordinates;undefined errors retain NaN',scope='current,near and far forecast errors under fixed source retention;empty spans unavailable;no learned provenance or historical correspondence'))
    return dict(controls=checks,posterior_rows=len(rows)//len(ALPHAS),rows=len(rows),sources=source_total,report_queries=len(rows)*8,unavailable_checkpoints=unavailable,numerical_acceptance=False,scope='full,recent half/quarter,spaced half/quarter;all future coordinates split into current,near,far;empty spans unavailable;8endpoint reports;equal source prior;no new observations,models or protected lineages')
