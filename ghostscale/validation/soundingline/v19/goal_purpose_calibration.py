"""Public-request grouping of complete goal calibration sufficient sums."""
...
def purpose_groups(packets):
    labels=[]
    for packet in packets:
        D.S.validate(packet)
        if packet['tier']!='E2-full':raise ValueError('tier')
        purpose=packet['inputs']['requested_purpose']
        if type(purpose) is not int or purpose not in PURPOSES:raise ValueError('requested purpose')
...
def sufficient(p,c,w,purposes):
    """Axes: law, position, goal, public purpose, probability bin."""
    p,c,w=(np.asarray(x,float) for x in (p,c,w));op=np.asarray(purposes)
    if p.ndim!=3 or p.shape[1:]!=(3,3) or c.ndim!=4 or c.shape[1:]!=p.shape or w.shape!=c.shape[:2]:raise ValueError('shape')
    if op.shape!=p.shape[:2] or not np.issubdtype(op.dtype,np.integer) or np.any(op<0) or np.any(op>=2):raise ValueError('purpose')
    ...
    for t,g in product(range(3),range(3)):
        ...
        for law in range(len(c)):
            bw[law,t,g]=np.bincount(code,weights=w[law],minlength=20).reshape(2,10)
            bp[law,t,g]=np.bincount(code,weights=w[law]*p[:,t,g],minlength=20).reshape(2,10)
            bc[law,t,g]=np.bincount(code,weights=w[law]*c[law,:,t,g],minlength=20).reshape(2,10)
    delta=bp-bc
    ...
    return dict(bin_weight=bw,bin_forecast_sum=bp,bin_correct_sum=bc,
        purpose_mass=mass,conditional_signed_error=conditional_signed,
...
def controls():
    ...
    x=sufficient(p,c,w,op);own=sufficient(p,p[None],w,op);empty=sufficient(p,c,w,np.zeros_like(op))
    return {'live:purpose_cancellation':bool(np.max(x['pooled_bin_error'])<1e-14 and np.min(x['cancellation_gap'][0,:,:2])>.099),
        'placebo:native_self':bool(np.max(own['grouped_bin_error'])==0),
        'positive:mass':bool(np.max(abs(x['bin_weight'].sum((3,4))-1))<1e-14),
        'positive:empty_groups':bool(np.isnan(empty['conditional_signed_error'][...,1:]).all())}
...
def run(root,plan,pulse):
    ...
    if cfg['tiers']!=['E2-full'] or cfg['bin_edges']!=C.EDGES.tolist() or cfg['purposes']!=list(PURPOSES):raise ValueError('design')
    ...
    purposes=purpose_groups([packets['packets'][k] for k in keys])
    ...
    write(root/'ARRAY_SCHEMA.json',dict(axes=['law','weighting','position','goal','purpose','bin'],law=ls,weighting=list(WEIGHTS),purpose=cfg['purposes'],bin_edges=cfg['bin_edges'],empty_means='NaN iff corresponding weight is exactly zero',sums='joint population mass; group conditioning occurs only by division after aggregation'))
    (root/'groups').mkdir();rows=[];timing=[];parent_error=0.;pool_error=0.;frames=0;parents=0
    weights=(mass,np.full_like(mass,1/len(keys)))
    native_groups=[sufficient(native[li],native[li:li+1],w[li:li+1],purposes) for li in range(len(ls)) for w in weights]
    ...
    for draw,seed,budget,arm,forecast in product(cfg['training_draws'],cfg['fit_seeds'],cfg['budgets'],ARMS,FORECASTS):
        pulse(phase='purpose-conditioned-calibration',draw=draw,seed=seed,budget=budget,arm=arm,forecast=forecast);tick=time.process_time()
        ...
        values=[sufficient(marg,native,w,purposes) for w in weights]
        ...
        for li,l in enumerate(ls):
            ...
            for wi,weighting in enumerate(WEIGHTS):
                val=values[wi]
                for t,g in product(range(3),range(3)):
                    # Group sums marginalized over purpose reproduce the
    ...
    (root/'goal_purpose_calibration_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    ...
    return dict(controls=checks,rows=len(rows),frame_forecasts=frames,packets=len(keys),parent_cells=parents,parent_max_error=parent_error,max_pooled_error=pool_error,**aggregate(rows,cfg),fits=0,scope='public purpose-conditioned calibration; joint group/bin sufficient sums; no historical correspondence')
