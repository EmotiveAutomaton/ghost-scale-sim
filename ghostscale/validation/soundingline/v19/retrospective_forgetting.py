"""Fixed recent-source windows with an explicit, mass-preserving remainder.
...
def evaluate(weight, st, law, source_rows, operator=None):
    ...
    keep = np.stack([np.ones(count, bool), t > cp//2, t > (3*cp)//4, np.zeros(count, bool)])
    ...
    increments = (direction.reshape(-1, len(w)) @ op).reshape(4, 8, st['future'].shape[1], 16)
    future_direction = (np.cumsum(increments, axis=-2) @ law.reshape(16, 32)).reshape(4, 8, -1, 4, 8)
    ...
    return dict(source_rows=ids.astype(np.int32), retained=keep, retained_sources=keep.sum(-1),
                ...
                full_hypothesis_float64_count=np.full(4, len(w), dtype=np.int64),
...
def controls():
    ...
    flat=summarize(evaluate(w,st,law,[[1,0,0],[4,1,0]]))
    law[:8,0]=np.eye(8)[0];law[8:,0]=np.eye(8)[1]
    live=summarize(evaluate(w,st,law,[[1,0,0],[4,1,0]]))
...
def run(root,plan,pulse):
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='retrospective-forgetting',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
    ...
    (root/'raw/retrospective_forgetting_summary_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    write(root/'EVIDENCE_ROLES.json',dict(reader='no new reader input',evaluator='supplied equal source prior,laws,complete posteriors and source identities;fixed recent source windows and explicit prior remainder;all future coordinates;undefined errors retain NaN',scope='bounded retrospective forgetting;no learned provenance or historical correspondence'))
    return dict(controls=checks,posterior_rows=len(rows)//len(ALPHAS),rows=len(rows),sources=source_total,report_queries=len(rows)*8,unavailable_checkpoints=unavailable,numerical_acceptance=False,scope='full,half,quarter,no retention;8endpoint reports;equal source prior;no new observations,models or protected lineages')
