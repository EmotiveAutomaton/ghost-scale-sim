"""Public-operation grouping of complete goal calibration sufficient sums."""
...
def sufficient(p,c,w,operations):
    """Axes: law, position, goal, public operation, probability bin."""
    p,c,w=(np.asarray(x,float) for x in (p,c,w));op=np.asarray(operations)
    if p.ndim!=3 or p.shape[1:]!=(3,3) or c.ndim!=4 or c.shape[1:]!=p.shape or w.shape!=c.shape[:2]:raise ValueError('shape')
    if op.shape!=p.shape[:2] or not np.issubdtype(op.dtype,np.integer) or np.any(op<0) or np.any(op>=6):raise ValueError('operation')
    ...
    for t,g in product(range(3),range(3)):
        ...
        for law in range(len(c)):
            bw[law,t,g]=np.bincount(code,weights=w[law],minlength=60).reshape(6,10)
            bp[law,t,g]=np.bincount(code,weights=w[law]*p[:,t,g],minlength=60).reshape(6,10)
            bc[law,t,g]=np.bincount(code,weights=w[law]*c[law,:,t,g],minlength=60).reshape(6,10)
    delta=bp-bc
    ...
    return dict(bin_weight=bw,bin_forecast_sum=bp,bin_correct_sum=bc,
        operation_mass=mass,conditional_signed_error=conditional_signed,
...
def controls():
    ...
    return {'live:operation_cancellation':bool(np.max(x['pooled_bin_error'])<1e-14 and np.min(x['cancellation_gap'][0,:,:2])>.099),
...
def run(root,plan,pulse):
    ...
    if cfg['tiers']!=['E2-full'] or cfg['bin_edges']!=C.EDGES.tolist() or cfg['operations']!=list(D.S.L.OPERATIONS):raise ValueError('design')
    ...
    for draw,seed,budget,arm,forecast in product(cfg['training_draws'],cfg['fit_seeds'],cfg['budgets'],ARMS,FORECASTS):
        pulse(phase='operation-conditioned-calibration',draw=draw,seed=seed,budget=budget,arm=arm,forecast=forecast);tick=time.process_time()
        ...
        values=[sufficient(marg,native,w,ops) for w in weights]
        ...
        for li,l in enumerate(ls):
            ...
            for wi,weighting in enumerate(WEIGHTS):
                val=values[wi]
                for t,g in product(range(3),range(3)):
                    # Group sums marginalized over operation reproduce the
    ...
    (root/'goal_operation_calibration_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    ...
    return dict(controls=checks,rows=len(rows),frame_forecasts=frames,packets=len(keys),parent_cells=parents,parent_max_error=parent_error,max_pooled_error=pool_error,**aggregate(rows,cfg),fits=0,scope='public operation-conditioned calibration; joint group/bin sufficient sums; no historical correspondence')
