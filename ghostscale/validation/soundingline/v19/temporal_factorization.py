"""Frozen joint forecasts versus products of within-step goal/operation marginals.
...
def factorize(q):
    ...
    # Each selected pair axis has 18 symbols; sum all remaining four axes.
    cube = q.reshape(-1, 3, 3, 3, 6, 6, 6)
    steps = np.stack([cube.sum(axis=tuple(a for a in range(1,7) if a not in (1+t,4+t))).reshape(-1,18)
                      for t in range(3)], axis=1)
    independent = np.ones_like(q)
    for t in range(3): independent *= steps[:,t,PAIRS[:,t]]
    if not np.allclose(independent.sum(1),1,rtol=0,atol=3e-12): raise ValueError('product normalization')
    return independent, steps


def controls():
    q=np.zeros((1,N));q[0,[0,2634]]=.5  # pairs (0,0,0) versus (7,7,0)
    p,_=factorize(q)
    expected=np.zeros((1,N));expected[0,[0,1980,654,2634]]=.25
    return {'live:temporal_dependence_removed':bool(np.array_equal(p,expected)),
            'placebo:temporal_independence_identity':bool(np.array_equal(factorize(expected)[0],expected)),
            'positive:within_step_dependence_retained':bool(np.count_nonzero(p)==4),
            'positive:incompatible_cross_paths_created':bool(p[0,1980]==.25 and p[0,654]==.25)}
...
def run(root, plan, pulse):
    ...
    for tier in cfg['tiers']:
        ...
        for li, lineage in enumerate(cfg['development_lineages']):
            pulse(phase='native-temporal-factorization', tier=tier, lineage=lineage)
            ...
            fact, steps = factorize(q)
            np.savez_compressed(root/'evaluator'/f'{tier}-{lineage}-marginals.npz', steps=steps)
        ...
        for draw, seed, budget, arm in product(cfg['training_draws'], cfg['fit_seeds'], cfg['budgets'], ARMS):
            pulse(phase='learned-temporal-factorization', tier=tier, draw=draw, seed=seed, budget=budget, arm=arm)
            ...
            fact, steps = factorize(q)
            np.savez_compressed(root/'marginals'/(stem+'.npz'), steps=steps)
    ...
    (root/'temporal_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    ...
    return dict(controls=checks,rows=len(rows),original_cells_reproduced=reproduced,original_max_error=max_error,
                ...
                fits=0,scope='constructed-method learned temporal dependence; native posterior products are separate evaluator rulers; no support restriction or historical correspondence')
