"""Time-balanced versus recent-source retention at matched source counts.
...
def run(root,plan,pulse):
    ...
    if not all(checks.values()) or cfg['alphas']!=list(ALPHAS) or cfg['selection_rule']!='midpoint-sorted-time-ranks':raise ValueError('controls/design')
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='time-retention',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
    ...
    (root/'raw/time_retention_summary_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
