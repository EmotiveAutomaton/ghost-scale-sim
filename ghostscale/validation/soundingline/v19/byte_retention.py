"""Source retention at matched structural byte budgets.
...
def retention(ids, checkpoint, st):
    if '_byte_costs' not in st:
        costs=[]
        for past in st['past']:
            pairs=np.unique(st['mapping']*16+past)
            counts=np.bincount(pairs//16,minlength=len(st['signatures']))
            costs.append(int(8*counts[counts>1].sum()+4*(3+2*len(pairs))))
        st['_byte_costs']=np.asarray(costs,dtype=np.int64)
    costs=st['_byte_costs'][ids[:,0]-1]
    overhead=8*len(st['signatures'])+4*12+8*32
    recent=[ids[:,0]>checkpoint//2,ids[:,0]>3*checkpoint//4]
    spaced=[spaced_mask(ids,int(k.sum())) for k in recent]
    budgets=[overhead+min(int(costs[r].sum()),int(costs[b].sum())) for r,b in zip(recent,spaced)]
    descending=np.argsort(-ids[:,0],kind='stable').tolist()
    masks=[np.ones(len(ids),bool)]
    for order in ('recent','spaced'):
        for i,budget in enumerate(budgets):
            chosen=sorted(np.flatnonzero(spaced[i]).tolist(),key=lambda j:int(ids[j,0]))
            priority=descending if order=='recent' else chosen+[j for j in descending if j not in chosen]
            mask=np.zeros(len(ids),bool);remaining=budget-overhead
            if remaining<0:raise ValueError('negative usable budget')
            for j in priority:
                if costs[j]<=remaining:mask[j]=True;remaining-=int(costs[j])
            masks.append(mask)
    return np.stack(masks),np.array([overhead+costs.sum(),*budgets,*budgets],dtype=np.int64),costs
...
def evaluate(weight, st, law, source_rows, operator=None):
    ...
    keep, budgets, source_costs = retention(ids, cp, st)
    ...
    used=8*floats+4*integer_counts+8*32
    if np.any(used>budgets):raise ValueError('storage budget exceeded')
    return dict(storage_budget_bytes=budgets,storage_used_bytes=used,unused_budget_bytes=budgets-used,source_storage_bytes=source_costs,source_rows=ids.astype(np.int32), retained=keep, retained_sources=keep.sum(-1),
...
def summarize(raw):
    ...
    if np.any(raw['storage_used_bytes']>raw['storage_budget_bytes']):raise ValueError('storage budget exceeded')
    if not np.array_equal(raw['storage_used_bytes'],8*raw['compact_float64_count']+4*raw['compact_int32_count']+256):raise ValueError('storage byte identity')
    if not np.array_equal(raw['unused_budget_bytes'],raw['storage_budget_bytes']-raw['storage_used_bytes']):raise ValueError('unused byte identity')
    if not np.array_equal(raw['storage_budget_bytes'][1:3],raw['storage_budget_bytes'][3:5]):raise ValueError('paired budgets')
    for wi,label in enumerate(WINDOWS):
        for key in ('storage_budget_bytes','storage_used_bytes','unused_budget_bytes'):
            out[label+'_'+key]=np.full(5,raw[key][wi])
    return out
...
def controls():
    ...
    return {'live:older_informative_time':bool(live['spaced_half_expected_squared_regret'][0]<live['recent_half_expected_squared_regret'][0]-.001),
            ...
            'positive:matched_byte_budgets':bool(np.array_equal(raw['storage_budget_bytes'][1:3],raw['storage_budget_bytes'][3:5]))}


def run(root,plan,pulse):
    cfg=plan['design'];checks=controls();write(root/'CONTROLS.json',checks)
    if not all(checks.values()) or cfg['alphas']!=list(ALPHAS) or cfg['selection_rule']!='fixed-byte-greedy-priority':raise ValueError('controls/design')
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='byte-retention',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
    ...
    (root/'raw/byte_retention_summary_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    write(root/'EVIDENCE_ROLES.json',dict(reader='no new reader input',evaluator='supplied equal source prior,laws,complete posteriors and source identities;fixed-byte-budget recency and midpoint-first priority retention; explicit prior remainder;all future coordinates;undefined errors retain NaN',scope='matched byte budgets including masses,metadata and shared prior;source counts may differ;no learned provenance or historical correspondence'))
    return dict(controls=checks,posterior_rows=len(rows)//len(ALPHAS),rows=len(rows),sources=source_total,report_queries=len(rows)*8,unavailable_checkpoints=unavailable,numerical_acceptance=False,scope='full and recency/midpoint priorities at fixed byte budgets;8endpoint reports;equal source prior;no new observations,models or protected lineages')
