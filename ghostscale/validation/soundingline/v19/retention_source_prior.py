...
def reuse_uniform(saved, ids):
    """Carry accepted uniform numerical outputs through an explicit storage schema.

    The source archive and complete input binding are checked by run(). This
    conversion neither recalculates forecasts nor changes numerical outcomes.
    """
    if not np.array_equal(saved['source_rows'], ids): raise ValueError('uniform source identity')
    raw = {k:np.asarray(v).copy() for k,v in saved.items()}
    n = len(ids)
    raw['source_probability'] = np.full(n,1/n)
    raw['source_normalizer'] = np.array(float(n))
    raw['forgotten_context_mass'] = raw.pop('forgotten_context_counts')/n
    raw['compact_float64_count'] += 13
    raw['compact_int32_count'] -= 12
    summarize(raw)
    return raw
...
def run(root,plan,pulse):
    ...
    if not all(checks.values()) or cfg['alphas']!=list(ALPHAS) or cfg['selection_rule']!='midpoint-sorted-time-ranks' or cfg['source_priors']!=list(PRIORS):raise ValueError('controls/design')
    ...
    inherited=read(root/'inputs/uniform/FINAL_REVIEW.json')
    if not inherited['numerical_acceptance']: raise ValueError('uniform numerical acceptance')
    inherited_plan=read(root/'inputs/uniform/PLAN.json')
    if inherited_plan['design']['selection_rule']!=cfg['selection_rule'] or inherited_plan['design']['alphas']!=cfg['alphas'] or inherited_plan['design']['lineages']!=cfg['lineages'] or inherited_plan['design']['draws']!=cfg['draws']: raise ValueError('uniform design')
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='retention-source-prior',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
                        uniform_path=root/'inputs/uniform/raw'/f'{prefix}-{index//cfg["batch_rows"]:03d}_points.npz'
                        with np.load(uniform_path,allow_pickle=False) as z:uniform_batch={k:z[k] for k in z.files}
                    ...
                    for prior in PRIORS:
                        ...
                        raw=(reuse_uniform({k[len(tag):]:v for k,v in uniform_batch.items() if k.startswith(tag)},source_ids) if prior=='uniform' else evaluate(w,st,law,source_ids,op,prior));metrics=summarize(raw)
    ...
    (root/'raw/retention_source_prior_summary_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    write(root/'EVIDENCE_ROLES.json',dict(reader='no new reader input',evaluator='supplied uniform,time-proportional and reciprocal-time source priors,laws,complete posteriors and source identities;matched-count recency and evenly spaced sorted-time retention; explicit prior remainder;all future coordinates;undefined errors retain NaN',scope='time-balanced source retention;no learned provenance or historical correspondence'))
