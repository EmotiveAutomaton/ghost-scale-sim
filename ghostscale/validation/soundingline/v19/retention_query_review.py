"""Independent query-context retention Bayes, proper-loss and storage reconstruction."""
...
def calculate(w, st, law, ids):
    ...
    # Independently select each fixed query, then integrate over all future times.
    query_regret = []; query_maximum = []
    for context in CONTEXTS:
        loss = excess[..., context].mean(axis=-1)
        distance = (delta[..., context, :]**2).sum(-1).mean(axis=-1)
        close(loss[both], distance[both], 'query one-hot proper-loss regret')
        query_regret.append(np.where(both, loss, np.nan))
        query_maximum.append(np.where(both, abs(delta[..., context, :]).max(axis=(-2, -1)), np.nan))
    query_regret = np.asarray(query_regret); query_maximum = np.asarray(query_maximum)
    close(query_regret.mean(axis=0)[both], regret[both], 'equal-query mean regret')
    maximum = abs(delta).max(axis=(-3, -2, -1)); tv = .5*abs(post-post[0:1]).sum(-1)
    close(query_maximum.max(axis=0)[both], maximum[both], 'query maximum recombination')
    ...
    out = dict(query_squared_regret=query_regret, query_max_future_probability_error=query_maximum, source_rows=ids.copy(), retained=keep, retained_sources=keep.sum(-1),
    ...
    for context in CONTEXTS:
        for wi, label in enumerate(WINDOWS):
            for suffix, values in [('expected_squared_regret', query_regret), ('expected_max_probability_error', query_maximum)]:
                summary[f'context{context}_'+label+'_'+suffix] = np.array([math.fsum(float(report[0, ai, e])*float(values[context, wi, ai, e]) for e in range(8) if both[wi, ai, e]) for ai in range(5)])
    return out, summary
...
def run(root, plan, pulse):
    ...
    if (design['alphas'] != list(ALPHAS) or design['selection_rule'] != 'midpoint-sorted-time-ranks' or design['query_contexts'] != list(CONTEXTS)) or not read(parent/'PARENT_REVIEW.json')['numerical_acceptance']: raise ValueError('design/parent')
    specs = read(parent/'SCHEDULES.json'); structures = {k:structure(v) for k, v in specs.items()}
    recorded = json.loads(gzip.decompress((original/'raw/retention_query_summary_points.json.gz').read_bytes()))
            ...
            for key, spec in specs.items():
                ...
                for i, (r, identity) in enumerate(zip(chosen, ids)):
                    if i % design['batch_rows'] == 0:
                        ...
                        batch_keys = set(batch); pulse(phase='independent-retention-query-review', lineage=lineage, evidence=evidence, checkpoint=cp, row=i)
    ...
    return dict(passed=True, controls=checks, rows=len(all_rows), posterior_rows=len(all_rows)//5, sources=sources, report_queries=len(all_rows)*8, strata=len(grouped), raw_batches=len(used), numerical_acceptance=False, scope='all four fixed query contexts,equal-context mean regret and maximum recombination,window identities,mass,histograms,report probabilities,support masks,posteriors,future coordinates,one-hot proper losses,total variation,storage and summaries reconstructed;separate original-row regroup and event adjudication required')
