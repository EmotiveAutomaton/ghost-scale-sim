"""Fixed-bin reliability of frozen fine-goal modal reports."""
...
def probabilities(x):
    ...
    if not np.isfinite(x).all() or np.any(x < 0) or np.any(x > 1+TOL):
...
def measure(p, c, w):
    """One step, complete frame support; explicit infinite proper loss."""
    p, c, w = (np.asarray(x, float) for x in (p, c, w))
    if p.ndim != 1 or p.shape != c.shape or p.shape != w.shape:
        raise ValueError('metric shape')
    pp, cc = probabilities(p), probabilities(c)
    if not np.isfinite(w).all() or np.any(w < 0) or abs(w.sum()-1) > 1e-10:
        raise ValueError('weights')
    codes = bins(p)
    bw = np.bincount(codes, weights=w, minlength=10)
    bp = np.bincount(codes, weights=w*p, minlength=10)
    bc = np.bincount(codes, weights=w*c, minlength=10)
    unsupported = ((pp == 0) & (cc > 0)) | ((pp == 1) & (cc < 1))
    infinite_mass = float(w[unsupported].sum())
    terms = np.zeros_like(p)
    take = (cc > 0) & (pp > 0)
    terms[take] -= cc[take]*np.log(pp[take])
    take = (cc < 1) & (pp < 1)
    terms[take] -= (1-cc[take])*np.log1p(-pp[take])
    return dict(confidence=float(w @ p), correctness=float(w @ c),
        signed_error=float(w @ (p-c)), bin_error=float(np.abs(bp-bc).sum()),
        squared_error=float(w @ ((p-c)**2)),
        brier=float(w @ (cc*(1-pp)**2+(1-cc)*pp**2)),
        binary_loss=None if infinite_mass > 0 else float(w @ terms),
        infinite_loss=infinite_mass > 0, infinite_mass=infinite_mass,
        clipped_count=int(np.count_nonzero((p != pp) | (c != cc))),
        clipped_mass=float(w @ ((p != pp) | (c != cc))),
...
def controls():
    w=np.array([.25,.75]); p=np.array([.6,.9]); c=np.array([.2,.4])
    own=measure(p,p,w); over=measure(p,c,w); uniform=measure(np.array([1/3]),np.array([1/3]),np.ones(1))
    return {'live:known_overconfidence': abs(over['signed_error']-.475)<1e-14,
        ...
        'positive:uniform_calibrated': uniform['signed_error']==0,
        'positive:edges': bins(np.array([.1,1.])).tolist()==[1,9]}
...
def aggregate(rows, cfg):
    ...
    for tier,forecast,weighting,step in product(cfg['tiers'],FORECASTS,WEIGHTS,range(3)):
        def records(arm,budget):return [idx[tier,budget,arm,forecast,weighting,step,l,d,s] for l,d,s in product(ls,ds,ss)]
        ...
        for arm,budget in product(ARMS,bs):
            ...
            m={k:float(np.mean([r[k] for r in rr])) if all(r[k] is not None for r in rr) else None for k in METRICS+('confidence','correctness','infinite_mass','clipped_mass')}
            ...
            means.append(dict(tier=tier,forecast=forecast,weighting=weighting,step=step,arm=arm,budget=budget,
                infinite_cells=sum(r['infinite_loss'] for r in rr),pooled_bin_error=float(np.abs(bp-bc).sum()),
...
def run(root,plan,pulse):
    ...
    rows=[];native=[];frames=0;error=0.;reproduced=0;timing=[];(root/'reports').mkdir()
    uniform=np.full(len(keys),1/len(keys))
    for li,l in enumerate(ls):
        mode=native_marg[li].argmax(2);p=np.take_along_axis(native_marg[li],mode[:,:,None],2)[:,:,0]
        for weighting,step in product(WEIGHTS,range(3)):
            v=measure(p[:,step],p[:,step],mass[li] if weighting=='native' else uniform)
            native.append(dict(lineage=l,weighting=weighting,step=step,**v))
    write(root/'evaluator/NATIVE_CALIBRATION.json',native)
    for draw,seed,budget,arm,forecast in product(cfg['training_draws'],cfg['fit_seeds'],cfg['budgets'],ARMS,FORECASTS):
        pulse(phase='goal-calibration',draw=draw,seed=seed,budget=budget,arm=arm,forecast=forecast);tick=time.process_time()
        ...
        np.savez_compressed(root/'reports'/(stem+'.npz'),chosen=chosen,confidence=p,native_correctness=correct,bin_ids=bins(p))
        ...
        for li,l in enumerate(ls):
            ...
            reproduced+=1
            for weighting,step in product(WEIGHTS,range(3)):
                v=measure(p[:,step],correct[li,:,step],mass[li] if weighting=='native' else uniform)
                rows.append(dict(tier='E2-full',budget=budget,arm=arm,forecast=forecast,weighting=weighting,step=step,lineage=l,draw=draw,seed=seed,fine_loss=float(loss[li]),**v))
    ...
    (root/'goal_calibration_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    ...
    return dict(controls=checks,rows=len(rows),native_rows=len(native),frame_forecasts=frames,packets=len(keys),parent_cells=reproduced,parent_max_error=error,**aggregate(rows,cfg),fits=0,scope='frozen modal-confidence calibration; native correctness is evaluator-only; no historical correspondence')
