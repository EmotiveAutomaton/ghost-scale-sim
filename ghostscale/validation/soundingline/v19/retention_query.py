"""Query-specific forecast errors for matched-count source retention.
...
def evaluate(weight, st, law, source_rows, operator=None):
    ...
    query_square = np.moveaxis((delta*delta).sum(-1).mean(axis=-2), -1, 0)
    query_max = np.moveaxis(abs(delta).max(axis=(-3, -1)), -1, 0)
    if np.max(abs(query_square.mean(axis=0)-squared)) > TOL:
        raise ValueError('query recombination')
    query_square = np.where(both[None], query_square, np.nan)
    query_max = np.where(both[None], query_max, np.nan)
    ...
    return dict(query_squared_regret=query_square, query_max_future_probability_error=query_max, source_rows=ids.astype(np.int32), retained=keep, retained_sources=keep.sum(-1),
...
def summarize(raw):
    ...
    if raw['query_squared_regret'].shape != (4, 5, 5, 8) or raw['query_max_future_probability_error'].shape != (4, 5, 5, 8):
        raise ValueError('query shape')
    mask = np.broadcast_to(~raw['comparable'], (4, 5, 5, 8))
    ...
    if np.nanmax(abs(raw['query_squared_regret'].mean(axis=0)-raw['future_squared_regret'])) > TOL:
        raise ValueError('query recombination')
    if np.nanmax(abs(raw['query_max_future_probability_error'].max(axis=0)-raw['max_future_probability_error'])) > TOL:
...
def run(root,plan,pulse):
    ...
    if not all(checks.values()) or cfg['alphas']!=list(ALPHAS) or cfg['selection_rule']!='midpoint-sorted-time-ranks' or cfg['query_contexts']!=list(CONTEXTS):raise ValueError('controls/design')
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='retention-query',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
