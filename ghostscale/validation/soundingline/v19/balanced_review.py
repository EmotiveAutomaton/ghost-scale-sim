"""Independent context-balanced retention Bayes, proper-loss and storage reconstruction."""
...
def select(ids, cp):
    """Order by within-context recency rank, then fixed context rank.

    This total ordering implements round-robin without the producer's queues.
    Unique source timestamps are validated before entry. Endpoint values do not
    participate in the ordering.
    """
    times = [int(row[0]) for row in ids]
    contexts = [int(row[1]) for row in ids]
    recent = [[t > cp//2 for t in times], [t > 3*cp//4 for t in times]]
    masks = [[True]*len(ids), *recent]
    ranks = [sum(c == contexts[i] and t > times[i] for t, c in zip(times, contexts))
             for i in range(len(ids))]
    for order in ORDERS:
        ordering = sorted(range(len(ids)), key=lambda i:(ranks[i], order.index(contexts[i])))
        for mask in recent:
            chosen = set(ordering[:sum(mask)])
            masks.append([i in chosen for i in range(len(ids))])
...
def controls():
    ...
    return {'live:older_informative_context': bool(live['balanced_half_expected_squared_regret'][0] < live['recent_half_expected_squared_regret'][0]-.001),
        ...
        'positive:matched_counts': bool(np.array_equal(raw['retained_sources'][1:3], raw['retained_sources'][3:5]) and np.array_equal(raw['retained_sources'][1:3], raw['retained_sources'][5:7]))}
...
def run(root, plan, pulse):
    ...
    if (design['alphas'] != list(ALPHAS) or design['context_orders'] != [list(x) for x in ORDERS]) or not read(parent/'PARENT_REVIEW.json')['numerical_acceptance']: raise ValueError('design/parent')
    specs = read(parent/'SCHEDULES.json'); structures = {k:structure(v) for k, v in specs.items()}
    recorded = json.loads(gzip.decompress((original/'raw/balanced_retention_summary_points.json.gz').read_bytes()))
            ...
            for key, spec in specs.items():
                ...
                for i, (r, identity) in enumerate(zip(chosen, ids)):
                    if i % design['batch_rows'] == 0:
                        ...
                        batch_keys = set(batch); pulse(phase='independent-balanced-review', lineage=lineage, evidence=evidence, checkpoint=cp, row=i)
    ...
    write(root/'EVIDENCE_ROLES.json', dict(reader='no new reader inputs', evaluator='independent source Bayes factors,joint group updates,bit schedules,every future coordinate and compact storage;undefined masks and support failures distinct;all original source mass retained in fixed context and recency rosters and original-prior remainder'))
