"""Fixed supplied input equivalence on retained primitive tables; no fitting."""
from itertools import product
import time
import numpy as np
from ..v18_3.io import digest, read, write, file_digest
from . import primitive_feedback as F

ARMS = ('unpooled', 'pool-undo', 'pool-current-wrong')
BUDGETS = (0, 16)


def group(key, arm):
    if arm not in ARMS:
        raise ValueError('pooling map')
    s, b, a, u, op = key
    if s == 1 and op == 3:
        if arm == 'pool-undo': u = -1
        if arm == 'pool-current-wrong': a = -1
    return s, b, a, u, op


def pooled_counts(counts, observed, arm):
    ...
    if arm not in ARMS: raise ValueError('pooling map')
    legal = set(F.roster('forward')); unique = {}
    for key, target in observed:
        key = tuple(key)
        if key not in legal or target not in range(8): raise ValueError('feedback')
        if key in unique and unique[key] != target: raise ValueError('conflicting duplicate input')
        unique[key] = target
    groups = {}
    for key in F.roster('forward'):
        g = group(key, arm)
        if g not in groups: groups[g] = np.ones(8)
        groups[g] += counts[key] - 1
    for key, target in unique.items(): groups[group(key, arm)][target] += 1
    result = counts.copy()
    for key in F.roster('forward'): result[key] = groups[group(key, arm)]
    return result, dict(unique_inputs=len(unique), observed_groups=len({group(k, arm) for k in unique}),
                        total_tool_groups=len(groups), prior_per_outcome_per_group=1)


def controls():
    counts = np.ones((2, 2, 8, 8, 6, 8)); key = (1, 0, 2, 2, 3)
    counts[key][3] += 2
    baseline, _ = pooled_counts(counts, [], 'unpooled')
    pooled, _ = pooled_counts(counts, [], 'pool-undo')
    observed = [(key, F.observation(key, 'original'))]
    once, info = pooled_counts(counts, observed, 'pool-undo')
    twice, _ = pooled_counts(counts, observed*2, 'pool-undo')
    untouched = np.ones(counts.shape[:-1], dtype=bool); untouched[1, :, :, :, 3] = False
    semantics = all(len({F.observation((1,b,a,u,3), rule) for u in range(8)}) == 1
                    for rule,b,a in product(F.M.RULES,range(2),range(8)))
    wrong = any(len({F.observation((1,b,a,u,3), rule) for a in range(8)}) > 1
                for rule,b,u in product(F.M.RULES,range(2),range(8)))
    table = F.R.normalized(once); q=(1,0,2,3,5,4)
    return {'live:known_tool_equivalence': semantics, 'placebo:wrong_pool_not_equivalent': wrong,
            'positive:unpooled_identity': np.array_equal(baseline, counts),
            'positive:one_prior': bool(pooled[1,0,2,0,3,3] == 3 and pooled[1,0,2,0,3].sum() == 10),
            'positive:undo_preserved': np.array_equal(once[untouched], counts[untouched]),
            'positive:deduplicate': np.array_equal(once, twice),
            'positive:group_counts': info['total_tool_groups'] == 16 and info['observed_groups'] == 1,
            'positive:path_sum': np.allclose(F.path_sum(table,q),F.R.propagate(table,q),atol=1e-12,rtol=0)}
...
def run(root, plan, pulse):
    ...
    for draw,mode in product(cfg['training_draws'],('original','composition-holdout')):
        start=time.process_time(); pulse(phase='pooling-parent',draw=draw,mode=mode)
        ...
        for rule,order,feedback,budget in product(F.M.RULES,F.ORDERS,F.FEEDBACK,BUDGETS):
            ...
            for arm in ARMS:
                ...
                pulse(phase='pooling-query',**identity)
                revised,info=pooled_counts(counts,observed,arm); table=F.R.normalized(revised)
                ...
                if arm=='unpooled':
                    ...
                    if not np.array_equal(pred,old): raise ValueError('unpooled feedback identity')
    ...
    write(root/'TIMING.jsonl',dict(measurements=timings,accounting='pooling,propagation,path sums and scores; no refit'))
    return dict(controls=checks,cells=cells,queries=len(qs),score_rows=rows,fits=0,scope='supplied semantic input equivalence; fixed one-count feedback; no learned law discovery,process correspondence or human intent')
