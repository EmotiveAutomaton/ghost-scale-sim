"""Finite deterministic storage policies under an interval of cue reliabilities."""
...
def solve(selection, counts, interval=((1,3),(1,1))):
    ...
    optimal = lambda r:max(at(p,r) for p in policies)
...
def controls():
    ...
    r=solve(s,(3,1,0));point=solve(s,(3,1,0),((2,3),(2,3)))
    null=fixture([1,2],[1,1],1,{'a':[1],'b':[2]},[[F(1,2)]*2]*3)
    return {'live:known_compromise':r['worst_regret']==[1,6] and r['fixed_worst_regret']==[1,4],
        'positive:point_channel':point['worst_regret']==[0,1],
        'placebo:identical_priors':all(x['worst_regret']==[0,1] for x in compare(null)['mixtures']),
        'positive:feasible_policy':all(max(p['used_bytes'])<=1 for p in r['policies'])}
...
def run(root, plan, pulse):
    ...
    for key,records in groups.items():
        pulse(phase='robust-channel-regret-libraries',library=len(selections))
    ...
    for i,r in enumerate(population):
        if i%256==0: pulse(phase='robust-channel-regret-rosters',row=i)
    ...
    (root/'raw/robust_channel_regret_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
