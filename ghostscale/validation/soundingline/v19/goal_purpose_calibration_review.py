"""Independent scalar public-purpose/bin sums and complete paired-law regrouping."""
...
def purpose_groups(packets):
    ...
    for packet in packets:
        ...
        request=packet['inputs']['requested_purpose']
        if type(request) is not int or request not in (0,1):raise ValueError('requested purpose')
        # Read only the legal public request; never infer adopted purpose.
        labels.append([request,request,request])
    return np.array(labels,dtype=int)


def sufficient(p,c,w,purposes):
    """Scalar memberships and accurate sums, independent of bincount aggregation."""
    p,c,w=(np.asarray(x,float) for x in (p,c,w));ops=np.asarray(purposes)
    if p.ndim!=3 or p.shape[1:]!=(3,3) or c.ndim!=4 or c.shape[1:]!=p.shape or w.shape!=c.shape[:2]:raise ValueError('shape')
    if ops.shape!=p.shape[:2] or not np.issubdtype(ops.dtype,np.integer) or np.any(ops<0) or np.any(ops>=2):raise ValueError('purpose')
    for a in (p,c):
    ...
    for t,g in product(range(3),range(3)):
        ...
        groups=[[i for i,code in enumerate(codes) if code==j] for j in range(20)]
    ...
    return dict(sums,purpose_mass=mass,conditional_signed_error=signed,
...
def controls():
    ...
    return {'live:opposite_purpose_error':bool(np.max(x['pooled_bin_error'])<1e-14 and np.min(x['cancellation_gap'][0,:,:2])>.099),
        ...
        'positive:mass':bool(np.max(abs(x['purpose_mass'].sum(3)-1))<1e-14)}
...
def run(root,plan,pulse):
    ...
    if design['tiers']!=['E2-full'] or design['bin_edges']!=list(EDGES) or design['purposes']!=list(PURPOSES):raise ValueError('design')
    ...
    ops=purpose_groups([packets['packets'][k] for k in keys])
    ...
    raw=json.loads(gzip.decompress((original/'goal_purpose_calibration_points.json.gz').read_bytes()))
    ...
    if schema['axes']!=['law','weighting','position','goal','purpose','bin'] or schema['law']!=ls or schema['weighting']!=list(WEIGHTS) or schema['purpose']!=list(PURPOSES) or schema['bin_edges']!=list(EDGES):raise ValueError('array schema')
    ...
    for draw,seed,budget,arm,forecast in product(design['training_draws'],design['fit_seeds'],design['budgets'],ARMS,FORECASTS):
        pulse(phase='independent-purpose-calibration',draw=draw,seed=seed,budget=budget,arm=arm,forecast=forecast)
        ...
        empty_bins+=int(np.sum(rebuilt['bin_weight']==0));empty_groups+=int(np.sum(rebuilt['purpose_mass']==0))
    ...
    result=dict(passed=True,rows=len(rows),packets=len(keys),frame_forecasts=frames,parent_cells=parents,group_bundles=frames//len(keys),empty_bins=empty_bins,empty_groups=empty_groups,controls=checks,
        **{'max_'+k+'_error':v for k,v in errors.items()},scope='scalar purpose/bin sums, empty means, native self, parent identities and paired-law estimates; no new fit or historical correspondence')
