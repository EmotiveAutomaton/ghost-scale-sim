"""Independent source-context Bayes and explicit categorical-loss reconstruction."""
...
def calculate(w, st, law, ids):
    """Enumerate source factors and reports without the producer's kernels."""
    ...
    count = len(ids); independent = np.empty((count, 8)); averaged = np.zeros((4, 8, len(w)))
    for s, (t, ctx, old) in enumerate(ids):
        ...
        averaged[ctx] += law[states, ctx].T / count
    reports = [(None, e) for e in range(8)] + [(ctx, e) for ctx in range(4) for e in range(8)]
    selected = [list(range(count)) if ctx is None else [s for s in range(count) if ids[s, 1] == ctx] for ctx, e in reports]
    source_endpoint = np.empty((5, count, 8)); report = np.empty((5, 40))
    source_post = np.zeros((5, count, 40)); posterior = np.zeros((5, 40, len(w))); entropy = np.zeros((5, 40))
    for ai, a in enumerate(ALPHAS):
        for s, (_, _, old) in enumerate(ids):
            for e in range(8): source_endpoint[ai, s, e] = math.fsum((a if e == old else 0., (1-a)*float(independent[s, e])))
        for r, (ctx, e) in enumerate(reports):
            sources = selected[r]
            den = math.fsum(float(source_endpoint[ai, s, e]) for s in sources) / count; report[ai, r] = den
            if den:
                for s in sources: source_post[ai, s, r] = source_endpoint[ai, s, e] / (count*den)
                copy = sum(ids[s, 2] == e for s in sources) / count
                likelihood = sum((averaged[c, e] for c in range(4)), start=np.zeros(len(w))) if ctx is None else averaged[ctx, e]
    ...
    future = (future_mass(posterior.reshape(200, len(w)), st) @ law.reshape(16, 32)).reshape(5, 40, -1, 4, 8)
    ...
    regret = np.empty((5, 32))
    for r, (ctx, e) in enumerate(reports[8:]):
        regret[:, r] = ((future[:, r+8]-future[:, e])**2).sum(-1).mean(axis=(-2, -1))
    out = dict(source_rows=ids.copy(), source_endpoint_probability=source_endpoint, report_probability=report,
               ...
               no_report_squared_loss=np.full((5, 40), base_loss.mean()),
               future_squared_gain=np.where(possible, direct_gain, np.nan),
               context_to_endpoint_squared_regret=np.where(possible[:, 8:], regret, np.nan))
    summary = {}
    for label, members in [('endpoint', range(8)), ('context', range(8, 40))]:
        summary[label+'_supported_mass'] = np.asarray([math.fsum(float(p[r]) for r in members) for p in report])
        summary[label+'_impossible_reports'] = np.asarray([sum(not mask[r] for r in members) for mask in possible])
        for key, values in [('source_entropy', entropy), ('future_optimal_squared_loss', optimal), ('future_squared_gain', gain)]:
            summary[label+'_expected_'+key] = np.asarray([math.fsum(float(report[a, r])*float(values[a, r]) for r in members if possible[a, r]) for a in range(5)])
    ...
    gap = summary['context_expected_future_squared_gain']-summary['endpoint_expected_future_squared_gain']
    explicit = np.asarray([math.fsum(float(report[a, r+8])*float(regret[a, r]) for r in range(32) if possible[a, r+8]) for a in range(5)])
    entropy_gap = summary['endpoint_expected_source_entropy']-summary['context_expected_source_entropy']
    close(gap, explicit, 'context refinement proper-loss identity')
    if np.min(gap) < -TOL or np.min(entropy_gap) < -TOL: raise ValueError('refinement monotonicity')
    summary['context_added_future_gain'] = gap
    summary['context_to_endpoint_regret'] = explicit
    summary['context_removed_source_entropy'] = entropy_gap
    return out, summary
...
def controls():
    ...
    _, live = calculate(w, st, law, [[1, 0, 0], [2, 1, 1]])
    _, same = calculate(w, st, law, [[1, 0, 0], [2, 0, 1]])
...
def run(root, plan, pulse):
    ...
    recorded = json.loads(gzip.decompress((original/'raw/report_context_summary_points.json.gz').read_bytes()))
            ...
            for key, spec in specs.items():
                ...
                for i, (r, identity) in enumerate(zip(chosen, ids)):
                    if i % design['batch_rows'] == 0:
                        ...
                        batch_keys = set(batch); pulse(phase='independent-report-context-review', lineage=lineage, evidence=evidence, checkpoint=cp, row=i)
    ...
    for k, v in dict(rows=len(all_rows), posterior_rows=len(all_rows)//5, sources=sources, report_queries=len(all_rows)*40, unavailable_checkpoints=unavailable).items():
    ...
    write(root/'EVIDENCE_ROLES.json', dict(reader='no new reader inputs', evaluator='independent scalar source Bayes factors,hypothesis updates and every future coordinate;undefined masks and support failures distinct;equal source prior and complete source roster supplied;fixed endpoint and context/endpoint report alphabets'))
    return dict(passed=True, controls=checks, rows=len(all_rows), posterior_rows=len(all_rows)//5, sources=sources, report_queries=len(all_rows)*40, strata=len(grouped), raw_batches=len(used), numerical_acceptance=False, scope='all report probabilities,source weights,entropy,support masks,posteriors,future coordinates,one-hot proper losses,context-refinement regrets and summaries reconstructed;separate original-row regroup and event adjudication required')
