"""Query-announced matched-count source retention with four-state costs.
...
BASE_WINDOWS = ('full', 'recent_half', 'recent_quarter', 'spaced_half', 'spaced_quarter')
WINDOWS = BASE_WINDOWS + tuple(f'announced{c}_{size}' for c in CONTEXTS for size in ('half', 'quarter'))
...
def retention(ids, checkpoint):
    ...
    base = [np.ones(len(ids), bool), *recent,
            *(spaced_mask(ids, int(k.sum())) for k in recent)]
    # Announce a context BEFORE retaining sources; use no endpoint or gain.
    for context in CONTEXTS:
        order = np.lexsort((-ids[:, 0], ids[:, 1] != context))
        for count in (int(k.sum()) for k in recent):
            mask = np.zeros(len(ids), bool)
...
def evaluate(weight, st, law, source_rows, operator=None):
    ...
    state_direction = np.cumsum(increments, axis=-2)
    both = possible & possible[0:1]
    applicable = np.zeros((4,len(WINDOWS)),bool)
    applicable[:,:5] = True
    query_square = np.full((4,len(WINDOWS),5,8),np.nan)
    query_max = np.full_like(query_square,np.nan)
    # Only the announced context is an eligible target of a conditional state.
    # Stream those contexts to avoid an unnecessary all-alpha/all-query tensor.
    for context in CONTEXTS:
        applicable[context,5+2*context:7+2*context] = True
        indices = np.flatnonzero(applicable[context])
        future_direction = state_direction[indices] @ law[:,context,:]
        shift = beta[indices,...,None,None]*future_direction[:,None]
        delta = shift-shift[0:1]
        query_square[context,indices] = np.where(both[indices],(delta*delta).sum(-1).mean(-1),np.nan)
        query_max[context,indices] = np.where(both[indices],abs(delta).max(axis=(-2,-1)),np.nan)
    squared = np.full((len(WINDOWS),5,8),np.nan)
    maximum = np.full_like(squared,np.nan)
    squared[:5] = query_square[:,:5].mean(0)
    maximum[:5] = query_max[:,:5].max(0)
    quotient_shift = beta[..., None]*group(direction, st)[:, None]
    tv = .5*abs(quotient_shift-quotient_shift[0:1]).sum(-1)
    eligible = applicable[:,:,None,None] & both[None]
    if np.any(query_max[eligible] > np.broadcast_to(tv,query_max.shape)[eligible]+TOL): raise ValueError('total variation bound')
    ...
    return dict(applicable_query=applicable, query_squared_regret=query_square, query_max_future_probability_error=query_max, source_rows=ids.astype(np.int32), retained=keep, retained_sources=keep.sum(-1),
...
def summarize(raw):
    ...
    for wi,label in enumerate(BASE_WINDOWS):
    ...
    if raw['query_squared_regret'].shape != (4, len(WINDOWS), 5, 8) or raw['query_max_future_probability_error'].shape != (4, len(WINDOWS), 5, 8):
        raise ValueError('query shape')
    applicable = np.zeros((4,len(WINDOWS)),bool); applicable[:,:5] = True
    for c in CONTEXTS: applicable[c,5+2*c:7+2*c] = True
    if not np.array_equal(raw['applicable_query'],applicable): raise ValueError('query applicability')
    mask = ~(applicable[:,:,None,None] & raw['comparable'][None])
    if not np.array_equal(np.isnan(raw['query_squared_regret']), mask) or not np.array_equal(np.isnan(raw['query_max_future_probability_error']), mask):
        raise ValueError('query support masks')
    tv = np.broadcast_to(raw['updated_group_total_variation'],mask.shape)
    regret, maximum = raw['query_squared_regret'],raw['query_max_future_probability_error']
    # Probability pushforward contracts total variation. For zero-sum errors,
    # squared Euclidean distance is at most twice total variation squared.
    if (not np.isfinite(regret[~mask]).all() or not np.isfinite(maximum[~mask]).all()
        or np.any(regret[~mask] < -TOL) or np.any(maximum[~mask] < -TOL)
        or np.any(maximum[~mask] > tv[~mask]+TOL)
        or np.any(regret[~mask] > 2*tv[~mask]**2+TOL)):
        raise ValueError('query probability distance bound')
    if np.nanmax(abs(raw['query_squared_regret'][:,:5].mean(axis=0)-raw['future_squared_regret'][:5])) > TOL:
        raise ValueError('query recombination')
    if np.nanmax(abs(raw['query_max_future_probability_error'][:,:5].max(axis=0)-raw['max_future_probability_error'][:5])) > TOL:
        raise ValueError('query maximum')
    if not np.isnan(raw['future_squared_regret'][5:]).all() or not np.isnan(raw['max_future_probability_error'][5:]).all():
        raise ValueError('conditional state has no universal score')
    for context in CONTEXTS:
        for wi, label in enumerate(BASE_WINDOWS):
            for suffix, field in [('expected_squared_regret', 'query_squared_regret'), ('expected_max_probability_error', 'query_max_future_probability_error')]:
                out[f'context{context}_'+label+'_'+suffix] = (truth*np.nan_to_num(raw[field][context,wi])).sum(-1)
    for context in CONTEXTS:
        for offset, size in enumerate(('half', 'quarter')):
            wi = 5 + 2*context + offset
            label = WINDOWS[wi]
            out[label+'_unsupported_mass'] = (truth*(~possible[wi])).sum(-1)
            out[label+'_supported_mass'] = (truth*possible[wi]).sum(-1)
            out[label+'_expected_squared_regret'] = (truth*np.nan_to_num(raw['query_squared_regret'][context,wi])).sum(-1)
            out[label+'_expected_max_probability_error'] = (truth*np.nan_to_num(raw['query_max_future_probability_error'][context,wi])).sum(-1)
            out[label+'_expected_group_total_variation'] = (truth*np.nan_to_num(raw['updated_group_total_variation'][wi])).sum(-1)
            for key in ('retained_sources','forgotten_mass','compact_float64_count','compact_int32_count','full_hypothesis_float64_count'):
                out[label+'_'+key] = np.full(5,raw[key][wi])
            if raw['retained_sources'][wi] != raw['retained_sources'][1+offset]:
                raise ValueError('announced matched count')
    # Four separately retained states pay four sets of masses and metadata.
    # The 32-float law-derived prior table is shared once, outside these totals.
    for offset, size in enumerate(('half', 'quarter')):
        for key in ('compact_float64_count','compact_int32_count'):
            out['four_announced_'+size+'_'+key] = np.full(5, sum(raw[key][5+2*c+offset] for c in CONTEXTS))
    return out
...
def controls():
    ...
    return {'live:older_informative_time':bool(live['announced0_half_expected_squared_regret'][0]<live['context0_recent_half_expected_squared_regret'][0]-.001),
...
def run(root,plan,pulse):
    ...
    if not all(checks.values()) or cfg['alphas']!=list(ALPHAS) or cfg['selection_rule']!='announced-context-first-then-recency' or cfg['query_contexts']!=list(CONTEXTS):raise ValueError('controls/design')
            ...
            for key,spec in specs.items():
                ...
                for index,(r,identity) in enumerate(zip(chosen,ids)):
                    if index%cfg['batch_rows']==0:
                        pulse(phase='announced-query-retention',lineage=lineage,evidence=evidence,checkpoint=cp,row=index)
    ...
    (root/'raw/announced_query_summary_points.json.gz').write_bytes(gzip.compress(canonical(rows),mtime=0))
    write(root/'EVIDENCE_ROLES.json',dict(reader='no new reader input',evaluator='supplied equal source prior,laws,complete posteriors and source identities;matched-count recency and evenly spaced sorted-time retention; explicit prior remainder;every future coordinate separately by all four query contexts;undefined errors retain NaN',scope='query announced before compression;only matching context scored for each conditional state;four states charged separately;no learned provenance or historical correspondence'))
    return dict(controls=checks,posterior_rows=len(rows)//len(ALPHAS),rows=len(rows),sources=source_total,report_queries=len(rows)*8,unavailable_checkpoints=unavailable,numerical_acceptance=False,scope='full,recent and spaced half/quarter plus four separately announced half/quarter states;only announced context scored per conditional state;four-state storage charged;equal source prior;all endpoints;no new observations,models or protected lineages')
