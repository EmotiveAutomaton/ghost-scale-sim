"""Independent time-balanced retention Bayes, proper-loss and storage reconstruction."""
...
def run(root, plan, pulse):
    ...
    if (design['alphas'] != list(ALPHAS) or design['selection_rule'] != 'midpoint-sorted-time-ranks') or not read(parent/'PARENT_REVIEW.json')['numerical_acceptance']: raise ValueError('design/parent')
    specs = read(parent/'SCHEDULES.json'); structures = {k:structure(v) for k, v in specs.items()}
    recorded = json.loads(gzip.decompress((original/'raw/time_retention_summary_points.json.gz').read_bytes()))
            ...
            for key, spec in specs.items():
                ...
                for i, (r, identity) in enumerate(zip(chosen, ids)):
                    if i % design['batch_rows'] == 0:
                        ...
                        batch_keys = set(batch); pulse(phase='independent-time-retention-review', lineage=lineage, evidence=evidence, checkpoint=cp, row=i)
