"""Independent bounded-retention Bayes, proper-loss and storage reconstruction."""
...
ALPHAS = (0., .25, .5, .75, 1.)
WINDOWS = ('full', 'half', 'quarter', 'none')
...
def calculate(w, st, law, ids):
    ...
    keep = np.array([[True, t > cp//2, t > 3*cp//4, False] for t, _, _ in ids]).T
    q = np.bincount(st['mapping'], weights=w, minlength=G)
    factors = np.zeros((4, 8, len(w))); histogram = np.zeros(8); contexts = np.zeros((4, 4), dtype=np.int64)
    ...
    joint = np.array([np.bincount(st['mapping'], weights=w*f, minlength=G) for f in factors.reshape(32, len(w))]).reshape(4, 8, G)
    ...
    close(report.sum(-1), np.ones((4, 5)), 'report mass')
    ...
    independent = (project(joint.reshape(32, G), st['signatures']) @ law.reshape(16, 32)).reshape(4, 8, times, 4, 8)
    ...
    out = dict(source_rows=ids.copy(), retained=keep, retained_sources=keep.sum(-1),
        ...
        compact_int32_count=np.array(integers, dtype=np.int64), full_hypothesis_float64_count=np.full(4, len(w), dtype=np.int64),
...
def controls():
    ...
    _, flat = calculate(w, st, law, [[1, 0, 0], [4, 1, 0]])
    law[:8, 0] = np.eye(8)[0]; law[8:, 0] = np.eye(8)[1]
    _, live = calculate(w, st, law, [[1, 0, 0], [4, 1, 0]])
    return {'live:old_informative_source': bool(live['half_expected_squared_regret'][0] > .01),
        'placebo:constant_law': bool(abs(flat['none_expected_squared_regret']).max() < TOL),
        'positive:full_retention_identity': bool(abs(live['full_expected_squared_regret']).max() < TOL),
        'placebo:certain_copy_identity': bool(abs(live['none_expected_squared_regret'][-1]) < TOL)}
...
def run(root, plan, pulse):
    ...
    recorded = json.loads(gzip.decompress((original/'raw/retrospective_forgetting_summary_points.json.gz').read_bytes()))
            ...
            for key, spec in specs.items():
                ...
                for i, (r, identity) in enumerate(zip(chosen, ids)):
                    if i % design['batch_rows'] == 0:
                        ...
                        batch_keys = set(batch); pulse(phase='independent-forgetting-review', lineage=lineage, evidence=evidence, checkpoint=cp, row=i)
    ...
    write(root/'EVIDENCE_ROLES.json', dict(reader='no new reader inputs', evaluator='independent source Bayes factors,joint group updates,bit schedules,every future coordinate and compact storage;undefined masks and support failures distinct;all original source mass retained in fixed windows and original-prior remainder'))
    return dict(passed=True, controls=checks, rows=len(all_rows), posterior_rows=len(all_rows)//5, sources=sources, report_queries=len(all_rows)*8, strata=len(grouped), raw_batches=len(used), numerical_acceptance=False, scope='all window identities,mass,histograms,report probabilities,support masks,posteriors,future coordinates,one-hot proper losses,total variation,storage and summaries reconstructed;separate original-row regroup and event adjudication required')
