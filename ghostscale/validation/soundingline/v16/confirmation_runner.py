...
def specification(root, fixture):
    if fixture:
        ...
        selected_packet = read(root/"packets/confirmation-selection-1.json")
        record = read(path)
        claims = record["selected"]
        if selected_packet["packet_hash"] != selection["packet_hash"] or record["packet_hash"] != selection["packet_hash"] or claims != selected_packet["identity"]["design"]["claims"]:
            raise ValueError("confirmation claims differ from the pre-data selection packet")
        selection_hash = file_digest(path)
    validate_claims(claims, fixture=fixture)
    cards = []
    for claim in claims:
...
def validate_claims(claims, *, fixture=False):
    if len(claims) > 3 or len({claim["card_id"] for claim in claims}) != len(claims) or len({claim["claim_id"] for claim in claims}) != len(claims):
        raise ValueError("confirmation requires at most three distinct frozen card claims")
    for claim in claims:
        conditions = claim["condition_ids"]
        n = claim["n_constructor_packets"]
        histories = 2 if claim["card_id"] == "M01" else len(conditions)
        if not conditions or len(set(conditions)) != len(conditions) or type(n) is not int or not 2 <= n <= 4096:
            raise ValueError("invalid confirmation condition or constructor allocation")
        if claim["histories_per_constructor_packet"] != histories or claim["total_fresh_acquisition_histories"] != n*histories or n*histories > 4096:
            raise ValueError("confirmation must respect its actual primary-history cap before generating data")
        if claim["alpha_planning"] != .05/3 or not claim["namespace"].startswith("fixture-v16-confirmation-" if fixture else "v16-confirmation-1-"):
            raise ValueError("confirmation requires its declared fresh namespace and familywise allocation")
        if not fixture and claim["power"]["planning_state"] != "adequate":
            raise ValueError("underpowered claims must remain exploratory before confirmation data")
...
def actual_controls(root, base, entry, packet, destination, runtime_join):
    ...
    with ReaderProcess(next_attempt(destination/"warm-readers"), extensions=EXTENSIONS) as raw:
        ...
        for condition in entry["design"]["conditions"]:
            ...
            if path.exists():
                receipt = read(path)
                if receipt["source_unit_sha256"] != file_digest(base/"units"/(row["unit_id"]+"_points.json")) or receipt["control_packet_hash"] != packet["packet_hash"]:
                ...
                receipt = condition_control(base, row, attempt, raw, set(entry["design"]["adversaries"]))
    ...
    if not cold_path.exists():
        with ReaderProcess(next_attempt(destination/"cold-readers"), extensions=EXTENSIONS) as raw:
            ...
            write(cold_path, {"instrument_state": "valid", "fresh_process": raw.child.pid != warm_pid,
                "requests": reader.calls, "resources": reader.reader.samples,
                "warm_pid": warm_pid, "cold_pid": raw.child.pid})
    ...
    result = {"execution_state": "completed", "instrument_state": "valid", "card_id": entry["card_id"],
        ...
        "conditions": checks, "source_inventory_sha256": file_digest(destination/"source_inventory_points.json"),
        "cold_sha256": file_digest(cold_path), "runtime_join": runtime_join,
...
def execute(root, heartbeat, *, resume=False, fixture=False):
    ...
    for entry in spec["cards"]:
        ...
        heartbeat(card_id=card, completed_units=completed, planned_units=spec["planned_condition_records"], operation="actual confirmation source controls")
