...
def command_for_goal(belief,target_cell):
    return max(range(4),key=lambda command:(float(np.dot(belief,LIKELIHOOD[command,target_cell])),-command))


def construction(belief,goal):
    index=GOALS.index(goal)
    values=PAIR_SUCCESS[index]@np.asarray(belief)
    return list(COMMAND_PAIRS[int(np.argmax(values))])
...
def choose(payload:bytes,policy):
    ...
    if policy=="value-learning" and max(scores)<=0:
        domain=None
    else:
        maximum=max(scores)
