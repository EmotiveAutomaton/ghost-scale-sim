...
def condition(source, row, receipt):
    ...
    physical_checks, syntax_only = 0, 0
    if "X02" in required:
        if len(evidence["recoding"]) != len(frames):
            raise ValueError("independent recoding control count differs")
        for frame, item in zip(frames, evidence["recoding"]):
            if len(item["representations"]) != 2:
                raise ValueError("independent macro representation count differs")
            kind = frame["kind"]
            physical_applicable = kind == "reading" or kind.endswith("recognition:public_reader") or "inquiry_stable:" in kind or kind in {"inquiry", "inquiry-learning", "inquiry-construction", "inquiry-reading"}
            if physical_applicable:
                if item["physical"].get("passed") is not True:
                    raise ValueError("declared physical recoding did not pass")
                physical_checks += 1
            else:
                if item["physical"] != {"scope": "macro representation only; no physical isomorphism declared for this algorithm"}:
                    raise ValueError("unsupported physical recoding must retain its explicit scope limitation")
                syntax_only += 1
    ...
    return {"source_frames": len(frames), "recorded_reader_calls": len(receipt["requests"]),
        "physical_recodings_checked": physical_checks, "macro_only_recodings": syntax_only,
...
def run(root, name, report):
    ...
    for entry in packet["identity"]["design"]["cards"]:
        ...
        if len(saved_conditions) != len(condition_reports) or {item["condition"] for item in saved_conditions} != set(expected):
            raise ValueError("independent saved control condition inventory differs")
        for checked in condition_reports:
            original = next(item for item in saved_conditions if item["condition"] == checked["condition"])
            if original.get("receipt_sha256", original.get("sha256")) != checked["receipt_sha256"]:
                raise ValueError("independent saved condition receipt identity differs")
        cold = read(cold_path)
