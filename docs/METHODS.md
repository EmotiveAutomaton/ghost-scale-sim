...
Asymmetric cue errors enumerate every three-label deterministic storage policy, retaining four rectangle-vertex scores, nine diagnostics, all minimax ties, reference policies and expected/realized byte charges. All 58 isolated controls pass. Complete-support synthetic runs took [5.3125, 5.234375, 5.375] CPU seconds; the inclusive estimate is 951.889 of 1,800 seconds. Original and both full replays have execution receipts; independent numerical acceptance awaits completion-event review. [Frozen contract](versions/v19-local-maker/ASYMMETRIC_CHANNEL_IMPLEMENTATION.md).

Independent repeated-cue review integrates elementary copy/fresh latent events, rebuilding every ordered-pair probability, conditional score, tie, value and byte charge across 28 libraries and 3,780 problems. It verifies 57,344 rows and 7,741,440 joins, 60,480 paired strata, 30,240 law strata and 3,780 equal-law cells. All 174 isolated controls and both full replays pass; the initial incomplete review fixture remains archived. [Report](versions/v19-local-maker/REPEATED_CUE_REPORT.md).

Repeated cue dependence integrates all nine ordered label pairs under copy rates zero, one-half and one; it preserves every conditional mask score, tie, byte charge and signed loss from assuming independence. All 220 isolated controls pass. Complete-support synthetic runs took [9.515625, 9.5625, 9.5] CPU seconds; the inclusive estimate is 974.581 of 1,800 seconds. Original and both full replays have execution receipts; independent numerical acceptance awaits completion-event review. [Frozen contract](versions/v19-local-maker/REPEATED_CUE_IMPLEMENTATION.md).

Independent cue-cost review reconstructs ancestor costs and rosters, all 28 libraries, 11,790 paid policy scores, every free/paid tie and break-even fee, then verifies 57,344 rows and 10,321,920 joins, 80,640 paired strata, 40,320 law strata and 5,040 equal-law cells. All 105 isolated controls and both full replays pass. Utility fees remain hypothetical and separate from CPU/bytes. [Report](versions/v19-local-maker/CUE_ACQUISITION_COST_REPORT.md).

Cue acquisition cost enumerates every paid three-label policy and free fixed mask under four hypothetical source-mass fees, preserving exact ties and break-even values. All 99 isolated controls pass; full-support synthetic CPU times were [2.84375, 3.015625, 3.09375]. The inclusive estimate is 916.078 of 1,800 seconds. A failed null fixture had inconsistent masses and baseline; its failure/source/log remain retained, and the fixture constructor now supplies a consistent null. Original and both complete replays have execution receipts, with numerical acceptance pending completion-event review. [Frozen contract](versions/v19-local-maker/CUE_ACQUISITION_COST_IMPLEMENTATION.md).

Independent channel-risk review reconstructs ancestor costs, all source rosters and 28 libraries; exact per-label integration verifies every policy before independent lottery enumeration and a convex-hull optimum check. It verifies 21,171 candidate lotteries including repeated threshold tables, all 57,344 rows and 2,580,480 joins, 20,160 paired strata, 10,080 law strata and 1,260 equal-law cells. All 67 isolated controls and both full replays pass. The unrestricted informed reference remains fixed when risky policies are excluded. [Report](versions/v19-local-maker/CHANNEL_REALIZED_RISK_REPORT.md).

Channel realized-risk constraints exclude policies before exact lottery optimization, preserving the unrestricted informed reference. Integer rational arithmetic retains every policy and extreme lottery; identical survivor sets share a table without deleting comparisons. All 43 isolated controls pass; full-support synthetic CPU times were [1.875, 1.90625, 1.9375]. The inclusive estimate is 925.491 of 1,800 seconds. The initial timing fixture collapsed 28 libraries to 20; its failed assertion and evidence are retained, and the repaired fixture covers all 28. The original was observed running, and original plus both full replays have execution receipts. Numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/CHANNEL_REALIZED_RISK_IMPLEMENTATION.md).

Independent coupled-arithmetic review reconstructs rational objective segments from boundary values, verifying all 220 vertex intersection events, 48 boundary vectors, 24 segments, 72 strata and nine equal-law summaries. All 29 isolated controls and both complete replays verify. A guaranteed optimum exists in all 24 problems, but every original computed-float tie stays optimal in only 17. The containing box certifies 22 problems. A zero-regret choice already exists everywhere on this path, resolving the prepared minimax-regret comparison without another run. [Report](versions/v19-local-maker/COUPLED_PRECISION_REPORT.md).

Coupled arithmetic interpolation uses exact rational lines and every vertex/objective intersection to partition one shared interpolation weight. All boundary objective vectors, interval ties, optimum sets and maximum regrets are retained and compared with the containing box. All 46 isolated controls pass; full-support synthetic CPU times were [2.25, 2.140625, 2.375]. The inclusive estimate is 910.194 of 1,800 seconds. Original and both full replays executed; numerical acceptance remains pending completion-event review. This is a sensitivity model, not certified real-valued accuracy. The initial fixture-type and portable-dispatch failures are retained with their source and logs. [Frozen contract](versions/v19-local-maker/COUPLED_PRECISION_IMPLEMENTATION.md).

Independent precision-stability review uses rational endpoints and sparse-coordinate contrast vectors to reconstruct all 9,360 ordered pairs and 149,760 vertex contrasts. All 72 strata, nine equal-law cells, 44 isolated controls and both complete replays verify. It certifies an optimum in 22 of 24 problems and every original tied choice in 17. Maximum regret is bounded by 4.797291036640061e-14 at length 128 within the specified arithmetic box; exact real-valued range containment remains unproved. [Report](versions/v19-local-maker/PRECISION_STABILITY_REPORT.md).

Precision stability converts producer/scalar range intervals to exact integers, retains every feasible ordered assignment pair and all sixteen vertex contrasts, and bounds regret and unresolved choices conservatively. All 41 isolated controls pass; full-support synthetic CPU times were [0.6875, 0.703125, 0.703125]. The inclusive estimate is 890.108 of 1,800 seconds. Original and both full replays executed; numerical acceptance remains pending completion-event review. The arithmetic box does not certify exact real-valued ranges. [Frozen contract](versions/v19-local-maker/PRECISION_STABILITY_IMPLEMENTATION.md). Randomized channel regret remains timing-blocked after one exact cache repair: the inclusive estimate is 2,071.663 against 1,800 seconds. Both source versions and synthetic fixtures are retained; no lottery performance result is accepted. [Timing record](../results/v19/G19-B-randomized-channel-regret-timing-1/README.md).

Independent robust-channel review reconstructs every policy and evaluates all score-line intersections and endpoints with rational arithmetic. All 420 structural problems, 57,344 rows, 860,160 joined evaluations, paired/equal-law summaries, 44 isolated controls and both complete replays verify. The robust choice matches the two-thirds nominal choice in all 420 problems and reduces worst expected regret relative to fixed storage in 147. Realized coverage loss stays separate. [Report](versions/v19-local-maker/ROBUST_CHANNEL_REGRET_REPORT.md).

Robust channel regret enumerates all deterministic three-label policies and minimizes maximum expected regret over cue reliability from one-third to one. Exact score lines reduce the maximum to endpoints; separate complete intersection checks validate that reduction. Realized coverage loss and individually feasible byte charges remain separate. All 37 isolated controls pass; full-support synthetic CPU times were [39.5, 39.21875, 39.234375]. The inclusive estimate is 1171.957 of 1,800 seconds. Original and both full replays executed; numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/ROBUST_CHANNEL_REGRET_IMPLEMENTATION.md).

Independent cue-channel review enumerates complete policies with rational arithmetic, freezes choices under nominal reliability and evaluates actual reliability while keeping the source mixture fixed. All 3,780 structural problems, 57,344 rows, 7,741,440 joined evaluations, paired/equal-law summaries, 46 isolated controls and both complete replays verify. The complete regroup is losslessly archived. Channel error remains separate from source-mixture error and forecast accuracy. [Report](versions/v19-local-maker/CHANNEL_MISSPECIFICATION_REPORT.md).

Cue-reliability misspecification freezes storage choices under each nominal channel before scoring under every actual channel, with the source mixture correctly supplied. It preserves nominally impossible labels, rational regret, signed gains and individually feasible bytes. All 39 isolated controls pass; complete-support synthetic CPU times were [5.25, 5.4375, 5.53125]. The frozen inclusive estimate is 859.752 of 1,800 seconds. Original and both full replays executed through the existing queue; numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/CHANNEL_MISSPECIFICATION_IMPLEMENTATION.md).

Independent prior-mixture review enumerates complete policies with rational arithmetic, selects nominal policies before evaluating actual mixtures, and reconstructs ancestor rosters, costs and prior optima. All 18,900 structural problems, 57,344 rows, 38,707,200 joined evaluations, paired/equal-law summaries, 21 isolated controls and both complete replays verify. The large complete regroup is losslessly archived. Source-mixture error remains separate from channel error and forecast accuracy. [Report](versions/v19-local-maker/PRIOR_MIXTURE_REPORT.md).

Prior-mixture misspecification freezes storage choices under each nominal mixture before scoring under every actual mixture, with cue reliability correctly supplied. It preserves zero nominal probability with positive actual probability, rational regret, signed gains and individually feasible bytes. All 26 isolated controls pass; complete-support synthetic CPU times were [5.96875, 6.296875, 6.171875]. The frozen inclusive estimate is 866.105 of 1,800 seconds. Original and both full replays executed through the existing queue; numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/PRIOR_MIXTURE_IMPLEMENTATION.md).

Independent noisy-prior review enumerates complete contingent policies using exact rational arithmetic and reconstructs ancestor rosters, costs and prior optima. All 1,260 channel problems, conditional scores, zero labels, ties, certainty choices, penalties and bytes verify, along with 57,344 rows, 2,580,480 joined evaluations, paired/equal-law summaries, 76 isolated controls and both full replays. Correctly using information and overtrusting its label remain separate. [Report](versions/v19-local-maker/NOISY_PRIOR_REPORT.md).

Noisy prior disclosure enumerates 15 mixtures under symmetric prior-identity channels with correctness probabilities 1/3, 2/3 and 1. It compares no cue, correct channel use and treating the label as certain, preserving exact conditional choices, ties, undefined zero-probability labels and individually feasible bytes. All 41 isolated controls pass; complete-support synthetic CPU times were [4.84375, 4.96875, 4.984375]. The frozen inclusive estimate is 874.050 of 1,800 seconds. Original and both full replays executed through the existing queue; numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/NOISY_PRIOR_IMPLEMENTATION.md).

Independent partial-disclosure review exhaustively enumerates contingent storage policies using rational arithmetic. All 2,100 disclosure problems, conditional scores, ties, zero-probability cells, information values and byte charges verify, along with 57,344 relational rows, 4,300,800 joined evaluations, 33,600 paired strata, 16,800 law strata, 2,100 equal-law cells, 43 isolated controls and both complete replays. Gross information value remains separate from acquisition cost and forecast accuracy. [Report](versions/v19-local-maker/PARTIAL_PRIOR_REPORT.md).

Partial prior disclosure enumerates all five partitions of three supplied prior identities and all 15 mixtures in quarters. It records exact conditional choices, all ties, zero-probability cells, gross information value and individually feasible bytes. All 39 isolated controls pass; complete-support synthetic runs took 2.15625, 2.234375 and 2.25 CPU seconds. The inclusive admission estimate is 829.283 of 1,800 seconds. Original and both full replays executed through the existing queue; independent numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/PARTIAL_PRIOR_IMPLEMENTATION.md).

Independent prior-information review exhaustively enumerates informed policies and reconstructs the robust coverage lottery using rational arithmetic. All 420 mixture problems, fixed and revealed choices, ties, information values and byte charges verify, along with 57,344 relational rows, 860,160 joined evaluations, paired/equal-law summaries, 46 isolated controls and both complete replays. Gross information value remains separate from acquisition cost and forecast accuracy. [Report](versions/v19-local-maker/PRIOR_INFORMATION_REPORT.md).

Prior-information storage enumerates all 15 denominator-four mixtures of three supplied source priors. It compares prior-revealed choices, the best fixed mask and the robust coverage lottery with exact rational masses, all ties and individually feasible bytes. All 41 isolated controls pass. Complete-support synthetic runs took 2.109375, 2.234375 and 2.296875 CPU seconds; the repaired inclusive admission estimate is 922.894 of 1,800 seconds. A handler-name failure occurred before science; its source, plan and charge remain preserved. A new plan changes only that name and both entry points are checked against it. The existing queue executed the corrected original and two full replays; numerical acceptance remains pending completion-event review. [Frozen contract](versions/v19-local-maker/PRIOR_INFORMATION_IMPLEMENTATION.md).

Independent realized-risk review reconstructs source rosters, structural costs, prior optima and all finite libraries. Full-constraint rational optimization verifies three realized-mass floors, every surviving and excluded mask, every basic optimum and tie, weights, expected masses and byte charges. All 172,032 rows, paired/equal-law summaries, 29 isolated controls and both complete replays verify. Expected coverage and realized protection remain separate. [Report](versions/v19-local-maker/REALIZED_RISK_STORAGE_REPORT.md).

Realized-risk storage filters the frozen finite mask library by three exact realized-mass floors before optimizing minimum expected coverage. Every realized mask separately meets its byte budget. Thirty-three isolated controls pass, including independent full-constraint rational optimization, known risk-cost tradeoffs and monotone feasible sets. Complete-support synthetic runs took 6.046875, 6.40625 and 6.375 CPU seconds; the inclusive admission estimate is 942.451 of 1,800 seconds. Original and both full replays executed through the existing queue; independent numerical review remains pending. [Frozen contract](versions/v19-local-maker/REALIZED_RISK_STORAGE_IMPLEMENTATION.md).

Independent randomized-regret review reconstructs all source rosters, structural costs and 84 prior-specific optima. Full-constraint rational vertex enumeration using Cramer's rule checks every basic lottery, tie choice, weight, expected prior regret, both baselines, coverage cost, realized regret and byte charge. All 57,344 rows, paired/equal-law summaries, 30 isolated controls and both complete replays verify. Expected protection and worst realized protection remain separate. [Report](versions/v19-local-maker/RANDOMIZED_SOURCE_REGRET_REPORT.md).

Randomized source-prior regret enumerates exact basic lotteries over individually byte-feasible masks, retaining all optimal basic solutions and a frozen rational lexicographic tie choice. Expected regret, minimum expected-mass cost and worst realized regret stay separate. Thirty-two isolated controls pass, including independent full-constraint determinant enumeration; a first failed dispatch-coverage check is retained and repaired. Complete-support synthetic runs took 2.75, 2.953125 and 2.953125 CPU seconds; inclusive admission estimate 914.994 of 1,800 seconds. Original and both full replays executed through the existing queue; independent numerical review remains pending. [Frozen contract](versions/v19-local-maker/RANDOMIZED_SOURCE_REGRET_IMPLEMENTATION.md).

Independent randomized-storage review reconstructs all source rosters, structural costs and 84 prior-specific optima. Full-constraint rational vertex enumeration using Cramer's rule checks all basic lotteries, tie choices, weights, expected masses and individually feasible byte charges. All 57,344 rows, paired/equal-law summaries, 34 isolated controls and both complete replays verify. Expected protection and worst realized protection remain separate. [Report](versions/v19-local-maker/RANDOMIZED_SOURCE_STORAGE_REPORT.md).

Randomized source storage enumerates exact basic lotteries over individually feasible masks, retaining all optimal basic solutions and a frozen rational lexicographic tie choice. Expected mass, worst realized mass and realized byte charges remain separate. Thirty-seven isolated controls pass, including independent full-constraint determinant enumeration. Complete-support synthetic runs took 1.859375, 1.984375 and 1.953125 CPU seconds; inclusive admission estimate 902.098 of 1,800 seconds. Original and both full replays executed through the existing queue; independent numerical review remains pending. [Frozen contract](versions/v19-local-maker/RANDOMIZED_SOURCE_STORAGE_IMPLEMENTATION.md).

Independent source-prior regret review reconstructs all source rosters and structural costs, 84 unrestricted prior-specific optima, 28 finite libraries, all rational regrets and ties, all 57,344 original rows and paired/equal-law summaries. Thirty-three isolated controls and both complete replays verify. Minimum worst regret and maximum minimum coverage choose different masks in seven problems, with coverage costs retained. [Report](versions/v19-local-maker/SOURCE_PRIOR_REGRET_REPORT.md).

Source-prior regret minimizes the largest exact rational opportunity loss against the prior-informed feasible optimum, within the frozen finite library. Every regret tie and the chosen minimum-mass cost remain visible. Thirty-three isolated controls pass. Complete-support synthetic runs took 1.859375, 2.0 and 2.140625 CPU seconds; inclusive admission estimate 903.615 of 1,800 seconds. The existing queue executed original and both full replays; independent numerical review remains pending. [Frozen contract](versions/v19-local-maker/SOURCE_PRIOR_REGRET_IMPLEMENTATION.md).

Independent robust source-mass review reconstructs source rosters and structural costs from provenance, all 84 prior-specific optima by exhaustive half-subsets, 28 candidate libraries with exact fractions, every mask, tie, byte and regret, all 57,344 rows, 448 paired strata and equal-law summaries. Original and both full replays verify; 32 isolated controls pass. The result is a candidate-library storage comparison with supplied priors. [Report](versions/v19-local-maker/ROBUST_SOURCE_MASS_REPORT.md).

Robust source-prior retention evaluates a finite deduplicated library of twelve policy identities per roster/budget problem. Exact rational masses under three prior vertices determine the greatest worst-case retained mass; every tie, recent-time choice, prior optimum, regret and byte charge is retained. Forty-one isolated controls pass. Complete-support synthetic comparisons took 1.609375, 1.65625 and 1.75 CPU seconds; inclusive admission estimate 899.912 of 1,800 seconds. The existing queue executed original and both full replays; independent numerical review remains pending. [Frozen contract](versions/v19-local-maker/ROBUST_SOURCE_MASS_IMPLEMENTATION.md).

Independent precision-regret review reconstructs all 24 law arrays and eight allocation arrays with scalar arithmetic, then verifies all 648 assignments, four vertices, three lengths, bytes, regret/absolute/balanced/vertex optima, undefined cases, uniform baselines and absolute-bound tradeoffs. All 72 original rows, nine equal-law cells and both complete replays verify; 33 isolated controls pass. Thirty-five arithmetic tie-set changes, including seven regret sets, remain separate from exact computed-float identities. [Report](versions/v19-local-maker/CONTEXT_PRECISION_REGRET_REPORT.md).

Minimax precision regret retains all 81 assignments per law and compares their worst log-range regret against vertex-informed feasible optima. It preserves all regret, absolute, balanced and vertex ties, absolute bounds and byte charges; undefined infinity-minus-infinity regret cannot win. Forty-five isolated controls pass, including exhaustive scalar optima, all denominator-four convex mixtures and a known absolute-versus-regret counterexample. Complete synthetic comparisons took 0.125, 0.125 and 0.15625 CPU seconds; the inclusive admission estimate is 885.160 of 1,800 seconds. Independent numerical acceptance awaits completion review. [Frozen contract](versions/v19-local-maker/CONTEXT_PRECISION_REGRET_IMPLEMENTATION.md).

Robust precision review independently reconstructs 24 law arrays and eight allocation arrays, all 648 assignments, four count vertices, three lengths, every optimum and regret, actual byte charges, 72 original rows and nine equal-law cells. Both complete replays verify; 35 isolated controls pass. Twenty-eight scalar tie changes are retained separately from computed-float identities. No attained-error or exact-real certification claim. [Report](versions/v19-local-maker/ROBUST_CONTEXT_PRECISION_REPORT.md).

Robust context precision enumerates all 81 assignments over four fixed query-count vertices at three lengths and byte budgets. It retains robust, balanced and vertex-specific optima, all ties, vertex regret, byte costs and uniform baselines. Thirty-five isolated controls pass, including exhaustive scalar minimax, all denominator-four convex mixtures, a balanced-choice counterexample, support loss and undercharged bytes. Three complete synthetic comparisons took 0.125, 0.125 and 0.140625 CPU seconds; the inclusive source-admission estimate is 884.854 of 1,800 seconds. Numerical acceptance awaits its completion review. [Frozen contract](versions/v19-local-maker/ROBUST_CONTEXT_PRECISION_IMPLEMENTATION.md).

Context precision review independently reconstructs all law values and exhaustively enumerates 81 assignments per law. Exact computed-float tie identities are checked separately from scalar normalization sensitivity; seven tie sets change, with maximum accumulated log-range regret 1.42e-14 at 128 observations. All 648 assignments, actual byte costs, dominated alternatives, 72 rows, nine equal-law cells and both complete replays verify. The first failed tie-identity check remains recorded; 28 isolated controls now pass without changing the producer or its frozen scoring rule. [Report](versions/v19-local-maker/CONTEXT_PRECISION_REPORT.md).

Independent schedule verification reconstructs binary casts, normalized laws, every statewise ratio and support mask, context maxima and attaining endpoints, integer schedules and exponential bounds. All 24 array sets, 144 original strata, 18 equal-law cells and both full replays verify; 30 isolated controls pass. The finite precision-allocation successor exhaustively retains 81 assignments per law, all byte charges, ties, dominance and feasible uniform baselines. Its 31 controls and complete synthetic timing pass. An immutable test-receipt helper failed after passing controls; the uniquely retained receipt was recovered by matching every source hash, and timing was repeated for the corrected source. Failed administrative work and superseded timing remain charged. [Schedule report](versions/v19-local-maker/SCHEDULE_LIKELIHOOD_ENVELOPE_REPORT.md); [allocation contract](versions/v19-local-maker/CONTEXT_PRECISION_IMPLEMENTATION.md).

Independent likelihood verification uses standard-library binary casts, scalar normalization/logarithms and an exponential extremal bound. All 24 raw array sets, 72 original strata and nine equal-law cells reconstruct, with both complete replay identities; 34 isolated controls pass. The fixed-query-schedule successor retains all context extrema and attaining endpoint sets, sums ranges over two prespecified balanced schedules, and checks equal-count order identity. Its 20 controls and full-support synthetic timing pass; numerical outcomes remain unaccepted. A failed portable-dispatch control and a failed pre-freeze document lookup are preserved and charged. [Likelihood report](versions/v19-local-maker/LAW_LIKELIHOOD_ENVELOPE_REPORT.md); [schedule protocol](versions/v19-local-maker/SCHEDULE_LIKELIHOOD_ENVELOPE_PROTOCOL.md).

Independent source-mass verification uses exhaustive meet-in-the-middle subsets rather than the producing dynamic program, separate structural sets, complete parent source rosters, exact integer/rational checks and all 688,128 original rows. The 5,376 paired strata, 2,688 law strata and 336 equal-law cells verify with both complete replays. All 32 isolated review/producer controls pass. The separate likelihood-envelope instrument derives the extremal posterior total-variation bound from a bounded likelihood ratio; explicit rational priors and likelihood products check it. Its 21 isolated controls and three complete synthetic timings pass, with a source-admission estimate of 884.24 of 1,800 CPU seconds. No likelihood result is accepted. [Allocation report](versions/v19-local-maker/SOURCE_MASS_FRONTIER_REPORT.md); [likelihood contract](versions/v19-local-maker/LAW_LIKELIHOOD_IMPLEMENTATION.md).

Independent law-rounding review reconstructs scalar binary casts, all coordinate extrema and attaining states, support masks, row-mass drift and explicit 65-digit expected one-hot losses. All 192 original rows and 24 equal-law cells verify with both complete replays. Source-mass allocation uses exact rational weights and integer-byte dynamic programming with recent-time tie breaks. All 28 isolated controls pass, including exhaustive small-subset comparisons, empty/full/zero-cost budgets, permutations, greedy failure and input corruption. Every original retained roster and storage budget is reproduced before projection; all policies pay the same 56-byte weighted-schema increment. Complete synthetic-support timings are 16.4375 and 16.171875 CPU seconds; the source-admission estimate is 1,021.59 of 1,800 seconds including verification and write-through reserves. The allocation outcome is not yet accepted. [Rounding report](versions/v19-local-maker/LAW_ROUNDING_ENVELOPE_REPORT.md); [allocation contract](versions/v19-local-maker/SOURCE_MASS_BYTE_IMPLEMENTATION.md).

The supplied-law rounding-envelope handler retains the original and stored laws, direct and once-normalized reconstructions, coordinate extrema with every attaining state, squared row-error bounds, mass drift, exact lost-support masks and explicit expected one-hot loss changes. All vertices, 360 fixed rational two-state mixtures and the uniform mixture are checked per variant. Scalar arithmetic, rare endpoints, state/context/endpoint permutations, corrupted support and deliberately undersized bounds pass among 23 isolated controls. Complete-support timing includes three executions and reserves independent reconstruction, equal-law regroup and documentary work within 1,800 CPU seconds. No fit, new observation or numerical scientific acceptance is involved in this admission. [Protocol](versions/v19-local-maker/LAW_ROUNDING_ENVELOPE_PROTOCOL.md).
...
Shared-law precision freezes the exact report state and evaluates float64/32/16 future laws with explicit expected one-hot loss, normalization drift and support loss. Highest-probability retention freezes three source priors and seven outcome-blind source masks with a weighted remainder. Both complete-support benchmarks include three executions and independent reconstruction. All controls, failed admission estimates, exact-state storage bindings and mixture-factorization sources remain preserved. [Shared-law method](versions/v19-local-maker/SHARED_LAW_PRECISION_REPORT.md); [retention method](versions/v19-local-maker/RETENTION_MASS_PRIORITY_REPORT.md).
...
Independent per-source joint group/past-state contraction and full-hypothesis Bayes, bit schedules and indexed accumulation check all report and future coordinates. Explicit structural sets recount storage; exact support and NaN masks stay separate. All 48 isolated controls and all 19 original-row metrics in 1,120 paired strata verify. [Review protocol](versions/v19-local-maker/AGGREGATE_REPORT_REVIEW_PROTOCOL.md).
...
Aggregate single-report state retains future-group mass, the eight-column joint group/report table and eight copy counts under one fixed uniform source prior. It reconstructs full-hypothesis Bayes and every future coordinate; source-table masses, metadata, shared laws and schedules are counted separately. All 43 isolated controls pass. Complete-support timing preserves the 3,600-second inclusive cap, with independent reconstruction and regroup reserved. Complete independent reconstruction and all 1,120 paired original-row strata now verify. [Admission](versions/v19-local-maker/AGGREGATE_REPORT_STATE_ADMISSION.md).

Independent structural sets recount source costs and exact rational midpoint priorities rebuild fixed-budget greedy selections. Direct group/report Bayes numerators and bit schedules reconstruct all future coordinates; explicit one-hot losses check regret. Equal-budget/unequal-count fixtures guard against count-matching assumptions. All 104 isolated controls pass; separate standard-library original-row regroup now verifies all 65 metrics, both paired byte-budget contrasts and storage identities in 1,120 draw strata, 560 law strata and 70 population cells. Unequal retained counts remain explicit. [Protocol](versions/v19-local-maker/BYTE_RETENTION_REVIEW_PROTOCOL.md).
...
Independent direct group/report Bayes numerators and bit schedules reconstruct all four query contexts. Explicit one-hot losses check regret, and structural sets recount storage. Context permutation and context swapping test labels beyond pooled recombination. All 128 isolated controls pass; separate original-row regroup now verifies all ninety metrics, both count contrasts and all six context pairs within 1,120 draw strata, 560 law strata and seventy population cells. Per-report raw maximum recombination is verified before report averaging; all fifty inherited metrics reproduce exactly. [Protocol](versions/v19-local-maker/RETENTION_QUERY_REVIEW_PROTOCOL.md).
...
Independent rational midpoint-time selection, direct group/report Bayes numerators, bit schedules and indexed state accumulation reconstruct all five retention states and eight endpoint reports. Explicit one-hot proper losses check regret, and independent structural sets recount compact storage. Exact support and NaN masks remain separate. All 380 isolated controls pass; the separate original-row regroup now verifies all 83 metrics and null masks, with exact reproduction of the 50 inherited metrics. [Protocol](versions/v19-local-maker/RETENTION_HORIZON_REVIEW_PROTOCOL.md).
...
Independent ranked-context selection, direct group/report Bayes numerators, bit schedules and indexed state accumulation reconstruct all seven retention states and eight endpoint reports. Explicit one-hot proper losses check regret, and independent structural sets recount compact storage. Exact support and NaN masks remain separate. All 80 isolated controls pass; independent original-row regroup now verifies all 1,120 draw strata, 560 law strata and 70 population cells, including both fixed context orders and both matched-count contrasts. [Protocol](versions/v19-local-maker/BALANCED_REVIEW_PROTOCOL.md).
...
Independent source-wise Bayes sums, bit schedules and indexed state accumulation reconstruct all prior-specific forecasts. Explicit one-hot proper losses independently check squared forecast regret. All supports and NaN masks stay exact. The first failed identity fixture is retained; 74 controls pass after using exact binary fractions. Independent original-row regroup now verifies all 1,120 draw strata, 560 law strata and 70 population rows. [Protocol](versions/v19-local-maker/PRIOR_REVIEW_PROTOCOL.md).
...
The independent checker and separate 224-stratum regroup are accepted. Independent bit transitions and indexed accumulation reconstruct complete joint supports and masses. Direct hypothesis products and separate past-state marginals check every report and updated distribution. Singleton weights are verified exactly; all mixed cells and zero masses remain. Total variation certifies an all-future bound, not a measured forecast maximum. Storage counts and complete batch timing coverage are separate; per-query division is amortization. [Protocol](versions/v19-local-maker/SUFFICIENT_REVIEW_PROTOCOL.md).
...
The accepted checker and separate vector regroup verify all 80 rows. The prospective joint-state instrument uses direct full-hypothesis Bayes and the total-variation bound on every future probability; its bound is not a measured forecast maximum. The independent checker enumerates complete past-state and future-schedule bindings with direct bit transitions and integer counters. Scalar Bayes kernels and separately rearranged arithmetic verify both quotient weights; scalar endpoint differences reconstruct future-coordinate maxima. Every original identity and multiplicity remains bound before equivalent calculations are shared. Impossible reports and binary64/tolerance counts remain separate. [Protocol](versions/v19-local-maker/RETROSPECTIVE_REVIEW_PROTOCOL.md).
...
The joint-partition handler freezes the complete 27-outcome target, exact joint and marginal forecast groups, both populations and 10,000 paired law resamples. Every setting retains all memberships, using hash-bound reuse only for identical complete partitions. An independent cross-moment calculation passes exhaustive scalar controls. Independent numerical review and the third regroup now verify all 624 summaries; exact joint and marginal partitions coincide on the retained roster. [Contract](versions/v19-local-maker/JOINT_FORECAST_PARTITION_ADMISSION.md).
...
Independent scalar step-pair marginal reconstruction verifies every product and metric. A third direct-index regroup of original rows verifies all 80 paired estimates and 128 means within 1.43e-13. Saved binary64 products retain executed ties; native targets and original fits remain inherited verified inputs. [Protocol](versions/v19-local-maker/TEMPORAL_FACTORIZATION_REVIEW_PROTOCOL.md).
...
The independent factorization checker uses scalar integer-label marginal sums, verifies saved binary64 products before tie-sensitive scoring, reconstructs every metric and separately groups original and reconstructed rows with paired lineage multiplicities. [Protocol](versions/v19-local-maker/JOINT_FACTORIZATION_REVIEW_PROTOCOL.md).
...
Independent uncertainty review uses scalar label-modulo operation sums, native conditional ratios and accurate weighted sums. All 10,240 strata reconstruct within 2.67e-14. Paired grouping uses scalar bootstrap multiplicities; a separate direct-index bootstrap of original rows verifies all 360 estimates and 64 means within 1.53e-14; localization fields remain inherited identity checks. [Protocol](versions/v19-local-maker/JOINT_UNCERTAINTY_REVIEW_PROTOCOL.md).
...
Independent support review verifies all 23,040 strata using separate scalar mechanics, sparse frame-local target sums and saved-normalizer comparisons. A second original-row regroup uses scalar sums and bootstrap multiplicities to reproduce all 128 budget contrasts and 32 learning areas within 2.67e-15. Parent fitted forecasts and native posterior masses are inherited verified inputs; candidate ties are adjudicated on executed binary64 forecasts. [Protocol](versions/v19-local-maker/JOINT_SUPPORT_REVIEW_PROTOCOL.md).
...
[Verified report](versions/v19-local-maker/UNCERTAIN_RELIABILITY_REPORT.md). 53 scientific batches now have independent acceptance.
...
[Finite-set protocol](versions/v19-local-maker/UNCERTAIN_RELIABILITY_PROTOCOL.md), [retrospective alternative](versions/v19-local-maker/RETROSPECTIVE_QUOTIENT_PROTOCOL.md), [robust-decision alternative](versions/v19-local-maker/ROBUST_REPLY_DECISION_PROTOCOL.md).
...
The independent mismatch checker reconstructs scalar Bayes sums, legal mechanics, conditional weights, actual reply probabilities and realized-branch proper scores. It separates zero-support conventions and infinite loss, verifies identities and reader roles, and regroups paired laws with all actual/assumed accuracy axes retained. [Protocol](versions/v19-local-maker/MISMATCH_REVIEW_PROTOCOL.md).

The fixed reliability-mismatch handler reuses validated binary channel and realized-reply scoring primitives, separating assumed accuracy in forecasts from actual accuracy in evaluator weights. Its complete fixture is checked against independent scalar conditioning; eleven isolated controls pass. No fitted model, selected field or newly sampled episode is introduced. [Protocol](versions/v19-local-maker/RELIABILITY_MISMATCH_PROTOCOL.md).
...
The independent joint-reply checker verifies 3,072 cells and 737,280 forecast vectors. A second original-cell regroup verifies all 7,488 estimates within 3.33e-16. It uses agreement-pattern likelihoods, scalar Bayes sums and independently reconstructed legal mechanics. It checks complete strings, actual probabilities, zero-mass fallbacks, explicit infinite loss, no-request and identical-marginal identities, every branch score and reader allowlists. Paired law regrouping separates joint-source and assumed-channel effects. [Protocol](versions/v19-local-maker/JOINT_REPLY_REVIEW_PROTOCOL.md).

The joint-field reply handler and independent checker have verified numerical acceptance, all 7,488 paired estimates and both complete replays. It enumerates no request, each single field and both fields under independent and shared-flip errors with identical marginal reliability, comparing joint conditioning with multiplied marginals. Half-reliability shared errors preserve parity, a required positive control. All actual probabilities, fallback conventions and realized proper scores remain explicit. No fitting is performed. [Protocol](versions/v19-local-maker/JOINT_REPLY_PROTOCOL.md). Retrospective updating and [reliability mismatch](versions/v19-local-maker/RELIABILITY_MISMATCH_PROTOCOL.md) remain prepared independent alternatives.
...
The independent repeated-reply checker verifies 12,288 cells and 3,768,320 forecast vectors. A second original-cell regroup reproduces 29,952 paired estimates within 4.44e-16. It uses match-count likelihoods, scalar Bayes sums and independently reconstructed legal mechanics. It checks complete strings, impossible copied strings, zero-mass fallbacks, explicit infinite loss, copy and zero-request identities, every branch score and reader allowlists. Paired law regrouping keeps source dependence and return/acquisition cost separate. [Protocol](versions/v19-local-maker/REPEATED_REVIEW_PROTOCOL.md).

The repeated-reply comparison has independent numerical acceptance after all 29,952 paired estimates and both complete replays verified. Joint-field dependence is implemented for source admission; retrospective updating and reliability mismatch remain two independent prepared alternatives. No new fit or protected lineage is admitted. Complete binary strings retain actual channel probabilities separately from assumed posteriors. Copied mixed strings have zero actual mass; fallback forecasts remain explicit. Expected branch scores, request counts and one-acquisition copied-display costs are separate. [Protocol](versions/v19-local-maker/REPEATED_DISCLOSURE_PROTOCOL.md).
...
The complete noisy-reply checker verifies 4,608 cells and 23,808 laws; an independent original-cell multiplicity regroup verifies 14,256 paired estimates. Scalar choice sensitivity has zero logarithmic/squared score effect. The checker reconstructs scalar Bayes weights and actual reply probabilities independently, then averages realized-branch proper scores. It preserves explicit infinite-loss mass and zero-support fallback, checks half-channel and truthful-parent identities, and separates frozen field selection from numerical tie sensitivity. Paired law resampling keeps native and equal-query populations distinct. [Protocol](versions/v19-local-maker/NOISY_REVIEW_PROTOCOL.md).

The noisy mechanical-reply comparison is implemented and source admitted after 11 isolated controls. It enumerates channel accuracies of 0.5, 0.75 and 1, comparing calibrated conditioning with trusting replies while holding field selection fixed. Retrospective evidence updating and repeated independent-versus-copied replies remain two prepared alternatives. No new fit or protected lineage is admitted. [Protocol](versions/v19-local-maker/NOISY_DISCLOSURE_PROTOCOL.md).
...
The optional-request checker independently reconstructs conditional laws, strict price decisions, population-matched random request rates and realized-branch proper scores. It preserves original binary64 selection while separately measuring scalar-entropy threshold sensitivity; paired law resampling keeps the two scored populations distinct. [Protocol](versions/v19-local-maker/OPTIONAL_REVIEW_PROTOCOL.md).
...
Independent disclosure verification rebuilds scalar conditional laws, visible-group entropy choices, zero-support replies, every expected randomized proper score and cost, and reader projections. Separate original-cell bootstrap and numerical tie sensitivity checks qualify acceptance. Optional requests reuse the frozen laws, compare strict benefit-versus-price decisions with matched-rate random requests, and keep policy-score mixtures distinct from mixed predictions. [Disclosure review](versions/v19-local-maker/DISCLOSURE_REVIEW_PROTOCOL.md), [optional protocol](versions/v19-local-maker/OPTIONAL_DISCLOSURE_PROTOCOL.md).
...
Independent replacement verification enumerates each supplied input group and reconstructs scalar counts with a single prior, complete path-sum forecasts and paired draw-within-lineage estimates. A second bootstrap-multiplicity regroup verifies all contrasts. One-field disclosure separately chooses a field from visible-group conditional entropy; exact randomized-policy scores and disclosed-field costs are explicit. [Replacement review](versions/v19-local-maker/POOLED_REPLACEMENT_REVIEW_PROTOCOL.md), [disclosure protocol](versions/v19-local-maker/METADATA_DISCLOSURE_PROTOCOL.md).
...
Metadata verification uses independently grouped visible tuples, legal completions, validated mechanics, scalar population sums and proper scores. All group/score denominators and 432 paired estimates verify, with legal and positive-native ambiguity separate. Pooled replacement preserves one prior per endpoint and discards stale counts only in observed groups; 12 isolated controls pass before source admission. [Metadata review](versions/v19-local-maker/INPUT_PRIVILEGE_REVIEW_PROTOCOL.md), [replacement protocol](versions/v19-local-maker/POOLED_REPLACEMENT_PROTOCOL.md).
...
A separate bootstrap-count-matrix regroup reproduces all 9,120 estimates from original cells. Metadata omission now distinguishes mechanically legal completions from native policy support, with explicit zero-mass fallback and infinite-loss accounting. [Protocol](versions/v19-local-maker/INPUT_PRIVILEGE_PROTOCOL.md).

Independent input-pooling reconstruction has ten isolated passing controls, including a complete native fixture and deliberate numerical, evidence and denominator corruption. All 433 original deterministic outputs match both complete replays. The checker uses scalar empirical/prior reconstruction and the previously validated explicit path sum; numerical acceptance and all 9,120 paired estimates are now independently verified. The separate frozen regression passes 247 Ghost and five Torch controls across 61 files without skips; subsequent pooling/reviewer test files have their own targeted admissions. [Pooling review](versions/v19-local-maker/PRIMITIVE_POOLING_REVIEW_PROTOCOL.md), [regression](versions/v19-local-maker/REGRESSION_2026-09-22.md).

The primitive input-pooling comparison passed ten isolated controls and is source-admitted with both complete replays. It fixes correct undo-buffer pooling, equally sized wrong current-artifact pooling and unpooled controls before outcomes, retaining one prior per group. Retrospective evidence updating and the hidden-metadata boundary remain independent prepared alternatives. The original-tool advantage and changed-tool reversal are now accepted as exploratory constructed-method estimates. [Pooling protocol](versions/v19-local-maker/PRIMITIVE_ALIAS_POOLING_PROTOCOL.md).
...
The completed checker retains 15,360 paired conditional estimates, with both fit draws inside eight lineages and native/equal-query populations separate. All 831 deterministic outputs match both complete replays. Equal-query intervals are degenerate because the same predictions repeat across lineages; they do not measure training or feedback uncertainty. [Full review](versions/v19-local-maker/PRIMITIVE_FEEDBACK_REPORT.md).

Future-schedule numerical reconstruction and both complete replays now verify. Independent scalar storage regroup includes shared int32 schedules/membership and unavailable-checkpoint overhead; float64 representation bytes remain separate from archive size, workspace and speed. The primitive-feedback checker separately reconstructs observations, count updates, explicit paths, proper scores, native/equal-query strata and paired conditional intervals. Nine isolated controls pass; the complete independent numerical reconstruction and both original replays are now accepted. [Review protocol](versions/v19-local-maker/PRIMITIVE_FEEDBACK_REVIEW_PROTOCOL.md).


The future-schedule comparison now has identical original, adjacent and extracted-source output maps across all 88 deterministic files. Source archives, 500 source files, 72 inputs per execution and all scientific export members verify. Independent numerical/storage adjudication now verifies the scoped array-byte result. The primitive-feedback handler preserves the saved unit prior, deduplicates input reports, updates only observed tool rows, and exports exact budget slices under content hashes. Its full path-sum check is independent of state propagation; native and equal-query weighting remain separate. [Feedback protocol](versions/v19-local-maker/PRIMITIVE_FEEDBACK_PROTOCOL.md).
...
The [routine/request checker](versions/v19-local-maker/ROUTINE_REVIEW_PROTOCOL.md) independently reconstructs native path probabilities, full revision states, five policy distributions, task-reference ties, observed masks, outcomes and weighted strata. Eight isolated tests include a complete native fixture and deliberate policy/outcome corruption. The original and both complete replays agree on all 27 deterministic files; independent numerical adjudication now verifies every policy, outcome and population stratum. The historical full regression remains 120 Ghost and five Torch passes, and is not relabelled by these targeted checks.

The [routine revision result](versions/v19-local-maker/ROUTINE_REVISION_REPORT.md) reports success and stipulated net cost separately, retaining all purpose/skill/routine cells. Fourteen checker controls and independent recombination verify 864 paired contrasts. The new [transient filtering ruler](versions/v19-local-maker/TRANSIENT_FILTER_PROTOCOL.md) compares static, last-sixteen-source and coherent possible-switch hypotheses using supplied endpoint laws. Duplicate source identity, exact observation provenance and true-state coverage are separate from prediction; no model fit or historical-path recovery is claimed.

The [transient filtering checker](versions/v19-local-maker/TRANSIENT_REVIEW_PROTOCOL.md) reconstructs native endpoint laws, exact source assignments, scaled hypothesis products, current-state remapping, forecasts and tie-inclusive credible sets independently of the producer. All 35 original/replay files match; numerical adjudication awaits checker execution. Nine isolated tests pass, including a full native fixture and reader-label rejection. Both failed test attempts are retained and charged. Paired strata preserve all sixteen makers and average two observation draws within each lineage. The earlier complete regression remains 120 Ghost and five Torch controls; these targeted checks do not relabel it.

The [purpose-switch result](versions/v19-local-maker/TRANSIENT_FILTER_REPORT.md) now has independent native-path, observation, posterior, score and interval reconstruction. Every paired stratum retains equal maker weights and nested observation draws. The [unknown-change protocol](versions/v19-local-maker/UNKNOWN_CHANGE_PROTOCOL.md) removes supplied time/type in separate arms, preserves full joint hypothesis arrays, reconstructs native skill/tool support and treats identical no-change type controls as paired duplicates, not extra evidence.

The [unknown-change checker](versions/v19-local-maker/UNKNOWN_REVIEW_PROTOCOL.md) reconstructs native paths and complete time/type hypotheses using scaled products independent of the producer's log weights. It binds every float64 joint row to explicit hypotheses, reconstructs source-time remapping, type masses, predictions, scores and all paired strata. Ten isolated controls pass, including a complete native fixture and corrupted-mapping rejection. All 51 original/adjacent/extracted-source outputs match; independent reconstruction and all 4,400 paired contrasts now verify. The earlier full regression remains 120 Ghost and five Torch controls. Identical no-change type streams never add independent observations.

The [support-transfer protocol](versions/v19-local-maker/SUPPORT_TRANSFER_PROTOCOL.md) reuses both saved count-table draws and fit modes, reconstructs every parent forecast and original-law aggregate, and crosses changed/stay queries with original-law support categories. All methods and both law references share final 1/32 smoothing. Empty strata retain zero mass and undefined conditional loss; supplied skill, belief and operations remain explicit input privilege. No new model fit occurs.

The [changed-tool support-transfer checker](versions/v19-local-maker/SUPPORT_TRANSFER_REVIEW_PROTOCOL.md) independently reconstructs counts from ordered training episodes, both native rule populations, complete path propagation, saved uniforms, forecasts, support and every score/denominator. It reuses the validated independent native/support rulers. Eight isolated tests pass, including a complete two-rule native fixture, paired regroup and corruption rejection. All 47 original/adjacent/extracted-source outputs match; campaign reconstruction and all 3,420 paired mean/contrast metric estimates now verify. The earlier 120 Ghost/five Torch regression remains separate.

Changed-tool transfer now independently reconstructs both native populations, saved counts, uniforms, all forecasts, support and changed/stay strata. A separate event review verifies all 3,420 mean/contrast metric estimates and paired lineage intervals. Pooled support benefits and actual changed-endpoint reversals are reported separately; cross-rule native weights are part of the estimand. [Numerical report](versions/v19-local-maker/SUPPORT_TRANSFER_REPORT.md).
...
The [actual-change-timing checker](versions/v19-local-maker/CHANGE_TIMING_REVIEW_PROTOCOL.md) reconstructs scaled hypothesis products, joint mappings, source-time and query-time states, and paired midpoint references independently of producer filtering and scoring. Eight isolated controls pass, including a complete native fixture and corruption rejection. Original and both complete replays match 77 outputs. Independent reconstruction now verifies every score, and separate regrouping reproduces all 8,800 metric estimates and intervals; means and contrasts retain all actual locations, fixed checkpoints, draw means and paired lineage intervals. Parent controls remain identities, never new replicates.

The [source-identity omission comparison](versions/v19-local-maker/SOURCE_OMISSION_PROTOCOL.md) relabels saved presentations without changing timestamps or contents. All five filters and priors remain frozen. Independent streams must retain exact forecasts and scores. Known-answer counted-twice likelihoods, neutral laws, independent products and a complete native fixture precede source admission. True provenance remains evaluator-only; repeated timestamps remain visible, so this is identity-only omission.

The [source-omission checker](versions/v19-local-maker/SOURCE_OMISSION_REVIEW_PROTOCOL.md) reconstructs scaled hypothesis products, joint mappings, source-time and query-time states, and paired identity-aware references independently of producer filtering and scoring. Eight isolated controls pass, including a complete native fixture and corruption rejection. Original and both complete replays match 77 outputs. Numerical acceptance awaits serial reconstruction; means and contrasts retain all actual change/source conditions, fixed checkpoints, draw means and paired lineage intervals. Parent controls remain identities, never new replicates.

Source-identity omission now has independent reconstruction of all joint arrays, forecasts and scores, plus separate recombination of 8,800 metric estimates and paired intervals. Prediction similarity and current-maker set inclusion are adjudicated separately. Partial source restoration supplies explicit equivalences at fixed budgets and ages, deduplicates identical corrections, and checks zero/full/label-only identities. Corrected source IDs select independent factors; unchanged source timestamps select likelihood states. No additional endpoint observation or learned provenance detector is supplied.

The [provenance-restoration checker](versions/v19-local-maker/PROVENANCE_RESTORATION_REVIEW_PROTOCOL.md) independently rebuilds correction sets and source assignments before applying the separately implemented scaled-product and score kernels. Its mandatory native fixture, scalar products and corruption controls execute in the serial worker before campaign reconstruction; their admission is not a pass claim. Every unique correction is paired against zero/full restoration, with age-order comparisons at fixed budgets, aliases preserved without added replicates, and paired lineage/draw uncertainty. Reset history is reselected after correction. The adjacent and extracted-source portable replays each match all 71 deterministic outputs with separately verified timing hashes. Independent reconstruction and separate recombination of all 29,500 estimates now verify, including correction aliases, true-source counts and all five measures. Prediction equivalence is scoped to the final unknown-time/type cells; uncertainty inclusion and reset selection remain separate.

Retained filter-cost accounting reconstructs hypothesis sizes, source-factor counts, projection work and component storage without executing forecasts. Original and both replay timings are complete filter calls, not primitive/query benchmarks. The fixed repeated-query surface is symbolic and omits parsing, normalization, additions and hardware costs; it must never be reported as measured break-even. All retained score cells accompany cost cells.

The independent retained-cost checker reconstructs all 337,920 rows, 21,120 means, 184,320 timing calls and 80 symbolic surfaces without producer counting/filtering functions. Nine isolated known-answer, fixture and corruption controls pass. Timing quantization and source-byte versus resident-memory distinctions remain explicit. The fixed-tempering handler freezes three likelihood exponents with unchanged priors, exact exponent-one parent checks and independent scalar-product controls; it adds no fitted model or reader evidence. [Cost report](versions/v19-local-maker/FILTER_COST_REPORT.md).

The [fixed-tempering checker](versions/v19-local-maker/PROVENANCE_TEMPERING_REVIEW_PROTOCOL.md) uses independently rescaled likelihood products, stream reconstruction and separate scoring kernels to check the complete joint distributions and all fixed-power strata. Supplied endpoint laws retain their prior verified provenance; no new law validation is claimed. Mandatory scalar, complete native-fixture, pairing and corruption tests precede campaign reconstruction in the serial worker. Parent reader inputs remain unchanged; powered distributions and truth stay scientific/evaluator-only. Eight mandatory controls, all 135,168 rows and 7,920 independently recombined estimates now verify. Both full replays match all 111 deterministic outputs. Inclusion with larger sets, proper scoring and practical endpoint similarity remain separate claims; all fixed powers, draw means and strata are retained.
...
The [current-state future-transition review](versions/v19-local-maker/MARGINAL_TRANSITION_REPORT.md) independently reconstructs all 624 time cells, 29,472 Cartesian blocks and 19,968 summaries; all 23 deterministic outputs match both full replays. Scalar law arithmetic and complement pair counts provide independent checks. Full-simplex structural witnesses remain distinct from reachable posteriors and symbolic certificates.
...
Independent retrospective-forgetting reconstruction and separate original-row regroup now verify all forty window metrics, 1,120 draw strata, 560 law strata and 70 population rows. Regret percentages use paired population means. Structural byte comparisons keep shared indices and prior tables explicit; reference execution is not optimized compact latency. [Report](versions/v19-local-maker/RETROSPECTIVE_FORGETTING_REPORT.md).

The context-balanced retention successor selects by time and context only, matches recency source counts, and checks a fixed reversal of context labels. Exact mass preservation and unknown remainders are unchanged. Eighty-nine isolated controls include scalar Bayes, one-hot losses, source-order invariance, outcome-blind selection and corruption checks. [Contract](versions/v19-local-maker/BALANCED_RETENTION_ADMISSION.md).

Independent context-balanced retention reconstruction and separate original-row regroup now verify all seventy window metrics, 1,120 draw strata, 560 law strata and 70 population rows. All four order/count contrasts remain paired; regret percentages use paired population means. Structural byte comparisons keep shared indices and prior tables explicit; reference execution is not optimized compact latency. [Report](versions/v19-local-maker/BALANCED_RETENTION_REPORT.md).

The time-balanced successor retains sorted-time midpoint ranks at the same counts as recency. Selection uses no contexts or outcomes. All 89 isolated controls pass, including scalar Bayes, materialized compact joint states, explicit one-hot losses, all midpoint counts, irregular source times, permutation, an older informative source and remainder corruption. Complete-support timing reserves both full replays and independent reconstruction within 3,600 CPU seconds. [Contract](versions/v19-local-maker/TIME_RETENTION_ADMISSION.md).

Independent time-retention reconstruction and separate original-row regroup now verify all fifty window metrics, 1,120 draw strata, 560 law strata and 70 population rows. Both count contrasts remain paired; regret percentages use paired population means. Structural byte comparisons keep shared indices and prior tables explicit; reference execution is not optimized compact latency. [Report](versions/v19-local-maker/TIME_RETENTION_REPORT.md).

Time-localized retention separates the current checkpoint, the near half of strictly future steps, and the remaining far steps. Empty spans are unavailable: raw NaN masks and JSON nulls never count as zero error. Weighted recombination must reproduce full-horizon regret. All 221 isolated controls and complete-support timing pass. The first fixture failure compared unsupported reports with numerical zero; its corrected test explicitly checks the undefined mask, with unchanged scientific scoring. [Contract](versions/v19-local-maker/RETENTION_HORIZON_PROTOCOL.md).

Source-prior retention remains timing-blocked after separate repeated-array and accepted-uniform-output reuse repairs: 4,378.23 seconds projected against 3,600. Its latest implementation passes 113 isolated controls. The two-report partial kernel passes 38 direct Bayes and future-bound controls, but full-support timing projects 12,854.69 seconds against 3,600; a native handler is not admitted. Both source histories and failures are retained. [Source-prior blocker](../results/v19/G19-B-retention-prior-admission/BLOCKER.json), [two-report blocker](../results/v19/G19-B-two-report-admission/BLOCKER.json).

Query-context retention now retains separate errors for all four legal contexts across the entire future. Equal-context regret recombination and maximum-error recombination are distinct controls. All 97 isolated checks pass; complete-support timing precedes source freeze, original execution and both full replays. Numerical acceptance still requires independent reconstruction and original-row regroup. [Admission](versions/v19-local-maker/RETENTION_QUERY_ADMISSION.md).
...
The context-disclosed aggregate prototype passes 30 scalar Bayes, one-hot loss, permutation, support and corruption controls before and after one linear projection optimization. Both complete-support timing admissions exceed the inclusive 3,600-second cap; no native scientific run is admitted. This is a retained engineering blocker, not a scientific null. [Admission and blocker](versions/v19-local-maker/AGGREGATE_CONTEXT_STATE_ADMISSION.md).

The three-prior aggregate prototype retains 25 float64 values per future group plus 24 copied-endpoint probabilities. Joint numerators mix before normalization; averaging normalized vertex posteriors is a separate rival. Scalar source-level Bayes, explicit proper losses, vertex/constant/certain-copy identities, permutations, support/mass corruption and complete synthetic dispatch are tested. The source-prior basis prototype passed 81 isolated controls, but complete-support timing projects 11,966.09 CPU seconds against its 3,600-second cap. No native scientific job was admitted; this is an engineering blocker, not a scientific null. [Admission](versions/v19-local-maker/AGGREGATE_PRIOR_BASIS_ADMISSION.md).

The storage-precision diagnostic retains actual float32/16 state arrays beside the exact float64 state. Both raw casting and one mass-preserving reconstruction query in float64. Underflow, normalization drift, lost support and unusable report mass remain separate from probability-weighted defined errors. Fixed probability strata retain mass and error sums without empty-stratum imputation. The finite-precision aggregate-state prototype passed 60 isolated controls, but complete-support timing after one storage repair projects 6,007.00 CPU seconds against its 3,600-second cap. No native scientific job was admitted; this is an engineering blocker, not a scientific null. [Contract](versions/v19-local-maker/AGGREGATE_PRECISION_ADMISSION.md).
