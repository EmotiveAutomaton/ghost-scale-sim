...
The [unknown-change checker](versions/v19-local-maker/UNKNOWN_REVIEW_PROTOCOL.md) reconstructs native paths and complete time/type hypotheses using scaled products independent of the producer's log weights. It binds every float64 joint row to explicit hypotheses, reconstructs source-time remapping, type masses, predictions, scores and all paired strata. Ten isolated controls pass, including a complete native fixture and corrupted-mapping rejection. All 51 original/adjacent/extracted-source outputs match; numerical acceptance awaits checker execution. The earlier full regression remains 120 Ghost and five Torch controls. Identical no-change type streams never add independent observations.
