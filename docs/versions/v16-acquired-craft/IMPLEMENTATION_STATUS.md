...
artifact. Exact likelihoods match independent scalar enumeration on the tiny
world. Independent interpreters and reducers verify programs, paired outcomes,
constructor/maker bootstrap intervals, and every reported scout aggregate.

K01/K03/K04/K05 have completed acquisition, adaptation, option and allocation
scouts. K02/P01/P02/P03 have completed reading
scouts. O01/O02/O03/O04 and S01/S02/S04/S05 have completed behavior scouts.
Their per-condition samples have 64 maker packets and eight constructor draws;
their claims remain descriptive. These are separate clustered studies, not one
pooled sample. Typed contrasts keep execution, instrument validity and criteria
separate. Expansion and confirmation remain open.

The inquiry machinery learns command mappings from executed examples and
compiles previously untrained two-cell compositions. Its finite episodes include
...
bounded, habitual and (in the graphic world) option production. Its independently
regenerated maker draws and scored predictions are retained in the mechanism
packet. This uses separately configured public world interfaces; it does not
establish transfer of fitted graphic-world parameters to assembly.

## Access qualification

The original native, acquisition, reading and behavior scouts passed serialized
public bytes to fixed reader functions within the evaluator process. Public
and private records were separate files, but that was not a separate operating
system process. Do not retroactively describe those scouts as process-isolated.

The new fixed-code reader worker receives only public JSON frames. Following
trusted imports, an audit guard denies file, subprocess and network access.
The known-answer test both reproduces a prediction and denies an attempted read
of a synthetic private answer file. This is a control on trusted scientific
code, not a general sandbox for hostile native programs. Inquiry uses this
worker from its first scout. The separate
[access replay](../../../results/v16/reader-access-replay-1/REPLAY.json) reproduced
all 166,018 retained requests across 4,162 unit records, including two fixtures,
with exact output agreement. Reader replay and full world replay differ.

## Runtime and remaining work

An actual supervisor was interrupted after saving work. Resume preserved clocks,
source locks and completed unit bytes, and the OS lock rejected a second owner.
The original K01 admission receipt is preserved. A later operational checkpoint
adapter validates its unchanged scientific source despite added CLI routes;
it does not re-prepare completed scientific work.

S03 completed its acquired-routine mismatch scout, with two committed decision
phases, paid inspection, actual assembly repairs and independent scoring. Its
...
was preserved without dependency synchronization. The final V15 relaunch had
