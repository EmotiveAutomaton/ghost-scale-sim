# Forecast-horizon retention: execution and replay review
...
Eighty-three metrics keep unsupported mass, supported expected squared regret, largest
...
executions. All 872 original frozen sources and
...
preserved in 20 scientific/evaluator archives. Reader inputs are
unchanged. Replay establishes reproduction; numerical acceptance remains pending.

The independent checker classifies each future offset into current, near or far,
retains empty spans as undefined, and verifies count-weighted recombination of
whole-future regret. Maximum errors do not obey that averaging identity. It
selects exact rational interval midpoints, constructs
...
All 380 isolated controls pass without skips or warnings. Controls include
...
and NaN-mask corruption, summary/pair/roster/timing/binding faults, independent span coverage through 129 forecast times, empty and one-step
futures, weighted recombination, native integration and portable dispatch. The absolute tolerance remains one trillionth.
...
1048.50 CPU seconds, including a 50 percent
margin and 180 seconds I/O allowance. Inclusive admission is
2660.31/3600 seconds, including prior
attempts, all three executions, tests, checker, 150 seconds original-row regroup
and 200 seconds operations. 875 checker sources and
1066 inputs are frozen. The existing serial queue was observed running
that checker below normal priority. All new controls and verification costs are charged to this same card. One
export attempt stopped at a conservative decimal archive-size guard; its receipt
and partial exports are retained. The exact 50,156,948-byte compressed summary
is preserved in an archive below 50 MiB. Scientific scoring is unchanged.
...
cells, retaining all 83 metrics and both spaced-minus-recent paired contrasts.
Both draws stay inside each equally weighted law. Empty forecast spans remain null, never zero. Source/report counts are not
...
consumed with shared ownership recorded. Highest-probability source selection and query-specific retention remain
prepared, unimplemented alternatives. Source-prior retention, two-report
dependence, source disclosure and prior-conditional reachable updating retain
timing blockers.
A3/C2/F2 prerequisites and untouched confirmation lineages remain protected.
V19 stays active with 85 independently accepted batches; resource ceilings and
immutable reporting times remain unchanged.

[Frozen contract](RETENTION_HORIZON_PROTOCOL.md).
[Independent protocol](RETENTION_HORIZON_REVIEW_PROTOCOL.md).
[Execution review](../../../results/v19/G19-B-retention-horizon-1/EXECUTION_REVIEW.json).
[Evidence roles](../../../results/v19/G19-B-retention-horizon-1/EXECUTION_EVIDENCE_ROLES.json).
